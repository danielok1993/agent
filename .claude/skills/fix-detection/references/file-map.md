...
| Room-shape deltas the verdict report cannot see | `python tools/compare_room_shapes.py sNN …` (IoU < 0.995 only); `python tools/diff_room_polygons.py [sNN …]` (EVERY changed polygon + added/removed/moved entities); `python tools/room_shape_crop.py sNN room_00NN [--after ID] [--only before\|after]` (before\|after crop, counterpart matched by IoU) — run crops AFTER compare_sweeps, which wipes `outputs/compare/<slug>/` |
| Barrier-level attribution (walls/rooms) | `python tools/diff_wall_network.py sNN X0 Y0 X1 Y1 [--base REF]` — faces/segments/fills in a probe box, working tree vs a detached worktree of `main`, one-sided differences listed; `… --idx N …` traces where path indices end up (lattice in/kept/out, faces, segments). Use it before reading verdict lines when a room merges or a phantom appears |
| Collinear-merge anchor probe | `python tools/probe_merge_anchor.py sNN [--support] [--list] [--json f]` — per merged run: seed vs longest member, displacement, angled members, and each member line's collinear strong-ink support at several reaches (reads the `trace` hook of `_merge_collinear_segs`) |
