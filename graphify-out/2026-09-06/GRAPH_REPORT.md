...
## Corpus Check
- 357 files · ~1,034,290 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5206 nodes · 12996 edges · 281 communities (199 shown, 82 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 995 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b52384e0`
...
1. `PathPrimitive` - 294 edges
2. `Candidate` - 189 edges
3. `PageData` - 178 edges
4. `TextSpan` - 160 edges
5. `detect_wall_network()` - 139 edges
6. `Region` - 119 edges
7. `PageScales` - 104 edges
8. `ScaleInfo` - 100 edges
9. `rooms_for()` - 95 edges
10. `_line_length()` - 85 edges
...
Cohesion: 0.12
Nodes (11): _contains(), select_room_scale(), effective_denominator(), mm_per_px(), px2_to_m2(), px_to_m(), Pixel ↔ metre conversion.  Everything downstream of extraction/extractor.py is 1, Nominal beats raw so 1:50 sheets compute exactly (scale/factor.py rule). (+3 more)

### Community 1 - "Door Assembly & Heuristics Core"
Cohesion: 0.09
Nodes (22): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode. (+14 more)
...
Cohesion: 0.07
Nodes (68): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+60 more)
...
Cohesion: 0.22
Nodes (20): _arc_corners(), _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _collect_door_leaves(), _collect_linework_door_leaves(), _find_thin_rectangle_cycle(), _is_door_leaf() (+12 more)
...
Cohesion: 0.07
Nodes (30): detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf(), OpenVTests, parked_stack() (+22 more)
...
Cohesion: 0.11
Nodes (21): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, diagonal_window(), framed_triple_window(), path(), quad() (+13 more)

### Community 18 - "windows.py"
Cohesion: 0.05
Nodes (27): dump_truth(), Path, Create the unlabeled ground-truth file for a newly adopted sheet., Write a sheet's verdicts back to tests/ground_truth/<slug>.json.      Round-trip, truth_path(), write_empty_truth(), address_match(), Shared address-detection patterns for corpus hygiene checks.  Two callers share (+19 more)
...
Cohesion: 0.09
Nodes (33): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document() (+25 more)
...
Cohesion: 0.17
Nodes (14): PageTruth, TruthItem, match_entities(), MatchResult, evaluate_page(), Score one page's entities against its three verdict lists., ClassifyTests, entity() (+6 more)
...
Cohesion: 0.11
Nodes (13): _collect_fill_rings(), detect_wall_network(), _fill_key(), _fill_ring_components(), _is_light_pen(), Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor. (+5 more)

### Community 41 - "_collect_wall_faces"
Cohesion: 0.16
Nodes (13): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), Every text scale near enough to this region to be about it, nearest first., The scale governing one region, or None.      `viewports` must arrive smallest-b, How far a text span sits from a region, or None if it is not near it.      Horiz (+5 more)
...
Cohesion: 0.09
Nodes (32): _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _fit_circle_3pt(), _prune_arc_cycle_caps(), _prune_arc_spurs(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The (+24 more)
...
Cohesion: 0.14
Nodes (10): DegenerateCompanionTests, DoorAssemblyTests, path(), PolylineArcBinCapTests, A zero-length stroke is never a leaf edge (s11's double doors).      CAD exports, 270-degree polyline arc with 16 segments — far wider than a quarter-circle door, Compute 15-degree angle bins for line segments — fixture sanity helper., Tests for the DOOR_POLYLINE_MAX_ANGLE_BINS cap that rejects furniture/appliance (+2 more)
...
Cohesion: 0.07
Nodes (31): _item(), load_truth(), The user's per-sheet verdicts, and how they are read.  One file per sheet under, Regression corpus: fixture resolution, ground truth, matching, and the sweep., iou(), BBox, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, IouTests (+23 more)

### Community 107 - "vline"
Cohesion: 0.16
Nodes (6): EntranceDoorTests, quarter_arc_lines(), Regression + new-feature tests for entrance-door threshold-line handling.      G, Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, rect_lines(), SingleLineLeafTests
...
Cohesion: 0.11
Nodes (15): _bridge_white_runs(), _equivalent_sides(), Band-shaped convex hulls closing the gaps in accepted white-ring runs.      gate, (short, long) of the rectangle with this polygon's area and perimeter.      The, _hface(), _bridge_white_runs is detect_rooms's ONLY production call site     (detection/ro, A bare horizontal wall-face _Seg for isolated merge-tolerance tests., Isolates _merge_collinear_segs's offset-tolerance scaling directly —     the exa (+7 more)
...
Cohesion: 0.08
Nodes (14): canonical_denominators(), cluster_denominators(), denominator_from_c(), Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present., The 1:N denominator for a /Measure /X conversion factor., Scale arithmetic: PDF /Measure conversion factors to a 1:N denominator.  Every n, The closed set a user may supply for a sheet nobody could scale.      Both prope (+6 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.06
Nodes (54): _accept_jamb_rings(), _building_masses(), _clip_plug_tails(), _contains_text(), detect_rooms(), _doorway_pens(), _drop_window_exterior_sides(), _edge_face_cover() (+46 more)
...
Cohesion: 0.09
Nodes (17): DeliberateVerdictsTests, EnterWithNothingTickedTests, entity(), _HeadlessReviewSheetTests, Path, tools/review.py's `_pick` / `review_sheet`, driven through the real InquirerPy p, Shared fixture: one fake corpus sheet with a persisted sweep run.      Mirrors t, The C1 regression test.      Against the old `inquirer.fuzzy(multiselect=True)` (+9 more)
...
Cohesion: 0.05
Nodes (41): _bbox_area(), _bbox_expanded(), _point_in_bbox(), _point_to_segment_distance(), BBox, Minimum distance from point p to line segment ab., Minimum distance between two line segments., _segments_min_distance() (+33 more)
...
Cohesion: 0.08
Nodes (36): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sha256_of() (+28 more)

### Community 149 - "_arc"
Cohesion: 0.07
Nodes (13): TestCase, Path, Skip helper for tests that need a real corpus sheet.  Corpus knowledge lives in, Return the sheet's path, or skip the test with an actionable message., require_sheet(), Every primitive, span AND image must land in the declared frame., A saved 200x400pt PDF with two lines, a word and an image, rotated.      Saved a, Builds all four rotations once; each test reopens what it needs. (+5 more)
...
Cohesion: 0.08
Nodes (35): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), The per-region scale table printed after each page., scale_table(), detection_scale() (+27 more)
...
Cohesion: 0.24
Nodes (9): Scale-factor behavior of walls/rooms gates: identity at 1.0, shrunk-world at 0.5, Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), rooms_for(), shrink(), TestRoomsScaled, TestWallNetworkScaled (+1 more)
...
Cohesion: 0.33
Nodes (3): The two classes the corpus sweep exposed on the first cut of the     rule: a wal, 135-degree strokes from face x0 to face x1, clipped to the band., TestDashRowDiscriminators
...
Cohesion: 0.05
Nodes (59): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, The whole page as one document. (+51 more)
...
Cohesion: 0.07
Nodes (22): DimensionMatch, measured_denominator(), BBox, The drawing's measured scale: the median implied denominator over the     matche, check_dimensions(), check_door_leaves(), _fmt_scale(), leaf_width_px() (+14 more)
...
Cohesion: 0.04
Nodes (42): _arc(), _chain(), _double_arc(), PruneArcCycleCapsTests, PruneArcSpursTests, A closed 4-segment loop has every vertex at degree 2 — no leaf         exists to, 11-segment arc whose far endpoint is a degree-3 junction because         two 1-s, linework_1318 shape: 11-segment arc whose far endpoint becomes a         degree- (+34 more)
...
Cohesion: 0.11
Nodes (19): _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), Type-specific NMS: higher confidence wins when two candidates overlap.      For, _suppress() (+11 more)
...
Cohesion: 0.08
Nodes (15): Bbox short-end edges of a sliding door: across the wall, never wall plane., Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, _restrict_swing_plugs(), _sliding_end_edges(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, Single swing doors: plugs live on the hinge edges, one wall plane.      Geometry, A filled wall band exported as two triangles (CAD fill triangulation).      Each, s15: the "steel ridge beam" line — a dashed line drawn as a row of     14.8px pi (+7 more)
...
Cohesion: 0.18
Nodes (14): main(), draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1., The two step-9 pictures (guarded so crop_step10.py can import the drawing helper, side_by_side(), _best_match() (+6 more)
...
Cohesion: 0.04
Nodes (79): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bbox_union(), _interval_overlap(), _line_angle_deg() (+71 more)

### Community 251 - "resolve_heights"
Cohesion: 0.13
Nodes (15): build_parser(), cmd_extract(), cmd_inspect(), main(), parse_page_spec(), positive_metres(), argparse type: a positive, finite height in metres., Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]. (+7 more)

### Community 255 - "render_page_svg"
Cohesion: 0.04
Nodes (69): _apply(), _as_transform(), classify_page(), _color_tuple(), extract_document(), extract_images(), extract_page(), extract_paths() (+61 more)

### Community 256 - "swing_door"
Cohesion: 0.11
Nodes (17): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, leaf_pair_door(), open_v_door(), page(), prim(), End-to-end door scale behavior on FAITHFUL 1:100 fixtures.  A faithful 1:100 exp, open_v (detection/doors/folding.py::_open_v_match) reads three gates:     gates. (+9 more)
...
Cohesion: 0.05
Nodes (81): _line_length(), _band_fill_cover(), _band_has_through_hatch(), _band_has_wall_material(), _band_material_ts(), _claims_far_side_pair(), _claims_far_side_sparse(), _claims_interior_pair() (+73 more)
...
Cohesion: 0.06
Nodes (20): probe_box.py <slug> <seal> x0 y0 x1 y1 [door_id ...]  Run the harness at the giv, tap_fsc(), tap_gap(), Step-11 checkpoint pictures: s01 at its true factor (0.542), the share gate alon, census(), The shipped rule AS IMPLEMENTED (rooms._doorway_pens on the room stage's own pas, Render each stroke pen's face ink (network.faces, stroked) over the sweep's rend, Per confident door with an INTERRUPTED plug (the doorway signature), per pen: wh (+12 more)
...
Cohesion: 0.11
Nodes (13): qualifying_clip_rects(), Read scissor rects off a fitz.Page and gate them. Returns [] if the     PDF expo, assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, Golden segmentation results on the corpus reference sheets (s01, s02, s11).  Mea, Load-bearing golden for SEGMENT_MAX_DEPTH = 7: at 6 the first-floor     plan and, This sheet carries full-page border rules. With the span filter applied     the, s15 measured 2026-08-13: 214 text spans bridge every gutter, so the     text-inc (+5 more)
...
## Knowledge Gaps
- **738 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+733 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `TestMarkerRings` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `client.py`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `Room Detection Tests`, `TestThickMaterialPairs`, `arcs.py`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `plumber.py`, `test_curve_arc_garden_doors.py`, `renderer.py`, `PruneArcSpursTests`, `batch_extract.py`, `TestVectorTextExclusion`, `_dedupe_openings`, `_vector_text_indices`, `test_window_detection.py`, `TestCheckDoorLeaves`, `TestWindowExteriorSide`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `TestWindowGates`, `TestWindowTightPairInterior`, `_covers`, `TestDashRowDiscriminators`, `collect_room_spans`, `squat_cap_window`, `wall_band_h`, `resolve_room_labels`, `bezier_arc`, `test_door_assembly.py`, `detect_doors`, `DoorV2OpeningCheckTests`, `vline`, `_find_openings`, `app.py`, `ablate.py`, `TestBandPocket`, `_collect_wall_faces`, `label_rooms`, `TestNetworkQueries`, `_is_arc_like`, `test_batch_extract.py`, `framed_triple_window`, `_segments_min_distance`, `render_page_svg`?**
  _High betweenness centrality (0.143) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `_fit_circle_3pt` to `test_layout_segmenter.py`, `Debug Trace Collector`, `client.py`, `ShaMismatchAgainstTruthTests`, `resolver.py`, `TestCliEquivalence`, `review.py`, `hline`, `ScaleInfo`, `DoorV2OpeningCheckTests`, `test_layout_golden.py`, `_dedupe_openings`, `TestXYCut`, `TestWindowTightPairInterior`, `resolve_page_regions`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `resolve_heights`, `render_page_svg`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Assembly & Heuristics Core` to `swing_door`, `EntranceDoorTests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `_dedupe_openings`, `ShaMismatchAgainstTruthTests`, `_check_opening_clear`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `review.py`, `viewport_bbox_to_px`, `plumber.py`, `ScaleInfo`, `PruneArcSpursTests`, `TestXYCut`, `TestCheckDoorLeaves`, `TestWindowExteriorSide`, `EntranceDoorTests`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `label_rooms`, `test_batch_extract.py`, `TestWindowGates`, `TestWindowTightPairInterior`, `_covers`, `collect_room_spans`, `squat_cap_window`, `resolve_room_labels`, `TestMarkerRings`, `test_door_assembly.py`, `detect_doors`, `vline`, `_find_openings`, `EntranceDoorTests`, `ablate.py`, `TestBandPocket`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_batch_extract.py`, `framed_triple_window`, `_segments_min_distance`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Are the 148 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 148 INFERRED edges - model-reasoned connections that need verification._
- **Are the 95 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `CrossGates`) actually correct?**
  _`Candidate` has 95 INFERRED edges - model-reasoned connections that need verification._