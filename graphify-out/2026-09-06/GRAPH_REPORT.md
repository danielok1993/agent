...
## Corpus Check
- 352 files · ~976,307 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5184 nodes · 12936 edges · 290 communities (203 shown, 87 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 991 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `babe8d3e`
...
Cohesion: 0.07
Nodes (22): draw_overlay(), One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, Region, _page_summary_dict(), DetectionScale, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict(), True when a scale resolved and quantities exist. (+14 more)

### Community 1 - "Door Assembly & Heuristics Core"
Cohesion: 0.12
Nodes (17): Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode., TestDiagonalWindowSeal, BBox, s18's lining at identity scale: a 12x97.5px box whose bottom-right         corne (+9 more)
...
Cohesion: 0.11
Nodes (38): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), _draw_regions(), _load_font(), BBox, Image (+30 more)
...
Cohesion: 0.12
Nodes (20): dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), Path (+12 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.07
Nodes (61): _component_indices(), _dedupe_door_components(), _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Prefer the strongest door when two candidates use the same primitives. (+53 more)
...
Cohesion: 0.08
Nodes (45): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+37 more)
...
Cohesion: 0.08
Nodes (26): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), Entity (+18 more)
...
Cohesion: 0.13
Nodes (12): leaf_pair (detection/doors/sliding.py) reads gates.DOOR_SLIDE_PANEL_MIN_THICKNES, TestSlidingScaleBehavior, detect(), LeafPairTests, ParkedLeafTests, PocketLeafTests, s04 door_0014: a pocket door drawn as a bare stroked qu (4.7x92px,     0.56px) h, A closed 4-segment stroked (fill-less) rectangle of `l` items. (+4 more)
...
Cohesion: 0.24
Nodes (14): DetectionPage, load_detection_pages(), Every detected page of the sheet (or only `pages`, 1-based)., sheet_pdf(), dump(), _fmt_face(), _fmt_seg(), _in_box() (+6 more)
...
Cohesion: 0.11
Nodes (33): _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower, _axial_offsets(), _collect_slide_panels(), _corners_bbox(), _detect_sliding_doors() (+25 more)

### Community 102 - "DoorV2OpeningCheckTests"
Cohesion: 0.10
Nodes (19): _collect_fill_rings(), detect_wall_network(), _fill_key(), _fill_ring_components(), _is_light_pen(), Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor. (+11 more)
...
Cohesion: 0.09
Nodes (21): One fixture per paper-space family (spec §Testing). Each fails if its     named, Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestPaperInvariance, hline(), horizontal_window(), A clean 2-line capped rectangle IS a window on 5-1133 (see Window B:         two, 5-1133 FP window_0006: 3 short parallel lines whose opening (15px) is         fa (+13 more)
...
Cohesion: 0.11
Nodes (21): _bridge_white_runs(), _equivalent_sides(), _fill_seam_indices(), _fill_seams(), _FillRing, _is_background_fill(), _rate_fill_classes(), Classify each fill color as wall material (True) or furniture (False).      Vect (+13 more)
...
Cohesion: 0.05
Nodes (60): _accept_jamb_rings(), _building_masses(), _clip_plug_tails(), _contains_text(), detect_rooms(), _door_plugs(), _doorway_pens(), _drop_window_exterior_sides() (+52 more)

### Community 120 - "TestNetworkQueries"
Cohesion: 0.11
Nodes (12): door_candidate(), Rect room with a 45px doorway gap in the top wall (240..285)., Closed stroked (fill-less) polyline exploded into chained `l` items., Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, rooms_for(), stroked_ring_path(), TestBboxSealFloor (+4 more)
...
Cohesion: 0.11
Nodes (23): _cross_validate(), Validate doors/windows against the wall-centerline network.      Doors keep the, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried., WallFace, WallSegment, continuous_h_wall(), door() (+15 more)
...
Cohesion: 0.10
Nodes (9): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, prim(), TestAssemblyGatesThreading, TestCrossGates, TestDoorGatesConstruction, TestFoldingGatesThreading, TestLeafGatesThreading (+1 more)
...
Cohesion: 0.16
Nodes (9): _door_attribute_overlay(), finalize_candidates(), Selected door-evidence keys to merge into Entity.attributes. {} for None / non-d, Promote candidates to entities, applying the offline confidence floors.      Gem, assembly_type must reach Entity.attributes through the pipeline passthrough., cand(), finalize_candidates applies the offline confidence floors unconditionally., TestFinalizeCandidates (+1 more)
...
Cohesion: 0.14
Nodes (17): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, page_dimensions(), The page's matches, on the FULL page — the takeoff's convention, and     what ru, fill_chains(), main() (+9 more)
...
Cohesion: 0.27
Nodes (11): _doc(), load_record(), mark_awaiting_review(), mark_awaiting_scale(), mark_failed(), mark_processing(), The takeoffs/{takeoffId} record: reading it, guarding it, moving its status.  Re, Measured, but nothing on the run carried a readable scale.      The document is (+3 more)
...
Cohesion: 0.07
Nodes (43): attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, room_dict(), Heights (+35 more)
...
Cohesion: 0.09
Nodes (17): DimensionMatch, assess_scale(), check_dimensions(), check_door_leaves(), _fmt_scale(), leaf_width_px(), _positive(), Is the resolved scale believable? Two checks that read the drawing itself.  A sc (+9 more)
...
Cohesion: 0.12
Nodes (10): band_segments(), hatch(), A band hatched THROUGH — every diagonal stroke ending on both faces — is a drawn, 45° strokes across the band; inset > 0 stops them short of each face., WALL_THROUGH_HATCH_MAX_PX is 72px — 610mm at 1:50 (W-gate census     2026-09-04), ThroughHatchBandTests, ThroughHatchCapReferenceTests, Horizontal wall drawn as two stroked faces. (+2 more)
...
Cohesion: 0.07
Nodes (35): classify(), diff_entities(), entity_id -> verdict, using the sweep's own matching order     (confirmed first,, (kept, removed, added): `before` entities paired to `after` entities by     type, PageTruth, SheetTruth, TruthItem, evaluate_page() (+27 more)
...
Cohesion: 0.19
Nodes (6): A lone stroked, unfilled `qu` item — a joinery-pen box., s04 BATHROOM 01 (room_0000, door_0002): the structural opening is     112px wide, s03 corridor room_0014: the jamb nibs beside door_0007/door_0019 are     closed, stroked_box_path(), TestDoorLiningRings, TestJambNibRings
...
Cohesion: 0.31
Nodes (8): BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, _wall_runs_through(), _clean(), main(), Record, per sheet, the populations each scaled gate discriminates, with world-mm, Annotate each entity with confirmed / fp / unreviewed / deferred., verdicts()
...
Cohesion: 0.09
Nodes (18): TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., Interrupted-run plugs need jambs that REACH the plug band and a mid     that is, A filled wall band exported as two triangles (CAD fill triangulation).      Each, A chimney breast / pier drawn as a closed box on the room side of a     wall ban, s15: the "steel ridge beam" line — a dashed line drawn as a row of     14.8px pi, ROOM_OPENING_SEAL_PX is 15px (127mm at 1:50; W-gate iteration 3 step     7, 2026 (+10 more)
...
Cohesion: 0.15
Nodes (10): _contains(), _ratio_pair(), Which drawing scale a room is measured at, and whether it can be trusted.  Pages, (w_ratio, h_ratio) of page over ISO size, orientation-matched., sheet_size_tokens(), verify_sheet_size(), effective_denominator(), Nominal beats raw so 1:50 sheets compute exactly (scale/factor.py rule). (+2 more)
...
Cohesion: 0.32
Nodes (6): census(), _gt_class(), _mrr(), Step 13, second census — the ENTERED narrow rooms: every emitted room whose mini, drop(), Where exactly the four s17 strips' long edges lie: every polygon vertex on the l
...
Cohesion: 0.13
Nodes (14): Exception, NotFound, Base class. `code` is a Firebase callable error code string., TakeoffFnError, TakeoffRequest, RunResult, FakeAuth, FakeReq (+6 more)
...
Cohesion: 0.07
Nodes (33): agreement(), measured_denominator(), BBox, The drawing's own dimension strings as a scale MEASUREMENT.  "3600" written besi, The drawing's measured scale: the median implied denominator over the     matche, How the measured scale relates to a claimed one: "ok" within     DIM_AGREE_TOL,, _effective_denominator(), _from_dimensions_warning() (+25 more)
...
### Community 258 - "TestWindowGates"
Cohesion: 0.53
Nodes (5): census(), _gt_class(), Step 13 census — `_is_band_pocket`'s spacing ceiling, WALL_MAX_THICKNESS_PX -> W, _room_diff(), _rooms()
...
Cohesion: 0.33
Nodes (3): nearest_collinear(), Collinear-face census: for every door edge end on every sheet (at the sheet's fa, lines: list of (p1, p2, meta). Outward direction is (ux, uy).
...
Cohesion: 0.04
Nodes (95): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), Scalar projection of p onto the unit axis (dx, dy) from origin., _band_fill_cover(), _band_has_through_hatch(), _band_has_wall_material() (+87 more)
...
Cohesion: 0.06
Nodes (29): main(), Step-11 checkpoint pictures: s01 at its true factor (0.542), the share gate alon, census(), The shipped rule AS IMPLEMENTED (rooms._doorway_pens on the room stage's own pas, Render each stroke pen's face ink (network.faces, stroked) over the sweep's rend, census(), edge_line(), owners_for() (+21 more)

### Community 276 - "TestCliEquivalence"
Cohesion: 0.07
Nodes (13): Client, init_client(), Vertex AI client construction.  Per-candidate validation was removed on 2026-07-, FakeBlob, FakeBucket, FakeDb, FakeDoc, The function must not change detection results.  tools/regress.py guards the CLI (+5 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `TestMarkerRings` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `QuadPerimeterTests`, `client.py`, `client.py`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Wall Network Construction & Tests`, `Room Polygonization Internals`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `Room Detection Tests`, `TestThickMaterialPairs`, `arcs.py`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `TestSwingHingePlugRestriction`, `plumber.py`, `test_curve_arc_garden_doors.py`, `renderer.py`, `PruneArcSpursTests`, `_collect_wall_faces`, `TestVectorTextExclusion`, `_dedupe_openings`, `test_window_detection.py`, `TestCheckDoorLeaves`, `TestWindowExteriorSide`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `HygieneRuleTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `label_rooms`, `test_batch_extract.py`, `_covers`, `TestDashRowDiscriminators`, `collect_room_spans`, `squat_cap_window`, `wall_band_h`, `resolve_room_labels`, `DoorV2OpeningCheckTests`, `bezier_arc`, `detect_doors`, `vline`, `_find_openings`, `app.py`, `ablate.py`, `TestBandPocket`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `_is_arc_like`, `test_door_assembly.py`, `framed_triple_window`, `_segments_min_distance`, `render_page_svg`?**
  _High betweenness centrality (0.147) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `_fit_circle_3pt` to `Pipeline Orchestration & Extraction`, `test_layout_segmenter.py`, `Debug Trace Collector`, `client.py`, `Wall Network Construction & Tests`, `ShaMismatchAgainstTruthTests`, `resolver.py`, `TestCliEquivalence`, `viewport_bbox_to_px`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `TestXYCut`, `TestWindowTightPairInterior`, `TestWindowTightPairInterior`, `TakeoffRequest`, `detect_doors`, `resolve_page_regions`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `resolve_heights`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Assembly & Heuristics Core` to `Pipeline Orchestration & Extraction`, `EntranceDoorTests`, `Door Detection & Tests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `_dedupe_openings`, `Double-Arc Split Tests`, `ShaMismatchAgainstTruthTests`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `TestSwingHingePlugRestriction`, `plumber.py`, `PruneArcSpursTests`, `batch_extract.py`, `TestXYCut`, `TestCheckDoorLeaves`, `TestWindowExteriorSide`, `EntranceDoorTests`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `label_rooms`, `test_batch_extract.py`, `collect_room_spans`, `squat_cap_window`, `resolve_room_labels`, `TestMarkerRings`, `vline`, `_find_openings`, `EntranceDoorTests`, `ablate.py`, `TestBandPocket`, `qualifying_clip_rects`, `TestNetworkQueries`, `_is_arc_like`, `test_door_assembly.py`, `_segments_min_distance`, `render_page_svg`?**
