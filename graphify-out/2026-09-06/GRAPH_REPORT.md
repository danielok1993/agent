...
## Corpus Check
- 314 files · ~780,872 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5016 nodes · 12583 edges · 277 communities (200 shown, 77 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 964 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7a32db42`
...
8. `rooms_for()` - 93 edges
9. `ScaleInfo` - 91 edges
...
## Communities (277 total, 77 thin omitted)
...
Cohesion: 0.11
Nodes (18): assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, _attach_text_spans(), page_fallback_region(), Grow paths-only boxes to absorb the text spans beside them.      The tier-2 cut, Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page() (+10 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.08
Nodes (40): _load_font(), baseline_dir(), baseline_run(), classify(), compare(), compare_runs(), _crop(), diff_entities() (+32 more)
...
Cohesion: 0.12
Nodes (12): detect_wall_network(), _fill_ring_components(), _fill_seam_indices(), _is_light_pen(), Path indices of fill-ring seams — see _fill_seams., Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor. (+4 more)

### Community 7 - "Debug Trace Collector"
Cohesion: 0.13
Nodes (11): Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), detect_schedules(), Schedule detection — tables carry real bboxes.  detect_schedules used to emit bb, extract_plumber_page must surface each table's bbox, normalized to     150-DPI p, _table(), TestDetectSchedulesBBox (+3 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.11
Nodes (15): _native_curve_chains(), Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, ChainedCurveSwingDetectionTests, _circle_arc_chain(), _curve(), FitCircle3PtTests, NativeCurveChainsTests, Three collinear points have no unique circumscribed circle. (+7 more)
...
Cohesion: 0.14
Nodes (14): _centre_in_any(), filter_page_data(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, A copy of page_data holding only primitives whose bbox centre falls in     one o, Text spans inside the given regions. Used to scope schedule detection to     sch, region_text_spans(), Page segmentation: split a sheet into its constituent drawings. (+6 more)

### Community 17 - "arcs.py"
Cohesion: 0.06
Nodes (56): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+48 more)
...
Cohesion: 0.11
Nodes (27): _component_indices(), _dedupe_door_components(), _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Prefer the strongest door when two candidates use the same primitives. (+19 more)
...
Cohesion: 0.11
Nodes (19): Tunable constants for page segmentation.  Values are measured, not guessed — see, build_ink_map(), is_page_spanning(), _is_unfilled_rect(), nested_frame_indices(), path_length(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng (+11 more)
...
Cohesion: 0.08
Nodes (25): One fixture per paper-space family (spec §Testing). Each fails if its     named, Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestPaperInvariance, hline(), path(), A clean 2-line capped rectangle IS a window on 5-1133 (see Window B:         two, 5-1133 FP window_0006: 3 short parallel lines whose opening (15px) is         fa (+17 more)
...
Cohesion: 0.04
Nodes (104): _bbox_expanded(), _bboxes_overlap(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), _project_onto_axis(), _projected_interval(), BBox (+96 more)
...
Cohesion: 0.05
Nodes (45): load_truth(), bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page.      `fallback` is a sc (+37 more)
...
Cohesion: 0.06
Nodes (22): _stored_info(), canonical_denominators(), cluster_denominators(), denominator_from_c(), format_scale(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta, Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present. (+14 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.07
Nodes (43): _accept_jamb_rings(), _building_masses(), _contains_text(), detect_rooms(), _drop_window_exterior_sides(), _edge_face_cover(), _edge_profile(), _EdgeProfile (+35 more)
...
Cohesion: 0.18
Nodes (7): Measured scale expectations across the regression corpus.  Every number was meas, s13 is the one corpus sheet whose viewport and printed scale disagree.      It m, read(), TestKnownConflict, TestSheetsWithNoRecoverableScale, TestTextScales, TestViewportScales
...
Cohesion: 0.09
Nodes (21): _collect_fill_rings(), _collect_wall_faces(), _fill_key(), _FillRing, _rate_fill_classes(), Classify each fill color as wall material (True) or furniture (False).      Vect, Return (stroked wall faces, filled-band centerlines)., A closed same-fill polygon reconstructed from exploded `l` items. (+13 more)
...
Cohesion: 0.22
Nodes (9): main(), Step-11 checkpoint pictures: s01 at its true factor (0.542), the share gate alon, draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1., The two step-9 pictures (guarded so crop_step10.py can import the drawing helper, side_by_side() (+1 more)
...
Cohesion: 0.17
Nodes (4): Rect room with a 45px doorway gap in the top wall (240..285)., Closed stroked (fill-less) polyline exploded into chained `l` items., stroked_ring_path(), wall_band_v()

### Community 153 - "viewport_bbox_to_px"
Cohesion: 0.07
Nodes (41): _interval_overlap(), _area(), _dedupe_by_perp(), _dedupe_openings(), _facing_cap_pairs(), _find_openings(), _glaze_index(), BBox (+33 more)
...
Cohesion: 0.20
Nodes (10): True when ``win`` stands beyond ``door``'s hinge-side jamb in the door's     own, _window_in_door_wall_run(), Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Bbox edges whose plug tails may SEEK their jamb beyond the fixed reach.      Onl, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, _restrict_swing_plugs(), _seek_edges(), _swing_hinge_edges() (+2 more)

### Community 165 - "ScaleInfo"
Cohesion: 0.09
Nodes (35): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, detection_scale(), _effective_denominator(), _gate_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete (+27 more)
...
Cohesion: 0.13
Nodes (8): A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co, A DRAWN DASH LINE — one line its line type exploded into a periodic     row of s, The two classes the corpus sweep exposed on the first cut of the     rule: a wal, 135-degree strokes from face x0 to face x1, clipped to the band., TestDashRowDiscriminators, TestDashRowExclusion, TestFarSidePairs, vline()
...
Cohesion: 0.05
Nodes (55): Entity, attach_takeoff(), _page_summary_dict(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu (+47 more)
...
Cohesion: 0.11
Nodes (19): PageTruth, SheetTruth, evaluate_page(), _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists., score_sheet() (+11 more)
...
Cohesion: 0.15
Nodes (8): _covers(), Three parallel lines with no perpendicular end-caps (e.g. a run of         dimen, Ground truth captured interactively on 5-1133-WD03.pdf (run     2026-06-19_12-02, Windows are drawn at any angle, not just axis-aligned. The cap-anchored     mode, 5-1133-WD03.pdf missed window at path idx 6475: three glazing panes         at 1, TestWindow51133Topology, TestWindowArbitraryAngle, TestWindowTopology
...
Cohesion: 0.06
Nodes (29): _layer_strong_prior(), Return a high confidence boost when a layer name conclusively names the type., _band_interior_clutter(), _cap_orientation_frames(), _cap_record(), _clutter_grid(), detect_windows(), _frame_axes() (+21 more)
...
Cohesion: 0.17
Nodes (7): Extraction puts geometry in the same frame as the declared page size.  page.get_, Every primitive, span AND image must land in the declared frame., A saved 200x400pt PDF with two lines, a word and an image, rotated.      Saved a, Builds all four rotations once; each test reopens what it needs., RotatedPdfTestCase, TestExtractPageFrame, write_rotated_pdf()
...
Cohesion: 0.07
Nodes (48): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document() (+40 more)
...
Cohesion: 0.47
Nodes (5): _clean(), main(), Record, per sheet, the populations each scaled gate discriminates, with world-mm, Annotate each entity with confirmed / fp / unreviewed / deferred., verdicts()
...
Cohesion: 0.03
Nodes (66): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _line_angle_deg(), _line_length(), Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f, _stroke_is_visible() (+58 more)
...
## Knowledge Gaps
- **709 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+704 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Window Detection & Tests`, `denominator_from_c`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `client.py`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Room Polygonization Internals`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `TestThickMaterialPairs`, `_fit_circle_3pt`, `_arc`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `TestSwingHingePlugRestriction`, `_double_arc`, `test_curve_arc_garden_doors.py`, `ScaleInfo`, `renderer.py`, `PruneArcSpursTests`, `TestWindowTopology`, `_collect_wall_faces`, `TestWindowArbitraryAngle`, `path`, `_vector_text_indices`, `test_window_detection.py`, `TestCheckDoorLeaves`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `fill_ring`, `_covers`, `TestDashRowDiscriminators`, `squat_cap_window`, `wall_band_h`, `TestMinWidthReference`, `TestMarkerRings`, `bezier_arc`, `resolve_room_labels`, `detect_doors`, `DoorV2OpeningCheckTests`, `vline`, `test_sliding_doors.py`, `_find_openings`, `app.py`, `_bridge_white_runs`, `TestBandPocket`, `_collect_wall_faces`, `_prune_arc_spurs`, `label_rooms`, `TestNetworkQueries`, `_is_arc_like`, `WallSegment`, `_segments_min_distance`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `render_page_svg` to `Pipeline Orchestration & Extraction`, `plumber.py`, `ScaleInfo`, `Debug Trace Collector`, `detect_doors`, `client.py`, `Wall Network Construction & Tests`, `batch_extract.py`, `Double-Arc Split Tests`, `TestXYCut`, `TestAnnotationPenBarriers`, `TestCliEquivalence`, `hline`, `resolve_heights`, `TakeoffRequest`?**
  _High betweenness centrality (0.100) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Assembly & Heuristics Core` to `Pipeline Orchestration & Extraction`, `denominator_from_c`, `EntranceDoorTests`, `TestWindowArbitraryAngle`, `Debug Trace Collector`, `_leaf_ink_indices`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `TestSwingHingePlugRestriction`, `plumber.py`, `ScaleInfo`, `TestWindowTopology`, `_collect_wall_faces`, `TestXYCut`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `EntranceDoorTests`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `fill_ring`, `TestWindowGates`, `_covers`, `squat_cap_window`, `resolve_room_labels`, `TestMinWidthReference`, `TestMarkerRings`, `vline`, `_find_openings`, `EntranceDoorTests`, `TestBandPocket`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`, `render_page_svg`?**
