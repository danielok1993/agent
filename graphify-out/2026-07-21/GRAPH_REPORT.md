...
## Corpus Check
- 57 files · ~85,278 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1149 nodes · 2909 edges · 115 communities (56 shown, 59 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 132 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0fcccd76`
...
- [[_COMMUNITY__collect_linework_door_leaves|_collect_linework_door_leaves]]
- [[_COMMUNITY_DoorEvidencePropagationTests|DoorEvidencePropagationTests]]
- [[_COMMUNITY_.wall_stroke_reference|.wall_stroke_reference]]

## God Nodes (most connected - your core abstractions)
1. `PathPrimitive` - 144 edges
2. `Candidate` - 98 edges
3. `detect_wall_network()` - 58 edges
4. `TextSpan` - 53 edges
5. `detect_windows()` - 50 edges
6. `detect_doors()` - 45 edges
7. `DebugTraceCollector` - 43 edges
8. `rooms_for()` - 41 edges
9. `_line_angle_deg()` - 30 edges
10. `_angle_diff_mod180()` - 30 edges
...
Cohesion: 0.10
Nodes (38): _absorb_hinged_white_rings(), _detect_folding_doors(), _fold_edges(), _fold_groups(), _pair_parked_stacks(), Pair two 2-leaf V-stacks parked at opposite jambs of one opening.      The physi, Detect arc-less folding/bifold doors. Returns (candidates, next index).      Run, Mark stroked-qu panels white when their coincident fill-ring edges exist.      F (+30 more)
...
Cohesion: 0.10
Nodes (24): _fit_oriented_rect(), Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u, detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf() (+16 more)
...
Cohesion: 0.19
Nodes (10): Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive., Record whether a line segment passed the polyline-arc length filter., _layer_hint(), _layer_strong_prior(), _layer_tokens(), Return True if any keyword is an exact token in the layer name.      Token-split (+2 more)
...
Cohesion: 0.21
Nodes (17): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _is_arc_like(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+9 more)
...
Cohesion: 0.15
Nodes (24): cmd_extract(), cmd_inspect(), main(), parse_page_spec(), Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]., generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON. (+16 more)

### Community 22 - "Candidate"
Cohesion: 0.16
Nodes (22): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+14 more)
...
Cohesion: 0.13
Nodes (14): _band_interior_clutter(), detect_windows(), _frame_axes(), _merge_mullion_chains(), Unit run-axis u (perpendicular to the caps) and perp-axis v (along caps).      C, Join collinear glazing segments across mullion blocks into logical panes.      A, Clutter strictly between the glazing panes — empty for a real window.      The s, Detect windows as capped openings bridged by a parallel glazing band.      For e (+6 more)

### Community 35 - "plumber.py"
Cohesion: 0.11
Nodes (31): detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), _building_masses(), detect_rooms(), _door_plugs(), _folding_chain_gap_plug(), _free_space_components() (+23 more)
...
Cohesion: 0.14
Nodes (14): _collect_fill_rings(), _collect_wall_faces(), _collect_weak_faces(), _fill_key(), _FillRing, _rate_fill_classes(), A closed same-fill polygon reconstructed from exploded `l` items., Annotation arrowhead: a tiny filled triangle or concave dart.          Walls are (+6 more)
...
Cohesion: 0.27
Nodes (6): detect_wall_network(), Build the internal wall-centerline network for a page., Horizontal wall drawn as two stroked faces., TestCenterlines, TestNetworkAssembly, wall_band_h()
...
Cohesion: 0.28
Nodes (14): classify_page(), _color_tuple(), extract_images(), extract_page(), extract_paths(), extract_text(), get_ocg_names(), normalize_bbox() (+6 more)

### Community 105 - "PageData"
Cohesion: 0.24
Nodes (13): Client, extract_document(), build_user_message(), call_gemini(), _candidate_to_dict(), encode_image_inline(), init_client(), parse_gemini_response() (+5 more)
...
Cohesion: 0.08
Nodes (46): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _band_has_wall_material() (+38 more)

### Community 112 - "_collect_linework_door_leaves"
Cohesion: 0.33
Nodes (9): _collect_linework_door_leaves(), _find_thin_rectangle_cycle(), BBox, Find the best thin-rectangle 4-cycle inside a (possibly messy) component.      F, Existing clean-closed-loop linework leaf validation, extracted as a helper., _snap_key(), _try_linework_leaf_clean_loop(), _layer_hint_from_layer() (+1 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `WallNetwork` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `windows.py`, `Arc Spur-Pruning Tests`, `Candidate`, `hline`, `detect_windows`, `plumber.py`, `_prune_arc_spurs`, `rect_room`, `_collect_wall_faces`, `_FillRing`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `TestNetworkQueries`, `extractor.py`, `_resolve_door_window_conflicts`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`, `app.py`, `_collect_linework_door_leaves`, `DoorEvidencePropagationTests`?**
  _High betweenness centrality (0.319) - this node is a cross-community bridge._
- **Why does `Candidate` connect `plumber.py` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `Candidate`, `hline`, `detect_windows`, `_collect_wall_faces`, `vline`, `TestWindowInteriorClutter`, `PageData`, `_resolve_door_window_conflicts`, `TestWindowArbitraryAngle`, `renderer.py`, `TestFloorPlansRegression`, `app.py`, `DoorEvidencePropagationTests`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `detect_windows()` connect `detect_windows` to `Window Detection & Tests`, `plumber.py`, `Door Detection & Tests`, `vline`, `TestWindowInteriorClutter`, `Debug Trace Collector`, `TestWindowArbitraryAngle`, `Window Geometry Internals`, `app.py`, `Arc Cycle-Cap Pruning Tests`, `WallNetwork`, `hline`?**
