...
## Corpus Check
- 57 files · ~83,958 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1147 nodes · 2902 edges · 107 communities (50 shown, 57 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 132 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c432b043`
...
Cohesion: 0.06
Nodes (39): _check_opening_clear(), _merge_double_door_assemblies(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, Merge pairs of adjacent single-door assemblies into double-swing candidates., detect_doors(), _curve(), CurveArcGardenDoorTests, _line() (+31 more)
...
Cohesion: 0.07
Nodes (40): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_iou(), _cross_validate(), _projected_overlap_1d(), BBox (+32 more)
...
Cohesion: 0.12
Nodes (34): _absorb_hinged_white_rings(), _detect_folding_doors(), _fold_edges(), _fold_groups(), _pair_parked_stacks(), Pair two 2-leaf V-stacks parked at opposite jambs of one opening.      The physi, Detect arc-less folding/bifold doors. Returns (candidates, next index).      Run, Mark stroked-qu panels white when their coincident fill-ring edges exist.      F (+26 more)
...
Cohesion: 0.10
Nodes (24): _interval_overlap(), _layer_hint(), _layer_strong_prior(), _layer_tokens(), Return True if any keyword is an exact token in the layer name.      Token-split, Return a high confidence boost when a layer name conclusively names the type., _area(), _band_interior_clutter() (+16 more)

### Community 14 - "Room Polygonization Internals"
Cohesion: 0.10
Nodes (22): detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf(), parked_stack(), ParkedStackPairTests (+14 more)
...
Cohesion: 0.15
Nodes (20): _angle_diff_mod180(), _line_angle_deg(), Smaller angular distance between two directions, both already mod 180°., _band_has_wall_material(), _claims_interior_pair(), _collapse_redundant_centerlines(), _line_intersection(), _merge_collinear_segs() (+12 more)

### Community 17 - "WallNetwork"
Cohesion: 0.18
Nodes (9): Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive., Record whether a line segment passed the polyline-arc length filter., _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower (+1 more)
...
Cohesion: 0.16
Nodes (28): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+20 more)
...
Cohesion: 0.06
Nodes (48): Client, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), _building_masses(), detect_rooms() (+40 more)
...
Cohesion: 0.22
Nodes (8): _collect_material_marks(), _collect_weak_faces(), _is_dashed(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0"., Hairline solid lines long enough to be wall pieces.      Never faces on their ow, (midpoint, angle) of every short solid stroke, gathered once per page.      Thes, _wall_layer_hint(), TestDashDetection
...
Cohesion: 0.14
Nodes (22): _bbox_center(), _bbox_expanded(), _bboxes_overlap(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), BBox, Minimum distance from point p to line segment ab. (+14 more)

### Community 114 - "_projected_interval"
Cohesion: 0.16
Nodes (14): _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _demote_lattice_faces(), _max_rung_stack(), True when two rungs' extents neither overlap nor nearly touch., Deepest simultaneous rung coverage along the run's axis.      Sweeps the rungs' (+6 more)
