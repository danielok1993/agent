...
## Corpus Check
- 57 files · ~91,778 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1197 nodes · 3063 edges · 116 communities (59 shown, 57 thin omitted)
- Extraction: 95% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 136 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ccf8c6df`
...
1. `PathPrimitive` - 153 edges
2. `Candidate` - 101 edges
...
9. `_line_angle_deg()` - 34 edges
10. `_angle_diff_mod180()` - 34 edges
...
Cohesion: 0.14
Nodes (17): _interval_overlap(), _area(), _band_interior_clutter(), _cap_orientation_frames(), _dedupe_by_perp(), _dedupe_openings(), _find_openings(), BBox (+9 more)
...
Cohesion: 0.07
Nodes (25): _check_opening_clear(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, detect_doors(), _curve(), CurveArcGardenDoorTests, _line(), _quarter_arc_bezier(), Garden-door detection for native single-Bezier (`curve_arc`) swings.  The polyli (+17 more)
...
Cohesion: 0.11
Nodes (37): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+29 more)
...
Cohesion: 0.08
Nodes (28): _prune_arc_cycle_caps(), Remove a small closed-cycle cap attached at a single articulation point.      So, _arc(), _chain(), _double_arc(), PruneArcCycleCapsTests, Build one (PathPrimitive, p1, p2, length, angle) tuple shaped like     the segs, Tests for _prune_arc_cycle_caps.      A 'closed-cycle cap' is a closed loop of s (+20 more)

### Community 13 - "Window Geometry Internals"
Cohesion: 0.25
Nodes (14): _collect_slide_panels(), _fit_oriented_rect(), _line_segs(), _panel_shape_ok(), Closed 4-8 segment loops among the given `l` segments, as oriented rects., Closed loops of white-filled `l` items — the Vectorworks joinery     signature:, Closed loops of fill-less stroked `l` items — the flattened-PDF drawing     styl, Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u (+6 more)
...
Cohesion: 0.11
Nodes (15): Background-colored rings that could be hollow walls or built-in runs.      White, One wall centerline segment (pixel space, y-down)., WallSegment, _white_wall_candidates(), TextSpan, DoorAssemblyTests, DoorEvidencePropagationTests, _far_wall_network() (+7 more)

### Community 16 - "Arc Cycle-Cap Pruning Tests"
Cohesion: 0.15
Nodes (25): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+17 more)

### Community 17 - "arcs.py"
Cohesion: 0.11
Nodes (26): _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _fit_circle_3pt(), _is_arc_like(), _prune_arc_spurs(), Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Detect door-swing arcs approximated by connected short line segments.      Some (+18 more)
...
Cohesion: 0.09
Nodes (27): _building_masses(), detect_rooms(), _door_plugs(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges(), Room detection: rooms are the connected free-space components between walls.  Ea, Fraction of a bbox area covered by the text spans lying over it. (+19 more)
...
Cohesion: 0.38
Nodes (7): door_open_leaf_path_indices(), Path indices of single-swing doors' OPEN leaf linework.      A swing door's leaf, _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), detect_schedules()
...
### Community 38 - "TestSwingHingePlugRestriction"
Cohesion: 0.29
...
Cohesion: 0.27
Nodes (6): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, Horizontal wall drawn as two stroked faces., TestCenterlines, TestNetworkAssembly, wall_band_h()
...
Cohesion: 0.10
Nodes (38): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _band_has_wall_material() (+30 more)
...
Cohesion: 0.40
Nodes (4): _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Window Geometry Internals`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `windows.py`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `hline`, `detect_windows`, `plumber.py`, `_prune_arc_spurs`, `TestSwingHingePlugRestriction`, `rect_room`, `_collect_wall_faces`, `_FillRing`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `TestNetworkQueries`, `_resolve_door_window_conflicts`, `TestWindowArbitraryAngle`, `PathPrimitive`, `app.py`, `TestFloorPlansRegression`, `constants.py`?**
  _High betweenness centrality (0.336) - this node is a cross-community bridge._
- **Why does `Candidate` connect `_resolve_door_window_conflicts` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Window Geometry Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `hline`, `detect_windows`, `plumber.py`, `TestSwingHingePlugRestriction`, `_collect_wall_faces`, `vline`, `TestWindowInteriorClutter`, `plumber.py`, `PageData`, `TestWindowArbitraryAngle`, `batch_extract.py`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `detect_windows()` connect `detect_windows` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `plumber.py`, `vline`, `TestWindowInteriorClutter`, `Debug Trace Collector`, `_resolve_door_window_conflicts`, `TestWindowArbitraryAngle`, `app.py`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `Arc Cap-Trim Tests`, `hline`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 56 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 56 INFERRED edges - model-reasoned connections that need verification._
- **Are the 32 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `TestDoorPenalties`) actually correct?**
  _`Candidate` has 32 INFERRED edges - model-reasoned connections that need verification._