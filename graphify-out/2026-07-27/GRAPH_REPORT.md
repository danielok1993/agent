...
## Corpus Check
- 57 files · ~96,743 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1234 nodes · 3177 edges · 111 communities (54 shown, 57 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 146 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f749961b`
...
3. `detect_wall_network()` - 66 edges
4. `TextSpan` - 57 edges
...
Cohesion: 0.10
Nodes (28): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _bbox_union(), _interval_overlap(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin. (+20 more)
...
Cohesion: 0.09
Nodes (30): _cross_validate(), BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried., Connected wall-centerline network (internal-only, never serialized). (+22 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.06
Nodes (41): _prune_arc_cycle_caps(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Trim non-arc cap segments off a 2-leaf simple chain.      Some CAD draftsmen dra, _split_double_arc(), _trim_chain_extension_caps(), _arc(), _chain() (+33 more)
...
Cohesion: 0.21
Nodes (11): _find_threshold_line(), Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, _bbox_area(), _bbox_center(), _bbox_expanded(), BBox, _find_nearby_label(), BBox (+3 more)
...
Cohesion: 0.05
Nodes (54): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), Prefer the strongest door when two candidates use the same primitives., Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics() (+46 more)

### Community 22 - "geometry.py"
Cohesion: 0.17
Nodes (23): _collect_door_swings(), _detect_curve_arc_double_partners(), _fit_circle_3pt(), _is_arc_like(), Fit a circle through 3 points. Returns (cx, cy, radius) or None if     the point, Pair single-Bezier `curve_arc` swings that form a garden-door split.      The po, _door_fallback_candidate(), _nearest_pair_distance() (+15 more)
...
Cohesion: 0.15
Nodes (11): _detect_polyline_arc_bboxes(), _prune_arc_spurs(), Detect door-swing arcs approximated by connected short line segments.      Some, Remove short leaf-spurs (door stops, cap lines) from an arc component.      A cl, PruneArcSpursTests, A closed 4-segment loop has every vertex at degree 2 — no leaf         exists to, 11-segment arc whose far endpoint is a degree-3 junction because         two 1-s, linework_1318 shape: 11-segment arc whose far endpoint becomes a         degree- (+3 more)
...
Cohesion: 0.08
Nodes (47): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _band_has_wall_material(), _claims_interior_pair(), _collapse_redundant_centerlines(), _collect_material_marks(), _collect_weak_faces() (+39 more)

### Community 115 - "constants.py"
Cohesion: 0.13
Nodes (26): _axial_offsets(), _collect_slide_panels(), _detect_sliding_doors(), _fit_oriented_rect(), _line_segs(), _merge_spans(), _pair_leaf_panels(), _panel_shape_ok() (+18 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `geometry.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Window Geometry Internals`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `arcs.py`, `windows.py`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `hline`, `detect_windows`, `plumber.py`, `_projected_interval`, `_collect_wall_faces`, `_FillRing`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `extractor.py`, `TestNetworkQueries`, `TestWindowArbitraryAngle`, `app.py`, `TestFloorPlansRegression`, `constants.py`?**
  _High betweenness centrality (0.303) - this node is a cross-community bridge._
- **Why does `Candidate` connect `_fit_circle_3pt` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Window Geometry Internals`, `geometry.py`, `hline`, `detect_windows`, `renderer.py`, `_collect_wall_faces`, `vline`, `TestWindowInteriorClutter`, `plumber.py`, `PageData`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`, `constants.py`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `detect_wall_network()` connect `Wall Network Construction & Tests` to `Door Detection & Tests`, `_projected_interval`, `Wall Cross-Validation`, `wall_band_h`, `TestMarkerRings`, `Arc Detection Primitives`, `Room Detection Tests`, `TestNetworkQueries`, `_FillRing`, `app.py`, `windows.py`, `constants.py`, `_fit_circle_3pt`, `geometry.py`?**
