...
## Corpus Check
- 220 files · ~351,529 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3779 nodes · 9637 edges · 219 communities (148 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 702 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b3132d2f`
...
## Communities (219 total, 71 thin omitted)
...
### Community 24 - "hline"
Cohesion: 0.13
Nodes (22): _bbox_area(), _bbox_center(), _bbox_expanded(), _bbox_union(), BBox, Minimum distance between two line segments., _segments_min_distance(), detect_labels() (+14 more)
...
Cohesion: 0.15
Nodes (13): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, hline(), paving_field(), Running-bond paving: continuous course lines, staggered joint lines.      Mirror, Four wall bands forming a closed rectangular room (outer faces at the     given, Striped fields (paving bonds, tile fields, treads) are not walls., Stroke-color pen identity: pairing, faint-ink demotion, dimension     chains, an (+5 more)
...
Cohesion: 0.12
Nodes (22): Entity, attach_takeoff(), _page_summary_dict(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, PageFrame, The space every coordinate in this document lives in.      extractor.page_transf (+14 more)
...
### Community 107 - "vline"
Cohesion: 0.18
Nodes (5): path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, TestFaceCollection, TestWeakFacePairs, weak_hatched_band_h()
...
Cohesion: 0.03
Nodes (121): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _interval_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis() (+113 more)
...
Cohesion: 0.08
Nodes (42): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+34 more)
...
Cohesion: 0.26
Nodes (10): cache_key(), page_content_hash(), Stable digest of a page's vector geometry and text. Changes if the PDF     is ed, Cache identity: the page's content AND the segmentation of it.      Region bboxe, page(), path(), Region bboxes ARE the filtering contract, and entries are permanent, so a     ch, regions() (+2 more)
...
Cohesion: 0.07
Nodes (14): TestCase, Path, Skip helper for tests that need a real corpus sheet.  Corpus knowledge lives in, Return the sheet's path, or skip the test with an actionable message., require_sheet(), LoaderTests, The corpus loader resolves slugs against the committed manifest.  Every test bui, Golden segmentation results on the corpus reference sheets (s01, s02, s11).  Mea (+6 more)
...
Cohesion: 0.07
Nodes (29): Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., compute_takeoff(), _largest_polygon(), OpeningTakeoff, Polygon, compute_takeoff — the pure core: rooms + scale + heights → metres.  No I/O, no p, A Polygon from whatever shapely returned; MultiPolygon → its largest part., One physical door or window, once. A shared opening carries both room     ids ra (+21 more)
...
Cohesion: 0.10
Nodes (25): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, detection_scale(), _effective_denominator(), _gate_denominator(), Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., The denominator allowed to drive gate scaling, or None to abstain.      Only a D, PageScales (+17 more)
...
Cohesion: 0.07
Nodes (75): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+67 more)
...
Cohesion: 0.29
Nodes (3): Rect room with a 45px doorway gap in the top wall (240..285)., TestClosedRooms, wall_band_v()
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Room Detection Tests` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `DoorAssemblyTests`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `hline`, `TestBlindWindowPocket`, `detect_windows`, `_projected_interval`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `denominator_from_c`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestFarSidePairs`, `TestWindowSpanOvershootRetune`, `TestRenderPageSvg`, `TestMinWidthNegativeControl`, `TestThickMaterialPairs`, `TestComponentFiltering`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.175) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Room Detection Tests` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Arc Detection Primitives`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `hline`, `plumber.py`, `_projected_interval`, `batch_extract.py`, `renderer.py`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestWindowSpanOvershootRetune`, `TestRenderPageSvg`, `TestMinWidthNegativeControl`, `TestComponentFiltering`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_find_openings`, `EntranceDoorTests`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `_is_light_pen` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `wall_band_v`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `_projected_interval`, `renderer.py`, `TestSwingHingePlugRestriction`, `scales_in_text`, `renderer.py`, `_dedupe_openings`, `TestBlindWindowPocket`, `swing_door`, `TestWindowTightPairInterior`, `_dedupe_openings`, `TestWindowExteriorSide`, `TestRenderPageSvg`, `TestMinWidthNegativeControl`, `TestComponentFiltering`, `detect_doors`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
