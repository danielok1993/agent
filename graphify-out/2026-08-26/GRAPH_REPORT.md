...
## Corpus Check
- 220 files · ~357,863 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3819 nodes · 9780 edges · 229 communities (158 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 712 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7f975abf`
...
1. `PathPrimitive` - 263 edges
2. `Candidate` - 165 edges
3. `PageData` - 164 edges
4. `TextSpan` - 143 edges
5. `Region` - 111 edges
6. `detect_wall_network()` - 98 edges
...
## Communities (229 total, 71 thin omitted)
...
Cohesion: 0.08
Nodes (22): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox short-end edges of a sliding door: across the wall, never wall plane., Thin barrier bands along the wall planes through a detected door.      The door, _sliding_end_edges(), TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet (+14 more)
...
Cohesion: 0.05
Nodes (39): _door_fallback_candidate(), _merge_double_door_assemblies(), BBox, Parse an evidence bbox value defensively; return None on any invalid shape., Merge pairs of adjacent single-door assemblies into double-swing candidates., _safe_bbox(), CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the (+31 more)

### Community 9 - "Room Detection Tests"
Cohesion: 0.06
Nodes (32): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+24 more)
...
Cohesion: 0.06
Nodes (43): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., compute_takeoff(), _largest_polygon(), OpeningTakeoff (+35 more)
...
Cohesion: 0.07
Nodes (36): _building_masses(), detect_rooms(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_wall_recess(), Polygon, Room detection: rooms are the connected free-space components between walls.  Ea (+28 more)
...
Cohesion: 0.05
Nodes (75): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _band_fill_cover() (+67 more)
...
Cohesion: 0.06
Nodes (50): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), Prefer the strongest door when two candidates use the same primitives., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_area() (+42 more)

### Community 115 - "_collect_wall_faces"
Cohesion: 0.13
Nodes (17): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), _candidate_to_dict() (+9 more)
...
Cohesion: 0.10
Nodes (21): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., How far a text span sits from a region, or None if it is not near it.      Horiz (+13 more)
...
Cohesion: 0.08
Nodes (40): manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sha256_of(), sheet_entry(), sheet_path() (+32 more)
...
Cohesion: 0.11
Nodes (27): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., scale_table() (+19 more)
...
Cohesion: 0.07
Nodes (63): _find_threshold_line(), Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+55 more)
...
Cohesion: 0.16
Nodes (6): Rect room with a 45px doorway gap in the top wall (240..285)., A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., TestClosedRooms, TestJambNib, wall_band_h(), wall_band_v()
...
Cohesion: 0.17
Nodes (8): DegenerateCompanionTests, DoorAssemblyTests, _far_wall_network(), path(), Minimal non-empty wall network located far from the doors under test., A single_line_leaf door with no surrounding wall AND no nearby label         is, A single_line_leaf door with no wall but WITH a nearby door label         (e.g., A zero-length stroke is never a leaf edge (s11's double doors).      CAD exports
...
Cohesion: 0.24
Nodes (8): _dedupe_by_perp(), Collapse near-collinear duplicates (same perp offset) to one record.      A toil, Largest run of panes spaced like glazing, not like stair treads.      Walks the, Distinct parallel glazing lines that connect cap ``c1`` to cap ``c2``.      A gl, _spanning_glazing(), _tight_band(), The pre-optimization full-pool scan, verbatim., Two records at the same perp AND len: pool order picks the survivor.
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Room Detection Tests` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `TestWindowArbitraryAngle`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `review.py`, `TestBlindWindowPocket`, `detect_windows`, `plumber.py`, `_projected_interval`, `_double_arc`, `renderer.py`, `TestSwingHingePlugRestriction`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `DoorAssemblyTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_prune_arc_spurs`, `_is_light_pen`, `ParkedLeafTests`, `PruneUnreadPageOutputTests`, `TestFoldingScaleBehavior`, `TestFarSidePairs`, `wall_band_h`, `TestThickMaterialPairs`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.157) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Arc Detection Primitives` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `TestExtractImagesInstances`, `TestWindowArbitraryAngle`, `Room Detection Tests`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `DoorAssemblyTests`, `TestWindowExteriorSide`, `_is_light_pen`, `PruneUnreadPageOutputTests`, `TestMinWidthNegativeControl`, `wall_band_h`, `DoorV2OpeningCheckTests`, `vline`, `_find_openings`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Door Detection & Tests` to `EntranceDoorTests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_check_opening_clear`, `arcs.py`, `RunDirTests`, `resolver.py`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `detect_windows`, `plumber.py`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `TestWindowTightPairInterior`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `DoorAssemblyTests`, `TestWindowExteriorSide`, `wall_band_h`, `detect_doors`, `vline`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `batch_extract.py`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Are the 127 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 127 INFERRED edges - model-reasoned connections that need verification._
- **Are the 82 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `CrossGates`) actually correct?**
  _`Candidate` has 82 INFERRED edges - model-reasoned connections that need verification._