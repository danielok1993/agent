...
## Corpus Check
- 220 files · ~353,511 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3788 nodes · 9679 edges · 206 communities (142 shown, 64 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 702 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4be35380`
...
Cohesion: 0.08
Nodes (18): _door_plugs(), _open_leaf_edges(), Barrier polygon sealing a window opening.      A horizontal/vertical window's bb, Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox short-end edges of a sliding door: across the wall, never wall plane., Thin barrier bands along the wall planes through a detected door.      The door, _sliding_end_edges(), _window_seal() (+10 more)
...
Cohesion: 0.06
Nodes (36): detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf(), OpenVTests, parked_stack() (+28 more)
...
Cohesion: 0.09
Nodes (17): Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, canonical_denominators(), cluster_denominators(), format_scale(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta, Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present., The nearest standard scale within tolerance, or None.      None is a real answer (+9 more)
...
Cohesion: 0.12
Nodes (25): Candidate, Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, PageFrame, The space every coordinate in this document lives in.      extractor.page_transf (+17 more)
...
Cohesion: 0.15
Nodes (16): _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _layer_strong_prior(), Return a high confidence boost when a layer name conclusively names the type., _band_interior_clutter(), _cap_orientation_frames() (+8 more)
...
Cohesion: 0.04
Nodes (110): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), Minimum distance from point p to line segment ab., Minimum distance between two line segments., Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f (+102 more)
...
Cohesion: 0.07
Nodes (41): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), build_plumber_counts(), build_pymupdf_counts() (+33 more)
...
Cohesion: 0.24
Nodes (7): The whole page as one document., to_document(), _page(), The takeoff.json document (takeoff/document.py)., _room(), TestDocumentShape, TestReferentialIntegrity
...
Cohesion: 0.12
Nodes (14): _attach_text_spans(), Grow paths-only boxes to absorb the text spans beside them.      The tier-2 cut, Split a page into drawing regions. Returns [] for a page with no vector     ink, segment_page(), PageData, block(), cut(), page() (+6 more)
...
Cohesion: 0.06
Nodes (39): _check_opening_clear(), door_open_leaf_path_indices(), _line_nears_bridge_interior(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, True when some point of segment p1-p2 lies within buffer_px of the bridge     li, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built (+31 more)
...
Cohesion: 0.06
Nodes (24): denominator_from_c(), The 1:N denominator for a /Measure /X conversion factor., parse_measure_viewports(), BBox, Convert a raw /VP bbox into 150-DPI pixel space.      Two steps, in this order., Split a PDF array string into its top-level ``<< >>`` dictionaries.      Depth-c, Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, split_pdf_dicts() (+16 more)

### Community 191 - "TestThickMaterialPairs"
Cohesion: 0.13
Nodes (18): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, diagonal_window(), framed_triple_window(), path(), quad() (+10 more)
...
Cohesion: 0.06
Nodes (78): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+70 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `RunDirTests` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestWindowArbitraryAngle`, `Debug Trace Collector`, `DoorAssemblyTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `plumber.py`, `_projected_interval`, `renderer.py`, `TestSwingHingePlugRestriction`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `_FillRing`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestFarSidePairs`, `TestRenderPageSvg`, `TestThickMaterialPairs`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_bridge_white_runs`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.169) - this node is a cross-community bridge._
- **Why does `Candidate` connect `batch_extract.py` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `client.py`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `TestBlindWindowPocket`, `plumber.py`, `ScaleInfo`, `renderer.py`, `_dedupe_openings`, `_FillRing`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_dedupe_openings`, `vline`, `_bridge_white_runs`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `_is_light_pen` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Wall Network Construction & Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestBlindWindowPocket`, `plumber.py`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `TestWindowTightPairInterior`, `_dedupe_openings`, `TestWindowExteriorSide`, `_is_light_pen`, `detect_doors`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
