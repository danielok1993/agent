...
## Corpus Check
- 220 files · ~349,608 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3768 nodes · 9603 edges · 208 communities (143 shown, 65 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 699 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `441f4033`
...
Cohesion: 0.06
Nodes (39): _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges(), Polygon (+31 more)
...
Cohesion: 0.06
Nodes (64): DoorGates, World-space door gates, pre-multiplied by the detection factor.      Fields keep, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+56 more)
...
Cohesion: 0.07
Nodes (39): _draw_legend(), draw_overlay(), _draw_regions(), cache_file(), cache_key(), load_regions(), page_content_hash(), Path (+31 more)
...
Cohesion: 0.20
Nodes (7): Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, rect_room(), rooms_for(), TestAnnotationPenBarriers, TestBlindWindowPocket, TestClosedRooms, TestComponentFiltering
...
Cohesion: 0.08
Nodes (24): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), path(), paving_field(), A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co (+16 more)
...
Cohesion: 0.10
Nodes (28): _is_arc_like(), _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower, PathPrimitive, bbox_aspect(), bezier_arc() (+20 more)
...
Cohesion: 0.12
Nodes (23): build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document(), extract_plumber_page(), _normalize_bbox_plumber(), BBox (+15 more)
...
Cohesion: 0.05
Nodes (85): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance() (+77 more)
...
Cohesion: 0.06
Nodes (62): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _native_curve_chains(), BBox (+54 more)
...
Cohesion: 0.22
Nodes (4): Rect room with a 45px doorway gap in the top wall (240..285)., TestOpeningSeals, wall_band_h(), wall_band_v()

### Community 120 - "TestNetworkQueries"
Cohesion: 0.20
Nodes (5): door_candidate(), Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, TestBboxSealFloor, TestPhantomDoorSeals

### Community 121 - "SplitDoubleArcTests"
Cohesion: 0.11
Nodes (17): MainExceptionIsolationTests, tools/review.py's main(): one sheet's unexpected failure must not kill the walk, DeliberateVerdictsTests, EnterWithNothingTickedTests, entity(), _HeadlessReviewSheetTests, Path, tools/review.py's `_pick` / `review_sheet`, driven through the real InquirerPy p (+9 more)
...
Cohesion: 0.06
Nodes (35): bind_scale(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., resolve_page_scales(), _cache_file(), _from_dicts(), load_stored(), match_stored() (+27 more)
...
Cohesion: 0.24
Nodes (4): _contains(), select_room_scale(), _region(), TestSelectRoomScale
...
Cohesion: 0.10
Nodes (29): Tunable constants for page segmentation.  Values are measured, not guessed — see, InkMap, Binary ink occupancy map over a page, used to find whitespace gutters., bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _clip_cut(), _col_profile() (+21 more)
...
Cohesion: 0.09
Nodes (26): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), BBox, Image, Room entities carry their closed polygon; draw its true shape instead     of the, FreeTypeFont, ImageDraw (+18 more)

### Community 165 - "ScaleInfo"
Cohesion: 0.11
Nodes (25): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, detection_scale(), _effective_denominator(), _gate_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., The denominator allowed to drive gate scaling, or None to abstain.      Only a D (+17 more)
...
Cohesion: 0.19
Nodes (4): Every 1:N denominator stated in one string, in the order written., scales_in_text(), Reading a 1:N scale out of text spans.  Every string below is copied verbatim fr, TestScalesInText
...
Cohesion: 0.17
Nodes (11): _layer_classes(), _layer_hint(), _layer_hint_from_layer(), _layer_strong_prior(), _layer_tokens(), The element classes named by a layer's tokens., Return True if any keyword is an exact token in the layer name.      Token-split, Return a high confidence boost when a layer name conclusively names the type. (+3 more)
...
Cohesion: 0.15
Nodes (11): Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, _restrict_swing_plugs(), _swing_hinge_edges(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A filled wall band exported as two triangles (CAD fill triangulation).      Each, Single swing doors: plugs live on the hinge edges, one wall plane.      Geometry, TestEmptyNetwork (+3 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `wall_band_h` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `_double_arc`, `_projected_interval`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `_dedupe_openings`, `TestWindowExteriorSide`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `Candidate` connect `hline` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Double-Arc Split Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `count_paths_in`, `TestAnnotationPenBarriers`, `plumber.py`, `batch_extract.py`, `_dedupe_openings`, `_FillRing`, `swing_door`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `_is_light_pen`, `_dedupe_openings`, `TestWindowExteriorSide`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `hline` to `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `test_extraction_transform.py`, `arcs.py`, `count_paths_in`, `geometry.py`, `TestAnnotationPenBarriers`, `TestExtractImagesInstances`, `detect_windows`, `_projected_interval`, `renderer.py`, `scales_in_text`, `test_curve_arc_garden_doors.py`, `_dedupe_openings`, `swing_door`, `TestWindowTightPairInterior`, `denominator_from_c`, `_dedupe_openings`, `TestWindowExteriorSide`, `wall_band_h`, `detect_doors`, `vline`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `framed_triple_window`, `_segments_min_distance`?**
