...
## Corpus Check
- 220 files · ~352,810 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3786 nodes · 9667 edges · 236 communities (165 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 702 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9119397a`
...
- [[_COMMUNITY__is_light_pen|_is_light_pen]]
- [[_COMMUNITY_test_ground_truth_hygiene.py|test_ground_truth_hygiene.py]]
- [[_COMMUNITY_test_takeoff_quantities.py|test_takeoff_quantities.py]]
- [[_COMMUNITY_load_truth|load_truth]]
- [[_COMMUNITY_PruneUnreadPageOutputTests|PruneUnreadPageOutputTests]]
- [[_COMMUNITY_test_add_sheet.py|test_add_sheet.py]]
- [[_COMMUNITY_render_page_svg|render_page_svg]]
- [[_COMMUNITY_HygieneRuleTests|HygieneRuleTests]]
- [[_COMMUNITY_add_sheet.py|add_sheet.py]]
...
- [[_COMMUNITY_client.py|client.py]]
- [[_COMMUNITY__dedupe_openings|_dedupe_openings]]
- [[_COMMUNITY_OpenLeafExclusionTests|OpenLeafExclusionTests]]
- [[_COMMUNITY_TestBatchNeverPrompts|TestBatchNeverPrompts]]
- [[_COMMUNITY_TestCrossWindowToleranceUnscaled|TestCrossWindowToleranceUnscaled]]
- [[_COMMUNITY_SentinelIdentityTests|SentinelIdentityTests]]

## God Nodes (most connected - your core abstractions)
1. `PathPrimitive` - 258 edges
...
## Communities (236 total, 71 thin omitted)
...
Cohesion: 0.11
Nodes (20): apply_classification(), build_request_parts(), classify_regions(), Page, Ask Gemini what each segmented region is.  One call per page. Each region goes a, Distinct text inside a region, largest font first. Many CAD exports     outline, Apply a classification response to a region list.      Returns new Region object, One API call for the whole page. Returns classified regions + warnings. (+12 more)
...
Cohesion: 0.07
Nodes (19): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, _production_cross_gates_unscaled_usages(), _production_door_gates_unscaled_usages(), quarter_bezier(), Scale-factor behavior of the door gates: identity at 1.0, linear at 0.5.  A "fai, Scan detection/**/*.py for PRODUCTION (non-import, non-comment) uses     of the, Ratchet on detection/'s production uses of DOOR_GATES_UNSCALED.      DOOR_GATES_ (+11 more)
...
Cohesion: 0.14
Nodes (6): Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, rect_room(), TestAnnotationPenBarriers, TestBlindWindowPocket, TestClosedRooms, TestComponentFiltering

### Community 18 - "windows.py"
Cohesion: 0.26
Nodes (3): AdoptTests, adopt(), Path
...
### Community 24 - "hline"
Cohesion: 0.10
Nodes (17): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict(), _contains(), select_room_scale(), effective_denominator(), Nominal beats raw so 1:50 sheets compute exactly (scale/factor.py rule). (+9 more)
...
Cohesion: 0.10
Nodes (18): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), paving_field(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, Running-bond paving: continuous course lines, staggered joint lines.      Mirror (+10 more)
...
Cohesion: 0.15
Nodes (14): _centre_in_any(), filter_page_data(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, A copy of page_data holding only primitives whose bbox centre falls in     one o, Text spans inside the given regions. Used to scope schedule detection to     sch, region_text_spans(), ImageRef (+6 more)
...
Cohesion: 0.09
Nodes (20): qualifying_clip_rects(), Read scissor rects off a fitz.Page and gate them. Returns [] if the     PDF expo, build_ink_map(), is_page_spanning(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng, Golden segmentation results on the corpus reference sheets (s01, s02, s11).  Mea, This sheet carries full-page border rules. With the span filter applied     the (+12 more)
...
Cohesion: 0.12
Nodes (22): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., PageFrame, The space every coordinate in this document lives in.      extractor.page_transf (+14 more)
...
Cohesion: 0.15
Nodes (17): sha256_of(), _check_provenance(), _ordered(), pending(), What a persisted sweep still needs verdicts on.  Reads the run output the sweep, Unreviewed detections, keyed by 1-based page then entity type.      Pages and ty, This sheet cannot be reviewed right now. Report it and move on., No persisted sweep output for this slug. (+9 more)
...
### Community 107 - "vline"
Cohesion: 0.16
Nodes (8): Resolve a scale for every floor-plan region on one page., resolve_page_scales(), A suspend_display factory that logs enter/exit, for the tests below., The regression: a live rich Progress torn down on a page that         resolved i, The renumbering hazard, end to end.          A scale stored against what used to, span(), TestResolvePageScales, viewport()

### Community 108 - "_bridge_white_runs"
Cohesion: 0.18
Nodes (13): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), The scale governing one region, or None.      `viewports` must arrive smallest-b, How far a text span sits from a region, or None if it is not near it.      Horiz, Every text scale near enough to this region to be about it, nearest first. (+5 more)
...
Cohesion: 0.47
Nodes (3): A chimney breast / pier drawn as a closed box on the room side of a     wall ban, TestWallRecess, text_span()

### Community 111 - "app.py"
Cohesion: 0.04
Nodes (107): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _interval_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox() (+99 more)
...
Cohesion: 0.07
Nodes (45): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), build_plumber_counts(), build_pymupdf_counts() (+37 more)
...
Cohesion: 0.13
Nodes (20): _collect_slide_panels(), _fit_oriented_rect(), _line_segs(), _merge_spans(), _panel_shape_ok(), _pocket_leaf_match(), Closed 4-8 segment loops among the given `l` segments, as oriented rects., Closed loops of white-filled `l` items — the Vectorworks joinery     signature: (+12 more)
...
Cohesion: 0.18
Nodes (13): DeliberateVerdictsTests, EnterWithNothingTickedTests, entity(), _HeadlessReviewSheetTests, Path, tools/review.py's `_pick` / `review_sheet`, driven through the real InquirerPy p, Shared fixture: one fake corpus sheet with a persisted sweep run.      Mirrors t, The C1 regression test.      Against the old `inquirer.fuzzy(multiselect=True)` (+5 more)
...
Cohesion: 0.10
Nodes (21): assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, _attach_text_spans(), page_fallback_region(), Grow paths-only boxes to absorb the text spans beside them.      The tier-2 cut, Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page() (+13 more)
...
Cohesion: 0.16
Nodes (15): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+7 more)
...
Cohesion: 0.19
Nodes (14): dump_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), Path, The user's per-sheet verdicts, and how they are read.  One file per sheet under, Create the unlabeled ground-truth file for a newly adopted sheet. (+6 more)
...
Cohesion: 0.11
Nodes (20): compute_takeoff(), _largest_polygon(), Polygon, compute_takeoff — the pure core: rooms + scale + heights → metres.  No I/O, no p, A Polygon from whatever shapely returned; MultiPolygon → its largest part., _room_polygon(), _warn(), is_verified() (+12 more)
...
Cohesion: 0.13
Nodes (13): _check_opening_clear(), _line_nears_bridge_interior(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, True when some point of segment p1-p2 lies within buffer_px of the bridge     li, DoorV2OpeningCheckTests, line(), PolylineArcBinCapTests, 270-degree polyline arc with 16 segments — far wider than a quarter-circle door (+5 more)
...
Cohesion: 0.26
Nodes (7): hline(), path(), Filled arrowhead triangle (a marker ring) pointing down at `tip`., Stairs are furniture to the room stage: a room polygon runs to the     enclosing, stair_arrowhead(), TestStairFurniture, vline()
...
Cohesion: 0.16
Nodes (16): latest_run(), Path, Where a sweep leaves its output.  Sweeps used to extract into a `tempfile.Tempor, Wipe and recreate this slug's output directory., The most recent run directory for this slug, or None.      Timestamp names sort, reset_slug_dir(), slug_dir(), _cache_missed() (+8 more)
...
Cohesion: 0.21
Nodes (10): detection_scale(), PageScales, info(), detection_scale(): PageScales + regions -> one detection factor per page., region(), TestDetectionScale, det_scale is resolved before the skip_detection branch in run_extract     (so th, warning_count comes from page_warnings, not all_warnings.      A scale warning a (+2 more)
...
Cohesion: 0.14
Nodes (11): fill_ring(), marker_ring(), path(), Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, Closed filled rectangle exploded into 4 chained `l` items., Filled triangle/dart exploded into chained `l` items (a leader tip)., Leader/dimension arrowheads share the wall pen on Vectorworks-style     exports;, TestCenterlines (+3 more)
...
Cohesion: 0.27
Nodes (4): DegenerateCompanionTests, DoorAssemblyTests, path(), A zero-length stroke is never a leaf edge (s11's double doors).      CAD exports
...
Cohesion: 0.24
Nodes (6): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, quarter_arc_lines(), Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, rect_lines(), SingleLineLeafTests
...
Cohesion: 0.07
Nodes (46): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups() (+38 more)
...
Cohesion: 0.29
Nodes (4): A window is a wall opening between inside and outside. When the space     on one, TestEmptyNetwork, TestWindowExteriorSide, wall_band_h()

### Community 210 - "TestSpanFilterIsLoadBearing"
Cohesion: 0.33
Nodes (3): denominator_from_c(), The 1:N denominator for a /Measure /X conversion factor., TestDenominatorFromC

### Community 211 - "MainExceptionIsolationTests"
Cohesion: 0.18
Nodes (15): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, Flip a manifest entry's `labeled` flag and write the manifest back.      `labele (+7 more)
...
### Community 216 - "TestRoomGatesConstruction"
Cohesion: 0.13
Nodes (23): _building_masses(), detect_rooms(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_wall_recess(), Polygon, Room detection: rooms are the connected free-space components between walls.  Ea (+15 more)

### Community 217 - "TestComponentFiltering"
Cohesion: 0.19
...
### Community 219 - "test_ground_truth_hygiene.py"
Cohesion: 0.21
...
### Community 220 - "test_takeoff_quantities.py"
Cohesion: 0.24
Nodes (9): attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, room_dict(), OpeningTakeoff (+1 more)

### Community 221 - "load_truth"
Cohesion: 0.29
Nodes (4): dumps_truth(), load_truth(), Serialize a sheet's verdicts exactly as they are stored on disk.      Split out, TestGroundTruthCarriesScales
...
### Community 223 - "test_add_sheet.py"
Cohesion: 0.29
Nodes (5): make_pdf(), NextSlugTests, Path, Adopting a new sheet into the corpus., next_slug()

### Community 224 - "render_page_svg"
Cohesion: 0.36
...
### Community 226 - "add_sheet.py"
Cohesion: 0.33
Nodes (5): address_match(), Shared address-detection patterns for corpus hygiene checks.  Two callers share, The matched address-like substring in `text`, or None., _kebab(), main()

### Community 227 - "FitCircle3PtTests"
Cohesion: 0.29
...
### Community 228 - "TestSlidingScaleBehavior"
Cohesion: 0.38
...
### Community 229 - "TestFoldingScaleBehavior"
Cohesion: 0.38
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `wall_band_h` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `DoorAssemblyTests`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `detect_windows`, `_projected_interval`, `renderer.py`, `TestSwingHingePlugRestriction`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `denominator_from_c`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestFarSidePairs`, `TestWindowSpanOvershootRetune`, `TestRenderPageSvg`, `TestMinWidthNegativeControl`, `TestThickMaterialPairs`, `FitCircle3PtTests`, `TestSlidingScaleBehavior`, `TestMarkerRings`, `TestFoldingScaleBehavior`, `DoorV2OpeningCheckTests`, `detect_doors`, `OpenLeafExclusionTests`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `Candidate` connect `TestMarkerRings` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `Room Detection Tests`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `plumber.py`, `_projected_interval`, `batch_extract.py`, `renderer.py`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestWindowSpanOvershootRetune`, `TestMinWidthNegativeControl`, `TestRoomGatesConstruction`, `test_takeoff_quantities.py`, `DoorV2OpeningCheckTests`, `_dedupe_openings`, `OpenLeafExclusionTests`, `TestCrossWindowToleranceUnscaled`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Double-Door Merge & Gemini Client` to `Door Assembly & Heuristics Core`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `wall_band_v`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `detect_windows`, `_projected_interval`, `renderer.py`, `scales_in_text`, `renderer.py`, `_dedupe_openings`, `TestBlindWindowPocket`, `swing_door`, `TestWindowTightPairInterior`, `_is_light_pen`, `_dedupe_openings`, `TestWindowExteriorSide`, `TestMinWidthNegativeControl`, `TestRoomGatesConstruction`, `_is_light_pen`, `FitCircle3PtTests`, `TestMarkerRings`, `detect_doors`, `OpenLeafExclusionTests`, `vline`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
