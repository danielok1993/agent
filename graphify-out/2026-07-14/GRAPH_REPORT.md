...
## Corpus Check
- 55 files · ~78,369 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1088 nodes · 2707 edges · 106 communities (49 shown, 57 thin omitted)
- Extraction: 95% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 120 edges (avg confidence: 0.53)
...
3. `detect_wall_network()` - 57 edges
...
## Communities (106 total, 57 thin omitted)
...
Cohesion: 0.12
Nodes (18): _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower, _layer_hint(), _layer_strong_prior(), _layer_tokens(), Return True if any keyword is an exact token in the layer name.      Token-split (+10 more)
...
Cohesion: 0.16
Nodes (25): _axial_offsets(), _collect_slide_panels(), _corners_bbox(), _detect_sliding_doors(), _fit_oriented_rect(), _merge_spans(), _pair_leaf_panels(), _panel_shape_ok() (+17 more)
...
Cohesion: 0.07
Nodes (55): _bboxes_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), _project_onto_axis(), _projected_interval() (+47 more)

### Community 17 - "WallNetwork"
Cohesion: 0.27
Nodes (13): _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, _find_anchored_leaf_line(), _find_leaf_companion_lines(), Search for a single line that could be the door leaf for this arc swing.      Ar, Find lines forming the same thin-rect leaf as the anchored leaf line.      Door (+5 more)
...
Cohesion: 0.15
Nodes (30): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), _native_curve_chains() (+22 more)
...
Cohesion: 0.07
Nodes (47): Client, _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_center(), detect_labels(), _find_nearby_label(), BBox (+39 more)
...
Cohesion: 0.14
Nodes (16): _door_fallback_candidate(), _merge_double_door_assemblies(), BBox, Parse an evidence bbox value defensively; return None on any invalid shape., Merge pairs of adjacent single-door assemblies into double-swing candidates., _safe_bbox(), DoubleDoorTests, Tests for _merge_double_door_assemblies: adjacent single-door assembly merging. (+8 more)
...
Cohesion: 0.27
Nodes (10): _bbox_area(), _bbox_iou(), _projected_overlap_1d(), BBox, Projected overlap fraction and perpendicular gap on bbox a's dominant axis., Type-specific NMS: higher confidence wins when two candidates overlap.      For, # NOTE: there is deliberately no "drop windows that look like wall linework", True when a wall FACE line runs unbroken through the bbox span.      A real wind (+2 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Door Assembly & Heuristics Core` to `Pipeline Orchestration & Extraction`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `WallNetwork`, `windows.py`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `Candidate`, `hline`, `detect_windows`, `plumber.py`, `_prune_arc_spurs`, `_collect_wall_faces`, `rect_room`, `_FillRing`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `TestNetworkQueries`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.298) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Candidate` to `Door Assembly & Heuristics Core`, `detect_windows`, `Door Detection & Tests`, `plumber.py`, `Wall Cross-Validation`, `Window Detection & Tests`, `Debug Trace Collector`, `_dedupe_openings`, `_collect_wall_faces`, `Room Detection Tests`, `vline`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`, `WallNetwork`, `_fit_circle_3pt`, `TestWindowInteriorClutter`, `hline`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `detect_wall_network()` connect `Wall Network Construction & Tests` to `Door Assembly & Heuristics Core`, `Door Detection & Tests`, `wall_band_h`, `Wall Cross-Validation`, `_collect_wall_faces`, `TestMarkerRings`, `rect_room`, `Room Detection Tests`, `TestNetworkQueries`, `_FillRing`, `Arc Cycle-Cap Pruning Tests`, `windows.py`, `Candidate`?**
