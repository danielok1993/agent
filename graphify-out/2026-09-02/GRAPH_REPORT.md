...
## Corpus Check
- 252 files · ~420,186 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4482 nodes · 11516 edges · 230 communities (161 shown, 69 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 888 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `eacefe88`
...
Cohesion: 0.07
Nodes (26): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+18 more)

### Community 1 - "Door Assembly & Heuristics Core"
Cohesion: 0.06
Nodes (26): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate (+18 more)
...
Cohesion: 0.07
Nodes (30): dump_truth(), dumps_truth(), load_truth(), Path, Create the unlabeled ground-truth file for a newly adopted sheet., Serialize a sheet's verdicts exactly as they are stored on disk.      Split out, Write a sheet's verdicts back to tests/ground_truth/<slug>.json.      Round-trip, truth_path() (+22 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.05
Nodes (79): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+71 more)
...
Cohesion: 0.05
Nodes (59): _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _fit_circle_3pt(), _is_arc_like(), _native_curve_chains(), _prune_arc_cycle_caps(), _prune_arc_spurs() (+51 more)
...
Cohesion: 0.06
Nodes (37): detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf(), OpenVTests, parked_stack() (+29 more)

### Community 15 - "Arc Cap-Trim Tests"
Cohesion: 0.13
Nodes (14): _bridge_white_runs(), _equivalent_sides(), (short, long) of the rectangle with this polygon's area and perimeter.      The, Band-shaped convex hulls closing the gaps in accepted white-ring runs.      gate, _bridge_white_runs is detect_rooms's ONLY production call site     (detection/ro, TestBridgeWhiteRunsGapScaling, Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, A filled wall band the exporter triangulated into two rings that     each carry (+6 more)
...
Cohesion: 0.06
Nodes (41): _accept_jamb_rings(), _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_door_lining() (+33 more)
...
Cohesion: 0.08
Nodes (51): _arc_corners(), _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _door_fallback_candidate(), _find_threshold_line(), _leaf_ink_indices(), _merge_double_door_assemblies() (+43 more)
...
Cohesion: 0.12
Nodes (19): _inline_number_array(), _inline_point_array(), _item(), _item_payload(), The user's per-sheet verdicts, and how they are read.  One file per sheet under, Stash a flat number array as a one-line literal; return its token.      json.dum, Stash a polygon as a one-line literal; return its token.      A polygon on one l, Serialize one verdict, omitting everything left at its default.      Ground trut (+11 more)

### Community 107 - "vline"
Cohesion: 0.06
Nodes (32): _check_opening_clear(), _line_nears_bridge_interior(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, True when some point of segment p1-p2 lies within buffer_px of the bridge     li, detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DegenerateCompanionTests, DoorAssemblyTests (+24 more)
...
Cohesion: 0.04
Nodes (120): _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), _projected_interval(), Minimum distance between two line segments., Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars. (+112 more)
...
Cohesion: 0.32
Nodes (5): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, OpenLeafExclusionTests, Tests for door_open_leaf_path_indices on merged double-swing pairs.      Geometr, French leaves are drawn closed IN the wall plane — legitimate wall         evide
...
Cohesion: 0.10
Nodes (29): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, The per-region scale table printed after each page., scale_table(), detection_scale(), _fallback_info() (+21 more)
...
Cohesion: 0.08
Nodes (32): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_request_text(), Polygon, Ask Gemini for the name written inside each detected room.  One text-only call p, The one user part: every room's spans as JSON, keyed by ordinal., _room_polygon() (+24 more)
...
Cohesion: 0.06
Nodes (36): attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, room_dict(), Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas. (+28 more)
...
Cohesion: 0.06
Nodes (17): _candidate_to_dict(), collect_warnings(), make_output_dir(), make_page_dir(), run_extract(), write_json(), FakeBlob, FakeBucket (+9 more)
...
Cohesion: 0.12
Nodes (15): Handoff: hatch-cell chords in the wall network (follow-up to `fix/s03-bedroom-corner-notch`), Outcome of the anchor iteration (2026-09-02, `fix/collinear-merge-anchor-line`), Outcome of the collinear-support anchor (2026-09-02, same branch), Prompt for the next agent (the collinear-support anchor), Prompt for the next agent (the merged run's anchor line — R2's function), Prompt that was executed for Gap C (the seam probe distance), Queued after it (Gap D — glyph-outline fill rings), Queued after it (the candidate-key conflation in `_fill_seams`) (+7 more)
...
### Community 419 - "FakeDoc"
Cohesion: 0.11
Nodes (18): Exception, Base class. `code` is a Firebase callable error code string., TakeoffFnError, TakeoffRequest, FakeDoc, The function is a transport wrapper: run_extract must be called with     exactly, A file that downloads but will not PARSE must warn and be skipped, the     same, /tmp is tmpfs charged against the 2 GiB memory budget, so peak usage     must be (+10 more)
...
## Knowledge Gaps
- **590 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+585 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Arc Detection Primitives` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Door Detection & Tests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `client.py`, `client.py`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `Arc Cap-Trim Tests`, `TestAnnotationPenBarriers`, `assistant.tsx`, `hline`, `review.py`, `TestBlindWindowPocket`, `detect_windows`, `plumber.py`, `_projected_interval`, `renderer.py`, `batch_extract.py`, `DoorV2OpeningCheckTests`, `TestNetworkQueries`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `label_rooms`, `TestWindowExteriorSide`, `_is_light_pen`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `TestMinWidthNegativeControl`, `test_batch_extract.py`, `TestFarSidePairs`, `TestAnnotationLayerVeto`, `wall_band_h`, `core-role-card.tsx`, `TestMarkerRings`, `detect_doors`, `vline`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`, `framed_triple_window`?**
  _High betweenness centrality (0.140) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `TestCliEquivalence` to `Pipeline Orchestration & Extraction`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Wall Network Construction & Tests`, `arcs.py`, `hline`, `plumber.py`, `FakeDoc`, `ScaleInfo`, `batch_extract.py`, `test_curve_arc_garden_doors.py`, `test_window_detection.py`, `TestXYCut`, `TestSlugForPath`, `TestSheetSize`, `DoorAssemblyTests`, `TakeoffRequest`, `detect_doors`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `test_curve_arc_garden_doors.py` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `_double_arc`, `plumber.py`, `renderer.py`, `batch_extract.py`, `TestXYCut`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `_is_light_pen`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `vline`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`?**
