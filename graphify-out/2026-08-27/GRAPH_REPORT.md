...
## Corpus Check
- 222 files · ~371,834 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3942 nodes · 10149 edges · 216 communities (145 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 737 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4ac40447`
...
Cohesion: 0.09
Nodes (27): Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_svg(), cache_file(), cache_key(), load_labels(), Path, On-disk cache of room labels, keyed by page content AND the room polygons the la (+19 more)

### Community 2 - "Window Detection & Tests"
Cohesion: 0.11
Nodes (16): _clutter_grid(), detect_windows(), _frame_axes(), _merge_mullion_chains(), Unit run-axis u (perpendicular to the caps) and perp-axis v (along caps).      C, Join collinear glazing segments across mullion blocks into logical panes.      A, Page-wide point buckets for _band_interior_clutter.      The clutter scan asks w, Detect windows as capped openings bridged by a parallel glazing band.      For e (+8 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.06
Nodes (38): Client, generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., render_page_png(), init_client(), Vertex AI client construction.  Per-candidate validation was removed on 2026-07-, Candidate (+30 more)
...
Cohesion: 0.08
Nodes (60): DoorGates, World-space door gates, pre-multiplied by the detection factor.      Fields keep, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+52 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.05
Nodes (70): DebugTraceCollector, Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Register a collected swing. Returns swing_id., Record the swing-anchored single-line leaf search outcome.          `result` is, Register a collected leaf. Returns leaf_id., Record a final candidate with its full confidence breakdown. (+62 more)
...
Cohesion: 0.11
Nodes (16): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, _stored_info(), Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, canonical_denominators(), format_scale(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta, One representative per cluster — how many DISTINCT scales are present. (+8 more)
...
Cohesion: 0.08
Nodes (27): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, A band hatched THROUGH — every diagonal stroke ending on both faces — is a drawn, hline(), path(), paving_field(), Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co (+19 more)
...
Cohesion: 0.08
Nodes (23): Tunable constants for page segmentation.  Values are measured, not guessed — see, build_ink_map(), is_page_spanning(), _is_unfilled_rect(), nested_frame_indices(), path_length(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng (+15 more)
...
Cohesion: 0.06
Nodes (48): attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, room_dict(), Heights (+40 more)
...
Cohesion: 0.19
Nodes (13): _fit_oriented_rect(), Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u, EndToEndTests, leaf(), parked_stack(), One folding leaf running p -> q, drawn in the Vectorworks joinery     signature:, Two leaves fanned open from one shared hinge (a parked bifold V)., OrientedRectFitTests (+5 more)
...
Cohesion: 0.04
Nodes (105): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis() (+97 more)
...
Cohesion: 0.06
Nodes (42): RuntimeError, bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page. (+34 more)
...
Cohesion: 0.07
Nodes (37): _collect_fill_rings(), _collect_material_marks(), _collect_stroked_rect_weak_faces(), _collect_stroked_rings(), _collect_wall_faces(), _collect_weak_faces(), _demote_stair_faces(), _fill_key() (+29 more)
...
Cohesion: 0.10
Nodes (35): InkMap, bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _chains_across(), _clip_cut(), _col_profile(), count_paths_in() (+27 more)
...
Cohesion: 0.16
Nodes (9): _merge_collinear_segs(), _pens_compatible(), Merge segments lying on the same infinite line into runs.      Bridges gaps up t, A wall's two faces are drawn by one pen; colorless (fill-outline)     geometry s, _hface(), A bare horizontal wall-face _Seg for isolated merge-tolerance tests., Isolates _merge_collinear_segs's offset-tolerance scaling directly —     the exa, TestMergeCollinearOffsetScaling (+1 more)
...
Cohesion: 0.12
Nodes (19): Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), Scale-factor behavior of walls/rooms gates: identity at 1.0, shrunk-world at 0.5, Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), rooms_for() (+11 more)
...
Cohesion: 0.18
Nodes (8): _layer_classes(), _layer_hint_from_layer(), _layer_strong_prior(), _layer_tokens(), The element classes named by a layer's tokens., Return a high confidence boost when a layer name conclusively names the type., Layer-name hints: CAD layer conventions pluralise the class name.  Measured on t, TestPluralLayerTokens
...
Cohesion: 0.14
Nodes (18): _glaze_index(), Two-axis lookup structure over a frame's glazing pool.      Every cap pair asks, Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, diagonal_window(), framed_triple_window() (+10 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `TestWindowExteriorSide` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `test_layout_segmenter.py`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Door Merge & Gemini Client`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cap-Trim Tests`, `test_extraction_transform.py`, `arcs.py`, `Arc Cycle-Cap Pruning Tests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `hline`, `test_layout_segmenter.py`, `detect_windows`, `plumber.py`, `_projected_interval`, `_double_arc`, `renderer.py`, `TestSwingHingePlugRestriction`, `TestNetworkQueries`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `TestWindowTightPairInterior`, `test_sliding_doors.py`, `_is_light_pen`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `squat_cap_window`, `TestWindowTightPairInterior`, `vline`, `_bridge_white_runs`, `_find_openings`, `TestWindowSpanOvershootRetune`, `app.py`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.163) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Detection & Tests` to `Pipeline Orchestration & Extraction`, `Window Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `DoorAssemblyTests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `TestAnnotationPenBarriers`, `TestBlindWindowPocket`, `test_layout_segmenter.py`, `TestNetworkQueries`, `plumber.py`, `test_extraction_transform.py`, `batch_extract.py`, `test_window_detection.py`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `test_sliding_doors.py`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `wall_band_h`, `DoorV2OpeningCheckTests`, `squat_cap_window`, `TestWindowTightPairInterior`, `vline`, `_bridge_white_runs`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestWindowSpanOvershootRetune`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `EntranceDoorTests` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `arcs.py`, `test_extraction_transform.py`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestBlindWindowPocket`, `detect_windows`, `_projected_interval`, `renderer.py`, `scales_in_text`, `test_window_detection.py`, `TestWindowTightPairInterior`, `_is_light_pen`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `wall_band_h`, `detect_doors`, `vline`, `_bridge_white_runs`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`?**
