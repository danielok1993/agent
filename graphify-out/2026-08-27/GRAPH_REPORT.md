...
## Corpus Check
- 220 files · ~362,746 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3856 nodes · 9882 edges · 218 communities (147 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 721 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7e05f66c`
...
5. `Region` - 111 edges
6. `detect_wall_network()` - 103 edges
...
## Communities (218 total, 71 thin omitted)
...
Cohesion: 0.26
Nodes (7): _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), run_heuristics emits per-stage wall-clock timings on its module logger.  The 202
...
Cohesion: 0.05
Nodes (36): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), _area(), _dedupe_openings(), BBox, Suppress overlapping detections from duplicate cap pairs (greedy NMS).      Dupl (+28 more)
...
Cohesion: 0.06
Nodes (36): _door_fallback_candidate(), door_open_leaf_path_indices(), _leaf_ink_indices(), _merge_double_door_assemblies(), BBox, Parse an evidence bbox value defensively; return None on any invalid shape., Merge pairs of adjacent single-door assemblies into double-swing candidates., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line (+28 more)
...
Cohesion: 0.07
Nodes (31): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, parse_height(), _prompt_ceiling(), Wall / opening heights — the one input the plan cannot supply.  0/20 corpus shee (+23 more)
...
Cohesion: 0.13
Nodes (8): Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co, Horizontal wall drawn as two stroked faces., TestCenterlines, TestFaceCollection, TestFarSidePairs, TestNetworkAssembly, wall_band_h()
...
### Community 107 - "vline"
Cohesion: 0.22
Nodes (4): quarter_arc_lines(), Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, rect_lines(), SingleLineLeafTests
...
Cohesion: 0.04
Nodes (116): _find_threshold_line(), Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bbox_expanded(), _bbox_union(), _bboxes_overlap() (+108 more)
...
Cohesion: 0.18
Nodes (12): DeliberateVerdictsTests, EnterWithNothingTickedTests, entity(), _HeadlessReviewSheetTests, Path, Shared fixture: one fake corpus sheet with a persisted sweep run.      Mirrors t, The C1 regression test.      Against the old `inquirer.fuzzy(multiselect=True)`, Down-arrow `downs` times, Space to tick, Enter to submit. (+4 more)
...
Cohesion: 0.17
Nodes (10): page_fallback_region(), Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page(), PageData, block(), A solid-ish blob: a horizontal line every 4px so every bin row is inked., TestPathsOnlyRetry (+2 more)
...
Cohesion: 0.14
Nodes (22): Native PDF clip rects, used as extra cut hints for the segmenter.  Clip rects ar, Tunable constants for page segmentation.  Values are measured, not guessed — see, InkMap, Binary ink occupancy map over a page, used to find whitespace gutters., bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _col_profile() (+14 more)
...
Cohesion: 0.10
Nodes (31): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, detection_scale(), DetectionScale (+23 more)
...
Cohesion: 0.10
Nodes (22): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, diagonal_window(), framed_triple_window(), path(), quad() (+14 more)
...
Cohesion: 0.07
Nodes (66): _pair_door_assemblies(), _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg() (+58 more)
...
Cohesion: 0.09
Nodes (19): TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A filled wall band exported as two triangles (CAD fill triangulation).      Each, A chimney breast / pier drawn as a closed box on the room side of a     wall ban, A lone stroked, unfilled `qu` item — a joinery-pen box., s04 BATHROOM 01 (room_0000, door_0002): the structural opening is     112px wide, Closed stroked (fill-less) polyline exploded into chained `l` items., s03 corridor room_0014: the jamb nibs beside door_0007/door_0019 are     closed (+11 more)
...
Cohesion: 0.23
Nodes (5): Rect room with a 45px doorway gap in the top wall (240..285)., A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., TestJambNib, wall_band_h(), wall_band_v()
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `_is_light_pen` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Debug Trace Collector`, `Room Detection Tests`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `TestWindowArbitraryAngle`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `test_extraction_transform.py`, `RunDirTests`, `resolver.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `_double_arc`, `_projected_interval`, `TestNetworkQueries`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `_FillRing`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `TestWindowExteriorSide`, `test_batch_extract.py`, `ParkedLeafTests`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_bridge_white_runs`, `_find_openings`, `EntranceDoorTests`, `app.py`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.156) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Arc Detection Primitives` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `TestExtractImagesInstances`, `TestWindowArbitraryAngle`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `ParkedLeafTests`, `wall_band_h`, `DoorV2OpeningCheckTests`, `vline`, `_bridge_white_runs`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.097) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `test_batch_extract.py` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `Arc Cap-Trim Tests`, `arcs.py`, `RunDirTests`, `geometry.py`, `TestAnnotationPenBarriers`, `detect_windows`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `test_window_detection.py`, `TestWindowTightPairInterior`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `TestWindowExteriorSide`, `ParkedLeafTests`, `wall_band_h`, `detect_doors`, `vline`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
