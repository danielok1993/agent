...
## Corpus Check
- 171 files · ~261,361 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2918 nodes · 7260 edges · 182 communities (118 shown, 64 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 453 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `89dd74b1`
...
1. `PathPrimitive` - 216 edges
2. `PageData` - 138 edges
3. `Candidate` - 121 edges
4. `TextSpan` - 107 edges
5. `Region` - 90 edges
6. `detect_wall_network()` - 77 edges
7. `detect_doors()` - 58 edges
8. `detect_windows()` - 52 edges
...
Cohesion: 0.13
Nodes (10): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Thin barrier bands along the wall planes through a detected door.      The door, Wide garden pairs: jamb-scale anchor window + parked-leaf edge veto., Plug extensions end at their supporting material; slide ends veto.      Geometry, Interrupted-run plugs need jambs that REACH the plug band and a mid     that is, TestGardenDoorSeals (+2 more)
...
Cohesion: 0.15
Nodes (20): _cross_validate(), BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), One merged wall-face run with the evidence its members carried., WallFace, continuous_h_wall() (+12 more)
...
Cohesion: 0.07
Nodes (71): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+63 more)
...
Cohesion: 0.15
Nodes (12): paving_field(), Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, Running-bond paving: continuous course lines, staggered joint lines.      Mirror, Striped fields (paving bonds, tile fields, treads) are not walls., Four wall bands forming a closed rectangular room (outer faces at the     given, Stroke-color pen identity: pairing, faint-ink demotion, dimension     chains, an, rect_room(), TestLatticeDemotion (+4 more)
...
Cohesion: 0.06
Nodes (39): _fit_oriented_rect(), Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u, detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf() (+31 more)

### Community 15 - "Arc Cap-Trim Tests"
Cohesion: 0.07
Nodes (46): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), Prefer the strongest door when two candidates use the same primitives., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_area() (+38 more)
...
Cohesion: 0.07
Nodes (30): Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive., Record whether a line segment passed the polyline-arc length filter., _fit_circle_3pt(), _native_curve_chains(), Fit a circle through 3 points. Returns (cx, cy, radius) or None if     the point, Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of (+22 more)
...
Cohesion: 0.08
Nodes (38): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay() (+30 more)

### Community 22 - "geometry.py"
Cohesion: 0.10
Nodes (43): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _is_arc_like(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+35 more)
...
Cohesion: 0.17
Nodes (9): bind_scale(), binding_texts(), Every text scale near enough to this region to be about it, nearest first., The scale governing one region, or None.      `viewports` must arrive smallest-b, The resolver-level assertion: a region sitting inside the measuring         view, The resolution ladder and how a scale binds to a floor plan.  Region binding is, TestBindingTexts, TestBindScale (+1 more)

### Community 101 - "TestMarkerRings"
Cohesion: 0.12
Nodes (16): _covers(), hline(), horizontal_window(), A clean 2-line capped rectangle IS a window on 5-1133 (see Window B:         two, 5-1133 FP window_0006: 3 short parallel lines whose opening (15px) is         fa, Three parallel lines with no perpendicular end-caps (e.g. a run of         dimen, Three parallel lines spaced far apart (e.g. stair treads) exceed the         gla, Ground truth captured interactively on 5-1133-WD03.pdf (run     2026-06-19_12-02 (+8 more)
...
Cohesion: 0.08
Nodes (24): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, _caption_distance(), _centroid(), _contains(), The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, How far a text span sits from a region, or None if it is not near it.      Horiz, _stored_info() (+16 more)

### Community 111 - "app.py"
Cohesion: 0.04
Nodes (83): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin. (+75 more)
...
Cohesion: 0.10
Nodes (31): cmd_extract(), cmd_inspect(), main(), parse_page_spec(), Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]., build_plumber_counts(), build_pymupdf_counts(), compare_counts() (+23 more)

### Community 115 - "_collect_wall_faces"
Cohesion: 0.11
Nodes (13): parse_measure_viewports(), Tier 1 — the scale the PDF states in its own viewport measure dictionaries.  A C, Every drawing scale this page's viewports state, smallest bbox first.      Small, Split a PDF array string into its top-level ``<< >>`` dictionaries.      Depth-c, Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, split_pdf_dicts(), viewport_scales(), _FakeDoc (+5 more)
...
Cohesion: 0.21
Nodes (7): assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, Split a page into drawing regions. Returns [] for a page with no vector     ink, segment_page(), PageData, TestSegmentPage, TestStageTimingLogs
...
Cohesion: 0.15
Nodes (12): InkMap, bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _clip_cut(), _col_profile(), Strip empty margins; returns absolute (start, end) bin indices., Widest fully-empty internal run of at least min_bins. Leading and     trailing r, First clip edge lying strictly inside the span with ink on both sides.      An e, _row_profile() (+4 more)
...
Cohesion: 0.16
Nodes (18): clip_cut_positions(), Native PDF clip rects, used as extra cut hints for the segmenter.  Clip rects ar, Convert clip edges to (row, col) cut candidates, in bin indices.      Each candi, Tunable constants for page segmentation.  Values are measured, not guessed — see, Page segmentation: split a sheet into its constituent drawings., _centre_in(), count_paths_in(), _edge_gap_sq() (+10 more)

### Community 151 - "normalize_bbox"
Cohesion: 0.17
Nodes (8): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DoorAssemblyTests, _far_wall_network(), path(), Minimal non-empty wall network located far from the doors under test., A single_line_leaf door with no surrounding wall AND no nearby label         is, A single_line_leaf door with no wall but WITH a nearby door label         (e.g.
...
Cohesion: 0.17
Nodes (12): detect_windows(), _frame_axes(), _merge_mullion_chains(), Unit run-axis u (perpendicular to the caps) and perp-axis v (along caps).      C, Join collinear glazing segments across mullion blocks into logical panes.      A, Detect windows as capped openings bridged by a parallel glazing band.      For e, A real window's glazing band is clear glass — nothing between the panes.     An, Control: the bare 2-line capped opening with an empty band interior is         s (+4 more)

### Community 156 - "TestBlindWindowPocket"
Cohesion: 0.25
Nodes (7): Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), shrink(), TestWallNetworkScaled, Horizontal wall drawn as two stroked faces., wall_band_h()
...
Cohesion: 0.11
Nodes (17): 1. The premise, verified, 2. Corpus scale census (measured 2026-08-12), 3. Does scale mismatch explain the bad sheets? Partially., 4. Constant classification table, 4b. Measurements (2026-08-12), 4c. Measurement-harness traps (2026-08-13), 4d. Door constant classification table (frozen 2026-08-13), 5. Decisions (2026-08-12 brainstorm, user-approved) (+9 more)
...
Cohesion: 0.12
Nodes (23): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, The scales block written into each page's summary.json entry., scale_summary_dict(), detection_scale(), DetectionScale, _effective_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete (+15 more)
...
Cohesion: 0.09
Nodes (29): _building_masses(), detect_rooms(), _folding_chain_gap_plug(), _free_space_components(), Room detection: rooms are the connected free-space components between walls.  Ea, World-space room gates pre-multiplied by the detection factor     (areas by fact, Fraction of a bbox area covered by the text spans lying over it., Barrier polygon sealing a window opening.      A horizontal/vertical window's bb (+21 more)
...
## Knowledge Gaps
- **406 isolated node(s):** `storage`, `sheets`, `Project purpose`, `Algorithm reference`, `Commands` (+401 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `client.py`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `wall_band_v`, `geometry.py`, `normalize_bbox`, `hline`, `TestWindowTightPairInterior`, `detect_windows`, `_double_arc`, `SplitDoubleArcTests`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `batch_extract.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `TestThickMaterialPairs`, `_dedupe_openings`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `_frame_axes`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `qualifying_clip_rects`, `framed_triple_window`?**
  _High betweenness centrality (0.140) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Arc Cap-Trim Tests` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `normalize_bbox`, `hline`, `TestWindowTightPairInterior`, `SplitDoubleArcTests`, `batch_extract.py`, `TestThickMaterialPairs`, `_dedupe_openings`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `_frame_axes`, `wall_band_h`, `TestMarkerRings`, `_find_openings`, `app.py`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Arc Cap-Trim Tests` to `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `EntranceDoorTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `wall_band_v`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `normalize_bbox`, `hline`, `detect_windows`, `SplitDoubleArcTests`, `_projected_interval`, `renderer.py`, `scales_in_text`, `TestThickMaterialPairs`, `_dedupe_openings`, `_frame_axes`, `TestWindowInteriorClutter`, `detect_doors`, `app.py`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Are the 99 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 99 INFERRED edges - model-reasoned connections that need verification._
