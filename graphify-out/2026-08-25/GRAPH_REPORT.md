...
## Corpus Check
- 219 files · ~346,576 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3743 nodes · 9532 edges · 209 communities (145 shown, 64 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 696 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4eab450e`
...
1. `PathPrimitive` - 254 edges
2. `PageData` - 164 edges
3. `Candidate` - 159 edges
4. `TextSpan` - 139 edges
5. `Region` - 111 edges
6. `detect_wall_network()` - 91 edges
...
Cohesion: 0.09
Nodes (22): _layer_strong_prior(), Return a high confidence boost when a layer name conclusively names the type., _band_interior_clutter(), _cap_orientation_frames(), _cap_record(), _clutter_grid(), detect_windows(), _frame_axes() (+14 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.07
Nodes (35): _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges(), Polygon (+27 more)
...
Cohesion: 0.08
Nodes (35): _bbox_area(), _bbox_iou(), _cross_validate(), CrossGates, BBox, World-space cross-validation gates, pre-multiplied by the factor.      Only the, # NOTE: there is deliberately no "drop windows that look like wall linework", True when a wall FACE line runs unbroken through the bbox span.      A real wind (+27 more)
...
Cohesion: 0.08
Nodes (14): prim(), _production_cross_gates_unscaled_usages(), quarter_bezier(), Scale-factor behavior of the door gates: identity at 1.0, linear at 0.5.  A "fai, Ratchet on detection/'s production uses of CROSS_GATES_UNSCALED.      CROSS_GATE, Scan detection/**/*.py for PRODUCTION (non-import, non-comment) uses     of the, A quarter-arc cubic Bezier of radius r, hinged at (cx, cy).      r is a WORLD ex, TestArcGatesThreading (+6 more)
...
Cohesion: 0.13
Nodes (17): _cache_file(), _from_dicts(), load_stored(), match_stored(), _merge(), BBox, Path, Add entries, replacing any that describe the same region.      Overlap-based rat (+9 more)
...
Cohesion: 0.10
Nodes (50): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+42 more)
...
### Community 24 - "hline"
Cohesion: 0.10
Nodes (17): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode. (+9 more)
...
Cohesion: 0.07
Nodes (42): attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, room_dict(), Heights (+34 more)
...
Cohesion: 0.09
Nodes (28): _dedupe_by_perp(), _find_openings(), _glaze_index(), Collapse near-collinear duplicates (same perp offset) to one record.      A toil, Largest run of panes spaced like glazing, not like stair treads.      Walks the, Two-axis lookup structure over a frame's glazing pool.      Every cap pair asks, Distinct parallel glazing lines that connect cap ``c1`` to cap ``c2``.      A gl, Pair facing caps and confirm a glazing band bridges each opening.      ``cap_poo (+20 more)
...
Cohesion: 0.17
Nodes (15): PageTruth, TruthItem, evaluate_page(), Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists., score_sheet(), ClassifyTests, entity() (+7 more)
...
Cohesion: 0.04
Nodes (88): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin. (+80 more)
...
Cohesion: 0.17
Nodes (14): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage() (+6 more)

### Community 115 - "_collect_wall_faces"
Cohesion: 0.07
Nodes (41): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay() (+33 more)
...
Cohesion: 0.17
Nodes (8): iou(), match_entities(), MatchResult, BBox, entity(), IouTests, MatchTests, Ground truth is matched to output geometrically.  Entity ids (door_0015) are ord
...
Cohesion: 0.08
Nodes (28): dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), Path (+20 more)
...
Cohesion: 0.11
Nodes (27): Tunable constants for page segmentation.  Values are measured, not guessed — see, InkMap, Binary ink occupancy map over a page, used to find whitespace gutters., bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _clip_cut(), _col_profile() (+19 more)
...
Cohesion: 0.06
Nodes (36): door_candidate(), fill_ring(), hline(), path(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, Filled arrowhead triangle (a marker ring) pointing down at `tip`., Stairs are furniture to the room stage: a room polygon runs to the     enclosing, A filled wall band exported as two triangles (CAD fill triangulation).      Each (+28 more)
...
Cohesion: 0.09
Nodes (22): _check_opening_clear(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, TextSpan, DoorAssemblyTests, DoorV2OpeningCheckTests, EntranceDoorTests (+14 more)
...
Cohesion: 0.15
Nodes (10): _FakeDoc, _FakePage, _Matrix, Parsing /VP -> /Measure viewport dictionaries.  The array strings below are copi, Just enough of fitz.Rect for viewport_scales: x0/y0/x1/y1 attributes., Just enough of fitz.Matrix for page_transform: a/b/c/d/e/f attributes., A page double with no fitz dependency -- no PDF is ever opened.      Only test_s, The bug lived here, not in viewport_bbox_to_px's arithmetic: the flip     formul (+2 more)
...
### Community 199 - "TestFarSidePairs"
Cohesion: 0.16
Nodes (8): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co, TestCenterlines, TestFarSidePairs, TestNetworkAssembly
...
### Community 202 - "TestBlindWindowPocket"
Cohesion: 0.40
Nodes (4): _production_door_gates_unscaled_usages(), Scan detection/**/*.py for PRODUCTION (non-import, non-comment) uses     of the, Ratchet on detection/'s production uses of DOOR_GATES_UNSCALED.      DOOR_GATES_, TestDoorGatesUnscaledStopgapRatchet

### Community 203 - "TestComponentFiltering"
Cohesion: 0.50
Nodes (3): Tier 1 — the scale the PDF states in its own viewport measure dictionaries.  A C, Every drawing scale this page's viewports state, smallest bbox first.      Small, viewport_scales()
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `wall_band_v` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `test_extraction_transform.py`, `RunDirTests`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `hline`, `_segments_min_distance`, `detect_windows`, `_double_arc`, `_projected_interval`, `SplitDoubleArcTests`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestThickMaterialPairs`, `TestFarSidePairs`, `TestBlindWindowPocket`, `TestWindowSpanOvershootRetune`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.176) - this node is a cross-community bridge._
- **Why does `Candidate` connect `hline` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `_segments_min_distance`, `plumber.py`, `SplitDoubleArcTests`, `batch_extract.py`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestPlumberTableBBox`, `_dedupe_openings`, `TestBlindWindowPocket`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_find_openings`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `swing_door` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Cap-Trim Tests`, `arcs.py`, `test_extraction_transform.py`, `_fit_circle_3pt`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `_projected_interval`, `SplitDoubleArcTests`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `TestFarSidePairs`, `wall_band_h`, `detect_doors`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `framed_triple_window`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Are the 120 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 120 INFERRED edges - model-reasoned connections that need verification._
