...
## Corpus Check
- 182 files · ~286,032 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3098 nodes · 7673 edges · 201 communities (131 shown, 70 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 482 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f06f7c87`
...
Cohesion: 0.22
Nodes (4): Record whether a line segment passed the polyline-arc length filter., Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive.
...
Cohesion: 0.18
Nodes (8): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, line(), path(), quarter_arc_lines(), Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, rect_lines(), SingleLineLeafTests
...
Cohesion: 0.08
Nodes (33): _cross_validate(), BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), BBox, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried. (+25 more)
...
Cohesion: 0.07
Nodes (66): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+58 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.05
Nodes (39): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate (+31 more)
...
### Community 24 - "hline"
Cohesion: 0.12
Nodes (20): door_open_leaf_path_indices(), _leaf_ink_indices(), _merge_double_door_assemblies(), Merge pairs of adjacent single-door assemblies into double-swing candidates., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, DoubleDoorTests, OpenLeafExclusionTests (+12 more)
...
Cohesion: 0.19
Nodes (10): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, TestCenterlines (+2 more)
...
Cohesion: 0.09
Nodes (39): cmd_extract(), cmd_inspect(), main(), parse_page_spec(), Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]., generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON. (+31 more)
...
Cohesion: 0.24
Nodes (8): detection_scale(), _effective_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., info(), detection_scale(): PageScales + regions -> one detection factor per page., region(), TestDetectionScale
...
Cohesion: 0.08
Nodes (39): _bbox_area(), _bbox_center(), _interval_overlap(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f (+31 more)
...
Cohesion: 0.27
Nodes (5): page(), Quarter-arc + a double-line leaf, as a faithful export at any scale.      radius, swing_door(), TestFaithfulExportDetection, TestOrchestratorWiring
...
Cohesion: 0.15
Nodes (21): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., The scales block written into each page's summary.json entry., scale_summary_dict(), scale_table() (+13 more)
...
### Community 192 - "TestWindowTopology"
Cohesion: 0.24
...
Cohesion: 0.40
Nodes (4): 270-degree polyline arc with 16 segments — far wider than a quarter-circle door, Compute 15-degree angle bins for line segments — fixture sanity helper., _seg_angle_bins(), wide_arc_lines()
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Debug Trace Collector` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cap-Trim Tests`, `arcs.py`, `test_extraction_transform.py`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `detect_windows`, `_double_arc`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `batch_extract.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `MainExceptionIsolationTests`, `TestOpeningSeals`, `TestThickMaterialPairs`, `TestNetworkQueries`, `TestBlindWindowPocket`, `TestXYCut`, `TestFloorPlansRegression`, `swing_door`, `TestCrossGatesUnscaledStopgapRatchet`, `client.py`, `TestWindowTopology`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `_seg_angle_bins`, `TestWindowSpanOvershootRetune`, `TestThickMaterialPairs`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.145) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `TestThickMaterialPairs` to `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Cap-Trim Tests`, `arcs.py`, `test_extraction_transform.py`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `detect_windows`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `scales_in_text`, `renderer.py`, `TestOpeningSeals`, `TestBlindWindowPocket`, `TestXYCut`, `TestDoorGatesUnscaledStopgapRatchet`, `client.py`, `TestFloorPlansRegression`, `detect_doors`, `app.py`, `TestAnnotationPenBarriers`, `_segments_min_distance`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `PageData` connect `test_layout_segmenter.py` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Debug Trace Collector`, `_dedupe_openings`, `Wall Network Construction & Tests`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cap-Trim Tests`, `fill_ring`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `TestNetworkQueries`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `scales_in_text`, `_collect_wall_faces`, `TestThickMaterialPairs`, `TestXYCut`, `swing_door`, `TestDoorGatesUnscaledStopgapRatchet`, `detect_doors`, `EntranceDoorTests`, `TestAnnotationPenBarriers`?**
