...
## Corpus Check
- 181 files · ~279,273 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3044 nodes · 7523 edges · 204 communities (135 shown, 69 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 468 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `399d0e31`
...
- [[_COMMUNITY__FakePage|_FakePage]]
- [[_COMMUNITY_test_door_assembly.py|test_door_assembly.py]]
...
## Communities (204 total, 69 thin omitted)
...
Cohesion: 0.16
Nodes (9): apply_classification(), Apply a classification response to a region list.      Returns new Region object, _CapturingClient, Stands in for genai.Client, recording the config it was called with., The response must be schema-constrained at decode time.      Plain JSON mode doe, region(), response(), TestApplyClassification (+1 more)
...
Cohesion: 0.06
Nodes (66): DoorGates, World-space door gates, pre-multiplied by the detection factor.      Fields keep, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+58 more)
...
Cohesion: 0.30
Nodes (8): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), detect_schedules()
...
Cohesion: 0.11
Nodes (22): _fit_circle_3pt(), _native_curve_chains(), Fit a circle through 3 points. Returns (cx, cy, radius) or None if     the point, Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, _clutter_grid(), Page-wide point buckets for _band_interior_clutter.      The clutter scan asks w, PathPrimitive, ChainedCurveSwingDetectionTests (+14 more)
...
Cohesion: 0.16
Nodes (19): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+11 more)

### Community 22 - "geometry.py"
Cohesion: 0.07
Nodes (67): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some, Estimate sweep angle of a Bézier arc from its endpoints and estimated center. (+59 more)
...
Cohesion: 0.21
Nodes (12): build_ink_map(), InkMap, is_page_spanning(), Binary ink occupancy map over a page, used to find whitespace gutters., bins[row][col] is 1 where drawn ink falls, 0 elsewhere., True for sheet furniture: a border rule or column divider that runs the     leng, page(), path() (+4 more)
...
Cohesion: 0.11
Nodes (22): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, diagonal_window(), framed_triple_window(), horizontal_window(), path() (+14 more)
...
Cohesion: 0.16
Nodes (11): Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode., TestCrossGates, BBox, A distant door must not suppress a window it only clips after the         20px d (+3 more)
...
Cohesion: 0.09
Nodes (17): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., canonical_denominators(), cluster_denominators(), denominator_from_c(), format_scale(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta, Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present. (+9 more)

### Community 111 - "app.py"
Cohesion: 0.05
Nodes (80): _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin. (+72 more)
...
Cohesion: 0.14
Nodes (14): page_fallback_region(), Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page(), PageData, block(), cut(), page() (+6 more)
...
Cohesion: 0.12
Nodes (21): _centre_in(), _clip_cut(), _col_profile(), count_paths_in(), _edge_gap_sq(), _fold_small_leaves(), _merge_captions(), _overlap_area() (+13 more)
...
Cohesion: 0.16
Nodes (21): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), The scales block written into each page's summary.json entry., scale_summary_dict(), detection_scale(), DetectionScale, _effective_denominator() (+13 more)
...
Cohesion: 0.10
Nodes (22): _building_masses(), detect_rooms(), _folding_chain_gap_plug(), _free_space_components(), Room detection: rooms are the connected free-space components between walls.  Ea, World-space room gates pre-multiplied by the detection factor     (areas by fact, Fraction of a bbox area covered by the text spans lying over it., Barrier polygon sealing a window opening.      A horizontal/vertical window's bb (+14 more)
...
Cohesion: 0.30
Nodes (6): The per-region scale table printed after each page., scale_table(), Table, raw is lifted verbatim from PDF text and can contain a bracket         sequence, region(), TestScaleTable
...
### Community 197 - "test_door_assembly.py"
Cohesion: 0.32
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `detect_windows`, `_double_arc`, `SplitDoubleArcTests`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `batch_extract.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `TestOpeningSeals`, `renderer.py`, `TestNetworkQueries`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `EntranceDoorTests`, `client.py`, `test_door_assembly.py`, `OpenLeafExclusionTests`, `TestThickMaterialPairs`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.151) - this node is a cross-community bridge._
- **Why does `Candidate` connect `wall_band_h` to `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `plumber.py`, `SplitDoubleArcTests`, `ScaleInfo`, `test_scale_units.py`, `batch_extract.py`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `TestWindowTightPairInterior`, `EntranceDoorTests`, `_dedupe_openings`, `TestCrossWindowToleranceUnscaled`, `test_door_assembly.py`, `OpenLeafExclusionTests`, `TestMinWidthNegativeControl`, `TestWindowInteriorClutter`, `TestMarkerRings`, `_find_openings`, `app.py`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `_projected_interval` to `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `EntranceDoorTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Cap-Trim Tests`, `wall_band_v`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `hline`, `detect_windows`, `SplitDoubleArcTests`, `renderer.py`, `test_scale_units.py`, `scales_in_text`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `EntranceDoorTests`, `test_door_assembly.py`, `OpenLeafExclusionTests`, `wall_band_h`, `detect_doors`, `app.py`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Are the 102 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 102 INFERRED edges - model-reasoned connections that need verification._
