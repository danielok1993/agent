...
## Corpus Check
- 414 files · ~1,555,979 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5535 nodes · 13556 edges · 327 communities (216 shown, 111 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 1018 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c8245f1e`
...
- [[_COMMUNITY_Outcome — iteration 3, step 12 (2026-09-06, branch `recalgate-denominator-stored-scale`, shipped pending the user's decision)|Outcome — iteration 3, step 12 (2026-09-06, branch `recal/gate-denominator-stored-scale`, shipped pending the user's decision)]]
...
Cohesion: 0.09
Nodes (29): _draw_legend(), draw_overlay(), _draw_regions(), cache_file(), cache_key(), load_regions(), page_content_hash(), Path (+21 more)
...
Cohesion: 0.05
Nodes (113): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+105 more)
...
Cohesion: 0.06
Nodes (30): apply_classification(), Apply a classification response to a region list.      Returns new Region object, _CapturingClient, Region classification parsing tests (gemini/classifier.py).  No API calls: apply, Stands in for genai.Client, recording the config it was called with., The response must be schema-constrained at decode time.      Plain JSON mode doe, region(), response() (+22 more)
...
Cohesion: 0.15
Nodes (17): _collect_material_marks(), _dash_row_indices(), _pen_key(), Path indices of drawn dash lines: annotation, never faces.      A dashed line ty, (midpoint, angle) of every short solid stroke, gathered once per page.      Thes, Quantized stroke color — the pen identity of a drawn line., The two classes the corpus sweep exposed on the first cut of the     rule: a wal, 135-degree strokes from face x0 to face x1, clipped to the band. (+9 more)
...
Cohesion: 0.15
Nodes (10): detect(), LeafPairTests, ParkedLeafTests, PocketLeafTests, s04 door_0014: a pocket door drawn as a bare stroked qu (4.7x92px,     0.56px) h, A closed 4-segment stroked (fill-less) rectangle of `l` items., parked_leaf: a stroked panel parked flush along a wall band that ends     at a j, sliding_of() (+2 more)
...
Cohesion: 0.07
Nodes (39): build_request_parts(), classify_regions(), BBox, Page, Ask Gemini what each segmented region is.  One call per page. Each region goes a, Render one region as its own PNG, scaled so its long edge is about     CROP_TARG, Distinct text inside a region, largest font first. Many CAD exports     outline, One API call for the whole page. Returns classified regions + warnings. (+31 more)

### Community 17 - "arcs.py"
Cohesion: 0.09
Nodes (28): _cross_validate(), Validate doors/windows against the wall-centerline network.      Doors keep the, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried., Connected wall-centerline network (internal-only, never serialized)., Path indices of every face that contributed to a centerline., Length-weighted median stroke width of the paired stroked faces.          Anchor, WallFace (+20 more)
...
Cohesion: 0.07
Nodes (48): build_parser(), cmd_extract(), cmd_inspect(), main(), parse_page_spec(), positive_metres(), argparse type: a positive, finite height in metres., Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]. (+40 more)
...
### Community 24 - "hline"
Cohesion: 0.08
Nodes (27): _area(), _band_interior_clutter(), _cap_orientation_frames(), _cap_record(), _clutter_grid(), _dedupe_openings(), detect_windows(), _frame_axes() (+19 more)
...
Cohesion: 0.06
Nodes (56): _apply(), _as_transform(), classify_page(), _color_tuple(), extract_document(), extract_images(), extract_page(), extract_paths() (+48 more)
...
Cohesion: 0.13
Nodes (15): Tunable constants for page segmentation.  Values are measured, not guessed — see, is_page_spanning(), _is_unfilled_rect(), nested_frame_indices(), path_length(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng, Path indices of nested sheet furniture: unfilled rectangles with at     least mi (+7 more)
...
Cohesion: 0.16
Nodes (16): sha256_of(), _check_provenance(), _ordered(), pending(), What a persisted sweep still needs verdicts on.  Reads the run output the sweep, Unreviewed detections, keyed by 1-based page then entity type.      Pages and ty, This sheet cannot be reviewed right now. Report it and move on., No persisted sweep output for this slug. (+8 more)

### Community 104 - "detect_doors"
Cohesion: 0.12
Nodes (15): Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), diagonal_window(), path(), A real window's glazing band is clear glass — nothing between the panes.     An, Control: the bare 2-line capped opening with an empty band interior is         s, 5-1133 FP w19/w21/w25/w32/w33: an insulation-hatched wall. The two         wall, Insulation hatch drawn with pure line segments (no re/qu/c): the         diagona (+7 more)
...
Cohesion: 0.06
Nodes (45): _item(), load_truth(), iou(), BBox, IouTests, delta(), attrib_delta.py <slug> FIELD=MULT [...]: run the harness once per config (each a, rooms in a with no IoU>=0.5 match in b. (+37 more)

### Community 107 - "vline"
Cohesion: 0.14
Nodes (10): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, EntranceDoorTests, line(), path(), quarter_arc_lines(), Regression + new-feature tests for entrance-door threshold-line handling.      G, Swing-anchored single-line leaf check (v3).      A door panel is often drawn as (+2 more)
...
Cohesion: 0.12
Nodes (10): DetectionScale, _contains(), select_room_scale(), _door(), room_polys holds unscaled rooms too, so the first assigned room can         be t, Referential integrity must hold in BOTH directions: if the opening         names, _room(), TestComputeTakeoff (+2 more)

### Community 111 - "app.py"
Cohesion: 0.14
Nodes (6): _hface(), A bare horizontal wall-face _Seg for isolated merge-tolerance tests., Isolates _merge_collinear_segs's offset-tolerance scaling directly —     the exa, TestMergeCollinearOffsetScaling, TestRoomGatesConstruction, TestWallGatesConstruction
...
Cohesion: 0.06
Nodes (31): bind_scale(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page.      `fallback` is a sc, resolve_page_scales(), match_stored(), _merge(), BBox, Add entries, replacing any that describe the same region.      Overlap-based rat (+23 more)
...
Cohesion: 0.17
Nodes (10): detect(), fold_chain(), FoldChainTests, folding_of(), OpenVTests, ParkedStackPairTests, open_v: a lone bifold drawn half-open as a wide V of stroked     double-line lea, The top jamb's face ends lifted dy px above the tip line: the         anchor-to- (+2 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.13
Nodes (16): dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item_payload(), Path, The user's per-sheet verdicts, and how they are read.  One file per sheet under, Create the unlabeled ground-truth file for a newly adopted sheet. (+8 more)
...
Cohesion: 0.03
Nodes (75): _accept_jamb_rings(), _back_edge_cover(), _building_masses(), _contains_text(), detect_rooms(), _drop_window_exterior_sides(), _edge_profile(), _EdgeProfile (+67 more)

### Community 126 - "_segments_min_distance"
Cohesion: 0.07
Nodes (18): _clip_plug_tails(), _door_plugs(), _open_leaf_edges(), End each bbox-edge plug's tails AT the material they touch.      _door_plugs tri, How far along (ux, uy) from corner a tail sample first touches wall material., Thin barrier bands along the wall planes through a detected door.      The door, Bbox edges of a garden-layout double door that are room floor, not wall.      A, _seek_jamb() (+10 more)
...
Cohesion: 0.04
Nodes (66): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), _merge_double_door_assemblies(), Prefer the strongest door when two candidates use the same primitives., Merge pairs of adjacent single-door assemblies into double-swing candidates., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line (+58 more)
...
Cohesion: 0.06
Nodes (28): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, True when a wall FACE line runs unbroken through the bbox span.      A real wind, _wall_runs_through(), prim(), _production_cross_gates_unscaled_usages(), _production_door_gates_unscaled_usages(), quarter_bezier() (+20 more)
...
Cohesion: 0.11
Nodes (27): cache_file(), cache_key(), load_labels(), Path, On-disk cache of room labels, keyed by page content AND the room polygons the la, Stable digest of the room outlines a labelling was made against.      A cached l, room_geometry_hash(), save_labels() (+19 more)
...
Cohesion: 0.21
Nodes (10): _bridge_white_runs(), _equivalent_sides(), Band-shaped convex hulls closing the gaps in accepted white-ring runs.      gate, (short, long) of the rectangle with this polygon's area and perimeter.      The, _bridge_white_runs is detect_rooms's ONLY production call site     (detection/ro, TestBridgeWhiteRunsGapScaling, Accepted hollow-wall/joinery _FillRing over the given rectangle., Bridges close OPEN SPANS in accepted white-ring runs; rings already     connecte (+2 more)
...
Cohesion: 0.24
Nodes (5): A saved 200x400pt PDF with two lines, a word and an image, rotated.      Saved a, Builds all four rotations once; each test reopens what it needs., RotatedPdfTestCase, TestPageTransform, write_rotated_pdf()
...
### Community 149 - "_arc"
Cohesion: 0.12
Nodes (6): TestCase, Path, Return the sheet's path, or skip the test with an actionable message., require_sheet(), LoaderTests, The corpus loader resolves slugs against the committed manifest.  Every test bui

### Community 150 - "TestAnnotationPenBarriers"
Cohesion: 0.12
Nodes (13): hline(), path(), Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, Filled arrowhead triangle (a marker ring) pointing down at `tip`., Stairs are furniture to the room stage: a room polygon runs to the     enclosing, ends: 'bands' — an 8px band across each end (the storage);         'lines' — a s, rect_room(), stair_arrowhead() (+5 more)
...
Cohesion: 0.11
Nodes (18): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), BBox, Image, Room entities carry their closed polygon; draw its true shape instead     of the, FreeTypeFont, ImageDraw (+10 more)
...
Cohesion: 0.08
Nodes (33): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, DimensionMatch, detection_scale(), `dimensions` is the page's ticked-dimension-string matches on the FULL     page, _fallback_info(), PageScales, A scale the user supplied for the whole run, as a ladder entry.      `source="us (+25 more)
...
Cohesion: 0.14
Nodes (9): Skip helper for tests that need a real corpus sheet.  Corpus knowledge lives in, Measured scale expectations across the regression corpus.  Every number was meas, s13 is the one corpus sheet whose viewport and printed scale disagree.      It m, The resolver-level assertion: a region sitting inside the measuring         view, read(), TestKnownConflict, TestSheetsWithNoRecoverableScale, TestTextScales (+1 more)
...
Cohesion: 0.10
Nodes (16): _collect_wall_faces(), _is_dashed(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0"., Return (stroked wall faces, filled-band centerlines)., fill_ring(), marker_ring(), Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, Filled triangle/dart exploded into chained `l` items (a leader tip). (+8 more)
...
Cohesion: 0.04
Nodes (62): attach_takeoff(), _page_summary_dict(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, The whole page as one document. (+54 more)

### Community 188 - "TestCheckDoorLeaves"
Cohesion: 0.20
Nodes (14): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, Flip a manifest entry's `labeled` flag and write the manifest back.      `labele (+6 more)
...
Cohesion: 0.15
Nodes (10): Scales stated on the sheet, unbound to any drawing.      inspect does not segmen, unbound_scale_lines(), cluster_denominators(), Group near-equal denominators, largest group first in input order.      Lives he, The inspect command's unbound scale listing.  inspect never segments regions, so, TestUnboundScaleLines, text(), viewport() (+2 more)
...
Cohesion: 0.23
Nodes (6): _plane_stamp(), The plug-less fallback seal, restricted to the door's wall-plane edges.      A d, How far, along (ux, uy) from corner, the material a plug tail touches runs., _tail_material_end(), A plug-less door seals along its wall-plane edges only (W-gate     iteration 3 s, TestPlaneRestrictedFallback
...
Cohesion: 0.16
Nodes (11): TruthItem, Regression corpus: fixture resolution, ground truth, matching, and the sweep., match_entities(), MatchResult, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, entity(), MatchTests, Ground truth is matched to output geometrically.  Entity ids (door_0015) are ord (+3 more)

### Community 203 - "HygieneRuleTests"
Cohesion: 0.12
Nodes (17): Scale-factor behavior of walls/rooms gates: identity at 1.0, shrunk-world at 0.5, Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), rooms_for(), shrink(), TestOrchestratorForwardsFactor, TestRoomsScaled (+9 more)
...
Cohesion: 0.15
Nodes (12): CLAUDE.md split — final verification, Complete repair log, Deferred minors (for the whole-branch review to triage), Knowledge graph refresh, Pre-move overlap measurements (reported, never acted on), `replace` repairs (post-review fix wave, 2026-09-09), Retrieval check, Step 1 — verifier output (+4 more)
...
Cohesion: 0.18
Nodes (10): Bbox edges whose plug tails may SEEK their jamb beyond the fixed reach.      Onl, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, Bbox short-end edges of a sliding door: across the wall, never wall plane., Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, _restrict_swing_plugs(), _seek_edges(), _sliding_end_edges(), _swing_hinge_edges() (+2 more)

### Community 227 - "resolve_room_labels"
Cohesion: 0.10
Nodes (17): probe_box.py <slug> <seal> x0 y0 x1 y1 [door_id ...]  Run the harness at the giv, tap_fsc(), tap_gap(), main(), draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1. (+9 more)
...
Cohesion: 0.29
Nodes (4): horizontal_window(), A W1-style horizontal window: 3 tight horizontal glazing lines centered     in a, A W4-style vertical window: 3 tight vertical glazing lines closed by two     hor, vertical_window()
...
Cohesion: 0.13
Nodes (21): drop(), _behind(), census(), _covering_faces(), _median(), _mrr(), _near_side_segments(), Step 16 — what makes s11's storage cupboard a SPACE when a cavity-wall reveal st (+13 more)

### Community 252 - "TestWindowTopology"
Cohesion: 0.33
Nodes (8): apply_repairs(), load_source(), main(), Read CLAUDE.md at the map's PINNED ref, never the working tree.      LOAD-BEARIN, Append-mode: replace this heading's section in an existing document,     insert, Re-flow one long line into 79-column paragraphs. Whitespace only.      break_on_, upsert(), wrap()

### Community 255 - "render_page_svg"
Cohesion: 0.06
Nodes (36): _effective_denominator(), _from_dimensions_warning(), _gate_choice(), _gate_denominator(), _measured_only_warning(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., The denominator a claim may drive the gates with ON ITS OWN, or None.      A DRA (+28 more)

### Community 256 - "swing_door"
Cohesion: 0.11
Nodes (15): leaf_pair_door(), open_v_door(), page(), prim(), End-to-end door scale behavior on FAITHFUL 1:100 fixtures.  A faithful 1:100 exp, open_v (detection/doors/folding.py::_open_v_match) reads three gates:     gates., leaf_pair (detection/doors/sliding.py) reads gates.DOOR_SLIDE_PANEL_MIN_THICKNES, Quarter-arc + a double-line leaf, as a faithful export at any scale.      radius (+7 more)
...
Cohesion: 0.07
Nodes (21): TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., A filled wall band exported as two triangles (CAD fill triangulation).      Each, A chimney breast / pier drawn as a closed box on the room side of a     wall ban, A window reveal in a cavity wall (s17 rooms 0015/0034): the wall is     drawn as, An ENTRANCE is a door seal that runs ALONG the space's boundary over     at leas, s15: the "steel ridge beam" line — a dashed line drawn as a row of     14.8px pi (+13 more)

### Community 264 - "QuadPerimeterTests"
Cohesion: 0.57
Nodes (6): fail(), invert(), main(), norm(), Split a generated doc into {heading: body} on '## ' headings.      A target docu, sections_of()
...
Cohesion: 0.02
Nodes (164): _angle_diff_mod180(), _bboxes_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), _project_onto_axis() (+156 more)
...
Cohesion: 0.15
Nodes (7): Stick-font text drawn as line strokes (s06/s11/s16/s20: no text     spans, every, HITL' in 14px stick glyphs, cap line y, baseline y + 14., A DRAWN DASH LINE — one line its line type exploded into a periodic     row of s, TestDashRowExclusion, TestNetworkAssembly, TestVectorTextExclusion, vline()
...
Cohesion: 0.09
Nodes (21): _collect_fill_rings(), detect_wall_network(), _fill_key(), _fill_ring_components(), _fill_seam_indices(), _is_light_pen(), Path indices of fill-ring seams — see _fill_seams., Group ring ids (restricted to `members`) connected by shared seams.      Exporte (+13 more)

### Community 313 - "fixture_picture.py"
Cohesion: 0.28
Nodes (7): main(), panel(), The step-17 pin fixture as a picture: TestWallRecessTabbedByAPerpendicularBand's, stage(), main(), The tabbed fixture (TestBandPocketTabbedByAPerpendicularBand) through the stage, run()
...
Cohesion: 0.27
Nodes (4): s15 measured 2026-08-13: 214 text spans bridge every gutter, so the     text-inc, segment(), TestGoldenSegmentation, TestS15PathsOnlyRetry
...
## Knowledge Gaps
- **863 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+858 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Double-Door Merge & Gemini Client` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `TestWindowArbitraryAngle`, `TestVectorTextExclusion`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `Room Detection Tests`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Room Polygonization Internals`, `client.py`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `test_extraction_transform.py`, `TestAnnotationPenBarriers`, `hline`, `_production_cross_gates_unscaled_usages`, `_production_door_gates_unscaled_usages`, `TestSwingHingePlugRestriction`, `plumber.py`, `_projected_interval`, `test_curve_arc_garden_doors.py`, `renderer.py`, `batch_extract.py`, `_collect_wall_faces`, `DoorV2OpeningCheckTests`, `test_layout_golden.py`, `jamb_pens.py`, `_dedupe_openings`, `_is_light_pen`, `EntranceDoorTests`, `_is_light_pen`, `TestWindowExteriorSide`, `test_batch_extract.py`, `_covers`, `TestDashRowDiscriminators`, `collect_room_spans`, `squat_cap_window`, `wall_band_h`, `TestMarkerRings`, `bezier_arc`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `ablate.py`, `_collect_wall_faces`, `TestNetworkQueries`, `test_door_assembly.py`, `resolve_heights`, `_segments_min_distance`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `_fit_circle_3pt` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `_projected_interval`, `ScaleInfo`, `batch_extract.py`, `client.py`, `TestThickMaterialPairs`, `ShaMismatchAgainstTruthTests`, `Arc Cycle-Cap Pruning Tests`, `resolve_page_regions`, `TestAnnotationPenBarriers`, `_dedupe_openings`, `resolver.py`, `TestCliEquivalence`, `test_door_assembly.py`, `TestXYCut`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `TestVectorTextExclusion` to `EntranceDoorTests`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `client.py`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `_leaf_ink_indices`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `TestAnnotationPenBarriers`, `_production_cross_gates_unscaled_usages`, `test_layout_segmenter.py`, `TestSwingHingePlugRestriction`, `_projected_interval`, `test_curve_arc_garden_doors.py`, `renderer.py`, `_collect_wall_faces`, `_is_light_pen`, `EntranceDoorTests`, `_is_light_pen`, `TestWindowExteriorSide`, `test_batch_extract.py`, `TestWindowTightPairInterior`, `collect_room_spans`, `squat_cap_window`, `vline`, `ablate.py`, `TestAnnotationPenBarriers`, `TestNetworkQueries`, `test_door_assembly.py`, `framed_triple_window`, `_segments_min_distance`?**
