...
## Corpus Check
- 273 files · ~507,816 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4740 nodes · 12043 edges · 260 communities (186 shown, 74 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 930 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `be5509ee`
...
- [[_COMMUNITY_TestSpanFilterIsLoadBearing|TestSpanFilterIsLoadBearing]]
- [[_COMMUNITY_probe_box.py|probe_box.py]]
...
Cohesion: 0.07
Nodes (32): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+24 more)
...
Cohesion: 0.07
Nodes (62): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+54 more)
...
Cohesion: 0.08
Nodes (34): dump_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), Path, The user's per-sheet verdicts, and how they are read.  One file per sheet under (+26 more)
...
Cohesion: 0.08
Nodes (19): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, True when a wall FACE line runs unbroken through the bbox span.      A real wind, _wall_runs_through(), prim(), _production_door_gates_unscaled_usages(), Scan detection/**/*.py for PRODUCTION (non-import, non-comment) uses     of the, Ratchet on detection/'s production uses of DOOR_GATES_UNSCALED.      DOOR_GATES_ (+11 more)
...
Cohesion: 0.13
Nodes (14): attrib.py <slug> <x0> <y0> <x1> <y1> FIELD=MULT [FIELD=MULT ...]  Runs the harne, attrib_rooms.py <slug> FIELD=MULT [...]: for each config (each alone on top of t, COLLINEAR_OFFSET_TOL forms, measured with the census harness on top of the curre, denom_at(), _install_taps(), load(), mm(), W-gate census harness: run the stage-5 chain exactly as tools/regress.py does, w (+6 more)
...
Cohesion: 0.17
Nodes (19): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, DetectionPage, load_detection_pages(), One corpus sheet's detection page data, exactly as tools/regress.py sees it.  Sh, Every detected page of the sheet (or only `pages`, 1-based). (+11 more)
...
Cohesion: 0.12
Nodes (28): _apply(), _as_transform(), classify_page(), extract_document(), extract_images(), extract_page(), extract_text(), get_ocg_names() (+20 more)
...
### Community 107 - "vline"
Cohesion: 0.17
Nodes (8): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, EntranceDoorTests, quarter_arc_lines(), Regression + new-feature tests for entrance-door threshold-line handling.      G, Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, rect_lines(), SingleLineLeafTests
...
Cohesion: 0.04
Nodes (122): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bbox_expanded(), _bbox_union(), _bboxes_overlap(), _interval_overlap(), _line_angle_deg() (+114 more)
...
Cohesion: 0.06
Nodes (27): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., _fallback_info(), The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, A scale the user supplied for the whole run, as a ladder entry.      `source="us, _stored_info(), Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, canonical_denominators(), cluster_denominators() (+19 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.05
Nodes (50): _accept_jamb_rings(), _building_masses(), _clip_plug_tails(), _contains_text(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _edge_face_cover() (+42 more)
...
Cohesion: 0.11
Nodes (13): paving_field(), Running-bond paving: continuous course lines, staggered joint lines.      Mirror, Striped fields (paving bonds, tile fields, treads) are not walls., Stroke-color pen identity: pairing, faint-ink demotion, dimension     chains, an, Stick-font text drawn as line strokes (s06/s11/s16/s20: no text     spans, every, HITL' in 14px stick glyphs, cap line y, baseline y + 14., Four wall bands forming a closed rectangular room (outer faces at the     given, rect_room() (+5 more)
...
Cohesion: 0.07
Nodes (36): _cross_validate(), Validate doors/windows against the wall-centerline network.      Doors keep the, BBox, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried., Connected wall-centerline network (internal-only, never serialized)., Path indices of every face that contributed to a centerline., Length-weighted median stroke width of the paired stroked faces.          Anchor (+28 more)

### Community 131 - "test_layout_segmenter.py"
Cohesion: 0.11
Nodes (18): _bridge_white_runs(), _equivalent_sides(), Band-shaped convex hulls closing the gaps in accepted white-ring runs.      gate, (short, long) of the rectangle with this polygon's area and perimeter.      The, _bridge_white_runs is detect_rooms's ONLY production call site     (detection/ro, TestBridgeWhiteRunsGapScaling, fill_ring(), marker_ring() (+10 more)
...
Cohesion: 0.13
Nodes (16): _collect_wall_faces(), _fill_seam_indices(), _fill_seams(), _FillRing, _is_dashed(), _rate_fill_classes(), Background-colored rings that could be hollow walls or built-in runs.      White, True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0". (+8 more)

### Community 140 - "ShaMismatchAgainstTruthTests"
Cohesion: 0.13
Nodes (16): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sheet_entry() (+8 more)
...
Cohesion: 0.11
Nodes (24): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, The per-region scale table printed after each page., scale_table(), detection_scale(), PageScales, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict() (+16 more)
...
Cohesion: 0.10
Nodes (16): dumps_truth(), Serialize a sheet's verdicts exactly as they are stored on disk.      Split out, SheetTruth, _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, score_sheet(), TestGroundTruthCarriesScales (+8 more)
...
Cohesion: 0.08
Nodes (39): attach_takeoff(), _page_summary_dict(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv (+31 more)

### Community 188 - "pipeline.py"
Cohesion: 0.15
Nodes (14): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., render_page_png(), _candidate_to_dict(), collect_warnings(), _entity_to_dict(), make_output_dir() (+6 more)
...
Cohesion: 0.09
Nodes (21): Evidence: what broke at f = 50/92.2 = 0.542 (all measured on the real PDF), Group 1 — safe reference moves, Group 2 — thin-margin moves (three of five tried and reverted), Group 3 — class fixes, Handoff: W-gate recalibration (the proper fix behind `fix/measured-scale-detection-factor`), How the ablation was done (reproduce in ~30 min), Outcome — iteration 2 (2026-09-04, branch `recal/w-gate-iter2`), Outcome — iteration 3, step 1 (2026-09-04, branch `fix/section-line-dashes-not-hatch`) (+13 more)
...
Cohesion: 0.16
Nodes (8): Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., A filled wall band exported as two triangles (CAD fill triangulation).      Each, TestEmptyNetwork, TestFillSeams, TestJambNib, TestSwingRecessDissolution, triangulated_fill_band_v()

### Community 227 - "TestRenderPageSvg"
Cohesion: 0.36
...
## Knowledge Gaps
- **647 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+642 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Arc Detection Primitives` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `Window Detection & Tests`, `test_layout_segmenter.py`, `TestWindowArbitraryAngle`, `client.py`, `Wall Network Construction & Tests`, `client.py`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `_fit_circle_3pt`, `_arc`, `TestAnnotationPenBarriers`, `review.py`, `viewport_bbox_to_px`, `TestBlindWindowPocket`, `TestSwingHingePlugRestriction`, `_double_arc`, `_projected_interval`, `test_curve_arc_garden_doors.py`, `renderer.py`, `QuadPerimeterTests`, `batch_extract.py`, `path`, `renderer.py`, `filter_page_data`, `TestWindowTightPairInterior`, `_double_arc`, `TestWindowExteriorSide`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `migrate-labour-rates-to-groups.ts`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `TestFarSidePairs`, `_covers`, `TestNetworkQueries`, `squat_cap_window`, `TestMinWidthReference`, `TestMarkerRings`, `bezier_arc`, `detect_doors`, `vline`, `_find_openings`, `app.py`, `TestAnnotationLayerVeto`, `_collect_wall_faces`, `TestThickMaterialPairs`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`, `framed_triple_window`, `_segments_min_distance`?**
  _High betweenness centrality (0.144) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `pipeline.py` to `Pipeline Orchestration & Extraction`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Wall Network Construction & Tests`, `resolver.py`, `_fit_circle_3pt`, `TestCliEquivalence`, `plumber.py`, `ScaleInfo`, `PruneArcSpursTests`, `test_window_detection.py`, `TestXYCut`, `_double_arc`, `DoorAssemblyTests`, `TakeoffRequest`, `TestRenderPageSvg`, `detect_doors`, `attrib_rooms.py`, `TestAnnotationPenBarriers`, `test_door_assembly.py`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Double-Door Merge & Gemini Client` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `Window Detection & Tests`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `client.py`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `TestSwingHingePlugRestriction`, `_double_arc`, `renderer.py`, `QuadPerimeterTests`, `filter_page_data`, `pipeline.py`, `TestWindowTightPairInterior`, `_double_arc`, `TestWindowExteriorSide`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `TestWindowTightPairInterior`, `._spy_kwargs`, `TestSheetIsScaled`, `squat_cap_window`, `TestMarkerRings`, `detect_doors`, `vline`, `label_rooms`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`?**
