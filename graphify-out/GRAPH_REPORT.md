...
## Corpus Check
- 52 files · ~69,583 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1007 nodes · 2423 edges · 106 communities (50 shown, 56 thin omitted)
- Extraction: 95% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 109 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ac7e7471`
...
1. `PathPrimitive` - 124 edges
2. `Candidate` - 84 edges
3. `detect_windows()` - 50 edges
4. `detect_wall_network()` - 47 edges
5. `TextSpan` - 43 edges
6. `detect_doors()` - 41 edges
7. `DebugTraceCollector` - 38 edges
...
## Communities (106 total, 56 thin omitted)
...
Cohesion: 0.16
Nodes (19): _arc_corners(), _collect_door_swings(), _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _collect_linework_door_leaves(), _find_thin_rectangle_cycle(), BBox (+11 more)
...
Cohesion: 0.13
Nodes (23): detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), _building_masses(), detect_rooms(), _door_plugs(), _free_space_components(), Room detection: rooms are the connected free-space components between walls.  Ea (+15 more)

### Community 7 - "Debug Trace Collector"
Cohesion: 0.13
Nodes (34): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+26 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.16
Nodes (11): _native_curve_chains(), Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, ChainedCurveSwingDetectionTests, _circle_arc_chain(), _curve(), NativeCurveChainsTests, The door_0051 pattern: native curves with shared endpoints group         into a, End-to-end via detect_doors: a 5-curve chain forming a 30° arc of     a larger c (+3 more)
...
Cohesion: 0.05
Nodes (46): _bridge_white_runs(), _collect_fill_rings(), _collect_wall_faces(), detect_wall_network(), _equivalent_sides(), _fill_key(), _FillRing, _is_dashed() (+38 more)
...
Cohesion: 0.06
Nodes (46): _angle_diff_mod180(), _perpendicular_spacing(), _point_to_segment_distance(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., Smaller angular distance between two directions, both already mod 180°. (+38 more)
...
Cohesion: 0.16
Nodes (19): _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _prune_arc_cycle_caps(), _prune_arc_spurs(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Trim non-arc cap segments off a 2-leaf simple chain.      Some CAD draftsmen dra, Detect door-swing arcs approximated by connected short line segments.      Some (+11 more)
...
Cohesion: 0.17
Nodes (21): _is_arc_like(), _component_indices(), _dedupe_door_components(), _merge_double_door_assemblies(), Prefer the strongest door when two candidates use the same primitives., Merge pairs of adjacent single-door assemblies into double-swing candidates., _collect_door_leaves(), _is_door_leaf() (+13 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `windows.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Window Geometry Internals`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `Arc Spur-Pruning Tests`, `Candidate`, `hline`, `detect_windows`, `extractor.py`, `_resolve_door_window_conflicts`, `_FillRing`, `_collect_wall_faces`, `vline`, `_bridge_white_runs`, `TestWindowInteriorClutter`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.289) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Candidate` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Room Detection Tests`, `Room Polygonization Internals`, `windows.py`, `hline`, `detect_windows`, `plumber.py`, `_resolve_door_window_conflicts`, `_FillRing`, `renderer.py`, `vline`, `TestWindowInteriorClutter`, `TestWindowArbitraryAngle`, `_dedupe_openings`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `detect_windows()` connect `detect_windows` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `vline`, `TestWindowInteriorClutter`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `TestWindowArbitraryAngle`, `_dedupe_openings`, `Window Geometry Internals`, `Wall Layer Hints & Snapping`, `windows.py`, `Candidate`, `hline`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Are the 44 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_FillRing`) actually correct?**
  _`PathPrimitive` has 44 INFERRED edges - model-reasoned connections that need verification._
