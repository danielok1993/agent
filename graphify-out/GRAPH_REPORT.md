# Graph Report - agent  (2026-09-04)

## Corpus Check
- 260 files · ~469,349 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4641 nodes · 11880 edges · 267 communities (187 shown, 80 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 922 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f5682fc6`
...
- [[_COMMUNITY_viewport_scales|viewport_scales]]
...
- [[_COMMUNITY_text_scales|text_scales]]
- [[_COMMUNITY_app.py|app.py]]
...
- [[_COMMUNITY_extract_plumber_page|extract_plumber_page]]
- [[_COMMUNITY_TestFarSidePairs|TestFarSidePairs]]
- [[_COMMUNITY_FitCircle3PtTests|FitCircle3PtTests]]
...
- [[_COMMUNITY_TestScaleInfoDefaults|TestScaleInfoDefaults]]
...
1. `PathPrimitive` - 286 edges
2. `PageData` - 177 edges
3. `Candidate` - 177 edges
4. `TextSpan` - 154 edges
5. `detect_wall_network()` - 127 edges
6. `Region` - 116 edges
7. `PageScales` - 94 edges
8. `ScaleInfo` - 91 edges
9. `detect_windows()` - 78 edges
10. `rooms_for()` - 77 edges
...
Cohesion: 0.04
Nodes (62): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_center(), Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f, _stroke_is_visible(), _bbox_iou() (+54 more)
...
Cohesion: 0.08
Nodes (17): _contains(), _ratio_pair(), Which drawing scale a room is measured at, and whether it can be trusted.  Pages, (w_ratio, h_ratio) of page over ISO size, orientation-matched., select_room_scale(), sheet_size_tokens(), verify_sheet_size(), effective_denominator() (+9 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.15
Nodes (11): ChainedCurveSwingDetectionTests, _circle_arc_chain(), _curve(), NativeCurveChainsTests, _qu_leaf(), The door_0051 pattern: native curves with shared endpoints group         into a, End-to-end via detect_doors: a 5-curve chain forming a 30° arc of     a larger c, Build a native cubic Bezier (`c`) primitive from 4 control points. (+3 more)
...
Cohesion: 0.21
Nodes (5): The nearest standard scale within tolerance, or None.      None is a real answer, snap_to_standard(), The closed set a user may supply for a sheet nobody could scale.      Both prope, TestSnapToStandard, TestSuppliableScales
...
### Community 24 - "hline"
Cohesion: 0.17
Nodes (8): InvalidArgument, Unauthenticated, parse_request(), Parsing and validating one callable request.  The tenant is taken from the verif, The supplied scale, or None.      Only a member of SUPPLIABLE_SCALES is accepted, _scale_denominator(), TestParseRequest, TestParseGsUri
...
Cohesion: 0.17
Nodes (6): _layer_annotation_veto(), _layer_tokens(), True when the layer name marks its ink as annotation (callouts,     dimensions,, _wall_layer_hint(), Layer-name hints: CAD layer conventions pluralise the class name.  Measured on t, TestPluralLayerTokens
...
Cohesion: 0.05
Nodes (55): detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), _accept_jamb_rings(), _building_masses(), _contains_text() (+47 more)
...
Cohesion: 0.04
Nodes (132): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+124 more)
...
Cohesion: 0.16
Nodes (13): TruthItem, Regression corpus: fixture resolution, ground truth, matching, and the sweep., iou(), match_entities(), MatchResult, BBox, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, entity() (+5 more)

### Community 107 - "vline"
Cohesion: 0.07
Nodes (32): _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _check_opening_clear(), _line_nears_bridge_interior(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, True when some point of segment p1-p2 lies within buffer_px of the bridge     li, detect_doors() (+24 more)

### Community 108 - "_bridge_white_runs"
Cohesion: 0.10
Nodes (31): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, denom_at(), load(), mm(), W-gate census harness: run the stage-5 chain exactly as tools/regress.py does, w (+23 more)
...
Cohesion: 0.16
Nodes (7): attributes_by_room(), The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv, _door(), room_polys holds unscaled rooms too, so the first assigned room can         be t, Referential integrity must hold in BOTH directions: if the opening         names, _room(), TestComputeTakeoff

### Community 111 - "app.py"
Cohesion: 0.03
Nodes (129): _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), _project_onto_axis(), _projected_interval() (+121 more)
...
Cohesion: 0.06
Nodes (40): load_truth(), bind_scale(), binding_texts(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page.      `fallback` is a sc, Every text scale near enough to this region to be about it, nearest first., resolve_page_scales(), _cache_file() (+32 more)
...
Cohesion: 0.14
Nodes (15): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., _caption_distance(), _centroid(), _contains(), The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, How far a text span sits from a region, or None if it is not near it.      Horiz, _stored_info(), canonical_denominators() (+7 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.12
Nodes (14): _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, _restrict_swing_plugs(), _swing_hinge_edges(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, Single swing doors: plugs live on the hinge edges, one wall plane.      Geometry (+6 more)
...
Cohesion: 0.18
Nodes (7): Golden segmentation results on the corpus reference sheets (s01, s02, s11).  Mea, Load-bearing golden for SEGMENT_MAX_DEPTH = 7: at 6 the first-floor     plan and, s15 measured 2026-08-13: 214 text spans bridge every gutter, so the     text-inc, segment(), TestGoldenSegmentation, TestS15PathsOnlyRetry, TestS17PlanElevationSeparation
...
Cohesion: 0.12
Nodes (17): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), TestThreading, framed_triple_window(), horizontal_window(), quad(), Window detection tests.  Ground truth was established interactively on s01 (form (+9 more)
...
Cohesion: 0.09
Nodes (37): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., scale_table() (+29 more)
...
Cohesion: 0.13
Nodes (16): PageTruth, SheetTruth, evaluate_page(), _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists., score_sheet() (+8 more)

### Community 179 - "write_review_overlays"
Cohesion: 0.17
...
Cohesion: 0.23
Nodes (15): build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document(), inspect_pdf(), _page_type_styled(), print_candidates_tree() (+7 more)
...
Cohesion: 0.09
Nodes (32): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., compute_takeoff(), _largest_polygon(), OpeningTakeoff (+24 more)

### Community 188 - "pipeline.py"
Cohesion: 0.17
Nodes (16): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), _candidate_to_dict() (+8 more)
...
Cohesion: 0.13
Nodes (14): Evidence: what broke at f = 50/92.2 = 0.542 (all measured on the real PDF), Group 1 — safe reference moves, Group 2 — thin-margin moves (three of five tried and reverted), Group 3 — class fixes, Handoff: W-gate recalibration (the proper fix behind `fix/measured-scale-detection-factor`), How the ablation was done (reproduce in ~30 min), Outcome — iteration 2 (2026-09-04, branch `recal/w-gate-iter2`), Prompt for the next agent (fresh context) (+6 more)
...
Cohesion: 0.15
Nodes (9): can_prompt(), prompt_for_scale(), Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, True only when stdin is a real terminal., Ask once for one region's scale. Returns "1:100", or None if skipped.      Asked, FakeStream, The interactive scale prompt.  The prompt must never run in batch_extract (Proce, TestCanPrompt (+1 more)

### Community 206 - "DoorAssemblyTests"
Cohesion: 0.16
Nodes (19): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, Flip a manifest entry's `labeled` flag and write the manifest back.      `labele (+11 more)
...
Cohesion: 0.08
Nodes (23): latest_run(), Path, Where a sweep leaves its output.  Sweeps used to extract into a `tempfile.Tempor, Wipe and recreate this slug's output directory., The most recent run directory for this slug, or None.      Timestamp names sort, reset_slug_dir(), slug_dir(), _cache_missed() (+15 more)
...
### Community 215 - "unbound_scale_lines"
Cohesion: 0.28
...
### Community 218 - "_prune_arc_spurs"
Cohesion: 0.18
...
### Community 219 - "test_curve_arc_garden_doors.py"
Cohesion: 0.22
...
### Community 221 - "text_scales"
Cohesion: 0.28
Nodes (6): Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, Every scale printed on the page, each carrying its span's bbox., text_scales(), Reading a 1:N scale out of text spans.  Every string below is copied verbatim fr, span(), TestTextScales

### Community 222 - "app.py"
Cohesion: 0.31
...
Cohesion: 0.09
Nodes (27): CallableRequest, Exception, build_response(), error_code(), _measure(), measure_takeoff(), Firebase entry point for the takeoff extraction pipeline.  This module is the on, The handler's real body, with its clients injected so it is testable.      Extra (+19 more)

### Community 224 - "cluster_denominators"
Cohesion: 0.31
...
Cohesion: 0.10
Nodes (21): Constants for the takeoff callable.  Runtime sizing is justified in the design d, FailedPrecondition, NotFound, Domain errors, carrying the callable error code they map to.  This module delibe, _doc(), load_record(), mark_awaiting_review(), mark_awaiting_scale() (+13 more)

### Community 230 - "bezier_arc"
Cohesion: 0.42
Nodes (4): bbox_aspect(), bezier_arc(), BezierAspectGateTests, One cubic Bezier approximating a circular arc of the given sweep.      Standard

### Community 231 - "TestFaceCollection"
Cohesion: 0.22
...
### Community 234 - "extract_plumber_page"
Cohesion: 0.36
Nodes (5): extract_plumber_page(), _normalize_bbox_plumber(), BBox, extract_plumber_page must surface each table's bbox, normalized to     150-DPI p, TestPlumberTableBBox

### Community 236 - "FitCircle3PtTests"
Cohesion: 0.29
...
### Community 239 - "TestVectorTextExclusion"
Cohesion: 0.43
...
### Community 244 - "squat_cap_window"
Cohesion: 0.40
...
## Knowledge Gaps
- **617 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+612 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `TestMarkerRings` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Window Detection & Tests`, `Door Detection & Tests`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `Room Detection Tests`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `TestAnnotationPenBarriers`, `assistant.tsx`, `review.py`, `viewport_bbox_to_px`, `TestBlindWindowPocket`, `detect_windows`, `_double_arc`, `renderer.py`, `QuadPerimeterTests`, `batch_extract.py`, `path`, `TestWindowTightPairInterior`, `_chain`, `TestWindowExteriorSide`, `_arc`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `TestMinWidthNegativeControl`, `test_batch_extract.py`, `fill_ring`, `TestWindowGates`, `_prune_arc_spurs`, `test_curve_arc_garden_doors.py`, `TestThickMaterialPairs`, `_covers`, `TestMinWidthReference`, `wall_band_h`, `bezier_arc`, `TestFaceCollection`, `detect_doors`, `vline`, `_bridge_white_runs`, `FitCircle3PtTests`, `TestFarSidePairs`, `app.py`, `TestVectorTextExclusion`, `_find_openings`, `TestAnnotationLayerVeto`, `_collect_wall_faces`, `squat_cap_window`, `qualifying_clip_rects`, `TestNetworkQueries`, `TestWindowSpanOvershootRetune`, `batch_extract.py`, `framed_triple_window`, `_segments_min_distance`?**
  _High betweenness centrality (0.151) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `pipeline.py` to `Pipeline Orchestration & Extraction`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Wall Network Construction & Tests`, `TestCliEquivalence`, `plumber.py`, `ScaleInfo`, `_dedupe_openings`, `test_window_detection.py`, `TestXYCut`, `PruneUnreadPageOutputTests`, `app.py`, `TakeoffRequest`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `extract_plumber_page`, `._spy_kwargs`, `app.py`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.114) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `vline` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `Room Detection Tests`, `ShaMismatchAgainstTruthTests`, `client.py`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `_double_arc`, `ScaleInfo`, `renderer.py`, `QuadPerimeterTests`, `pipeline.py`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `text_scales`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `FitCircle3PtTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Are the 141 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 141 INFERRED edges - model-reasoned connections that need verification._
- **Are the 63 inferred relationships involving `PageData` (e.g. with `InkMap` and `PageRegionResult`) actually correct?**
  _`PageData` has 63 INFERRED edges - model-reasoned connections that need verification._