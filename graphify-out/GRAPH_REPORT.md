...
## Corpus Check
- 220 files · ~369,704 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3921 nodes · 10096 edges · 238 communities (162 shown, 76 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 737 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3dc28cfd`
...
- [[_COMMUNITY_postprocess.py|postprocess.py]]
- [[_COMMUNITY_app.py|app.py]]
- [[_COMMUNITY_line|line]]
- [[_COMMUNITY_test_sliding_doors.py|test_sliding_doors.py]]
...
- [[_COMMUNITY_can_prompt|can_prompt]]
...
1. `PathPrimitive` - 280 edges
2. `PageData` - 172 edges
3. `Candidate` - 167 edges
4. `TextSpan` - 150 edges
5. `Region` - 113 edges
...
## Communities (238 total, 76 thin omitted)

### Community 0 - "Pipeline Orchestration & Extraction"
Cohesion: 0.07
Nodes (50): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+42 more)
...
Cohesion: 0.07
Nodes (27): Barrier polygon sealing a window opening.      A horizontal/vertical window's bb, _window_seal(), Candidate, _door_attribute_overlay(), finalize_candidates(), Selected door-evidence keys to merge into Entity.attributes. {} for None / non-d, Promote candidates to entities, applying the offline confidence floors.      Gem, _room_entity() (+19 more)
...
Cohesion: 0.05
Nodes (40): _cross_validate(), CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, Drop window candidates that materially sit on a detected door.      Door symbols, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _resolve_door_window_conflicts(), _wall_runs_through() (+32 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.06
Nodes (50): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+42 more)

### Community 7 - "Debug Trace Collector"
Cohesion: 0.06
Nodes (51): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges() (+43 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.08
Nodes (51): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+43 more)
...
Cohesion: 0.13
Nodes (19): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., Heights, PageFrame, The space every coordinate in this document lives in.      extractor.page_transf, True when a scale resolved and quantities exist., RoomTakeoff, TakeoffPage (+11 more)
...
Cohesion: 0.04
Nodes (117): _angle_diff_mod180(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars. (+109 more)
...
Cohesion: 0.10
Nodes (10): door_candidate(), Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, rooms_for(), TestBboxSealFloor, TestClosedRooms, TestComponentFiltering, TestOpeningSeals (+2 more)
...
Cohesion: 0.11
Nodes (21): opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., compute_takeoff(), _largest_polygon(), OpeningTakeoff, Polygon (+13 more)
...
Cohesion: 0.13
Nodes (17): _collect_fill_rings(), _collect_wall_faces(), _fill_key(), _fill_seam_indices(), _fill_seams(), _FillRing, _is_dashed(), _rate_fill_classes() (+9 more)
...
Cohesion: 0.20
Nodes (10): The whole page as one document., One room: geometry, its opening ids, and its quantities.      `quantities` is No, room_dict(), to_document(), _door(), _page(), The takeoff.json document (takeoff/document.py)., _room() (+2 more)
...
Cohesion: 0.39
Nodes (3): The nearest standard scale within tolerance, or None.      None is a real answer, snap_to_standard(), TestSnapToStandard
...
Cohesion: 0.23
Nodes (6): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DegenerateCompanionTests, DoorAssemblyTests, path(), A zero-length stroke is never a leaf edge (s11's double doors).      CAD exports
...
Cohesion: 0.10
Nodes (20): PageTruth, SheetTruth, evaluate_page(), _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists., score_sheet() (+12 more)
...
Cohesion: 0.10
Nodes (34): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document() (+26 more)
...
Cohesion: 0.13
Nodes (13): detect_wall_network(), _fill_ring_components(), _is_light_pen(), Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), path() (+5 more)
...
### Community 203 - "collect_room_spans"
Cohesion: 0.38
...
Cohesion: 0.24
Nodes (5): parse_answer(), Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, The denominator in an answer, accepting "1:100" or "100". None to skip., The interactive scale prompt.  The prompt must never run in batch_extract (Proce, TestParseAnswer

### Community 206 - "DoorAssemblyTests"
Cohesion: 0.08
Nodes (47): manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, Flip a manifest entry's `labeled` flag and write the manifest back.      `labele, set_labeled(), sha256_of() (+39 more)
...
### Community 211 - "batch_extract.py"
Cohesion: 0.27
...
### Community 216 - "line"
Cohesion: 0.32
Nodes (4): line(), quarter_arc_lines(), Swing-anchored single-line leaf check (v3).      A door panel is often drawn as, SingleLineLeafTests

### Community 217 - "postprocess.py"
Cohesion: 0.26
Nodes (11): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_iou(), _projected_overlap_1d(), BBox, Projected overlap fraction and perpendicular gap on bbox a's dominant axis. (+3 more)

### Community 218 - "app.py"
Cohesion: 0.31
...
Cohesion: 0.25
Nodes (7): PolylineArcBinCapTests, 270-degree polyline arc with 16 segments — far wider than a quarter-circle door, Compute 15-degree angle bins for line segments — fixture sanity helper., Tests for the DOOR_POLYLINE_MAX_ANGLE_BINS cap that rejects furniture/appliance, rect_lines(), _seg_angle_bins(), wide_arc_lines()

### Community 221 - "label_rooms"
Cohesion: 0.33
...
### Community 223 - "PruneUnreadPageOutputTests"
Cohesion: 0.24
...
### Community 234 - "TestWindowTightPairInterior"
Cohesion: 0.33
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Arc Detection Primitives` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `test_layout_segmenter.py`, `Room Detection Tests`, `Wall Network Construction & Tests`, `TestWindowArbitraryAngle`, `Double-Arc Split Tests`, `ShaMismatchAgainstTruthTests`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cap-Trim Tests`, `test_extraction_transform.py`, `arcs.py`, `Arc Cycle-Cap Pruning Tests`, `resolver.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `_double_arc`, `test_extraction_transform.py`, `_projected_interval`, `renderer.py`, `TestSwingHingePlugRestriction`, `TestNetworkQueries`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestXYCut`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `line`, `test_sliding_doors.py`, `PruneUnreadPageOutputTests`, `wall_band_h`, `EntranceDoorTests`, `TestMarkerRings`, `TestVectorTextExclusion`, `detect_doors`, `squat_cap_window`, `DoorV2OpeningCheckTests`, `vline`, `_bridge_white_runs`, `_find_openings`, `TestWindowSpanOvershootRetune`, `app.py`, `TestWindowTightPairInterior`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Detection & Tests` to `Pipeline Orchestration & Extraction`, `Window Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `DoorAssemblyTests`, `TestWindowArbitraryAngle`, `ShaMismatchAgainstTruthTests`, `Arc Cycle-Cap Pruning Tests`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `test_extraction_transform.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `test_window_detection.py`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `test_sliding_doors.py`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `TestExtractPageFrame`, `test_batch_extract.py`, `line`, `postprocess.py`, `test_sliding_doors.py`, `TestMinWidthNegativeControl`, `wall_band_h`, `EntranceDoorTests`, `DoorV2OpeningCheckTests`, `squat_cap_window`, `TestWindowTightPairInterior`, `vline`, `_bridge_white_runs`, `_find_openings`, `TestWindowSpanOvershootRetune`, `app.py`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Debug Trace Collector` to `EntranceDoorTests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `_check_opening_clear`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `test_extraction_transform.py`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `test_window_detection.py`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `SplitDoubleArcTests`, `collect_room_spans`, `TestWindowExteriorSide`, `TestExtractPageFrame`, `test_batch_extract.py`, `line`, `test_sliding_doors.py`, `label_rooms`, `PruneUnreadPageOutputTests`, `wall_band_h`, `EntranceDoorTests`, `TestMarkerRings`, `detect_doors`, `vline`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Are the 135 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 135 INFERRED edges - model-reasoned connections that need verification._
- **Are the 60 inferred relationships involving `PageData` (e.g. with `InkMap` and `PageRegionResult`) actually correct?**
  _`PageData` has 60 INFERRED edges - model-reasoned connections that need verification._