...
## Corpus Check
- 220 files · ~359,628 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3837 nodes · 9827 edges · 227 communities (154 shown, 73 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 717 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `80f47f0a`
...
- [[_COMMUNITY_app.py|app.py]]
- [[_COMMUNITY_TestSpanFilterIsLoadBearing|TestSpanFilterIsLoadBearing]]
- [[_COMMUNITY__spanning_glazing|_spanning_glazing]]
- [[_COMMUNITY__is_light_pen|_is_light_pen]]
...
1. `PathPrimitive` - 267 edges
2. `Candidate` - 166 edges
3. `PageData` - 164 edges
4. `TextSpan` - 145 edges
5. `Region` - 111 edges
6. `detect_wall_network()` - 100 edges
7. `PageScales` - 90 edges
8. `ScaleInfo` - 86 edges
9. `Entity` - 70 edges
10. `detect_windows()` - 69 edges
...
Cohesion: 0.15
Nodes (13): _area(), _dedupe_openings(), detect_windows(), _frame_axes(), _merge_mullion_chains(), BBox, Unit run-axis u (perpendicular to the caps) and perp-axis v (along caps).      C, Join collinear glazing segments across mullion blocks into logical panes.      A (+5 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.08
Nodes (18): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox short-end edges of a sliding door: across the wall, never wall plane., Thin barrier bands along the wall planes through a detected door.      The door, _sliding_end_edges(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, Interrupted-run plugs need jambs that REACH the plug band and a mid     that is (+10 more)
...
Cohesion: 0.08
Nodes (75): _arc_corners(), _collect_door_swings(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _component_indices() (+67 more)
...
Cohesion: 0.07
Nodes (22): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, True when a wall FACE line runs unbroken through the bbox span.      A real wind, _wall_runs_through(), prim(), _production_cross_gates_unscaled_usages(), _production_door_gates_unscaled_usages(), quarter_bezier() (+14 more)
...
Cohesion: 0.10
Nodes (23): PageRegionResult, Segment the page, classify its regions, and decide what detection sees.      cla, resolve_page_regions(), block(), one_blob_page(), page_with_a_dropped_strip(), parse_failing_classifier(), raster_page() (+15 more)
...
### Community 24 - "hline"
Cohesion: 0.10
Nodes (30): _detect_curve_arc_double_partners(), _is_arc_like(), Pair single-Bezier `curve_arc` swings that form a garden-door split.      The po, _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower, Closed 4-8 segment loops among the given `l` segments, as oriented rects. (+22 more)
...
Cohesion: 0.22
Nodes (10): _centre_in_any(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, Text spans inside the given regions. Used to scope schedule detection to     sch, region_text_spans(), path(), Region filtering tests (layout/filter.py)., region() (+2 more)
...
Cohesion: 0.05
Nodes (50): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv (+42 more)
...
Cohesion: 0.06
Nodes (48): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage() (+40 more)
...
Cohesion: 0.05
Nodes (90): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin. (+82 more)
...
Cohesion: 0.20
Nodes (9): Minimum distance between two line segments., _segments_min_distance(), BBox, True when the two segments cross at an interior point.      _segments_min_distan, Min distance between a segment and an axis-aligned bbox (0 if touching)., True when any centerline corridor (dilated by thickness/2 + expand) hits bbox., Max fraction of the bbox long axis covered by one near-collinear centerline., _segment_bbox_distance() (+1 more)

### Community 115 - "_collect_wall_faces"
Cohesion: 0.09
Nodes (22): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), _candidate_to_dict() (+14 more)
...
Cohesion: 0.11
Nodes (15): Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Barrier polygon sealing a window opening.      A horizontal/vertical window's bb, _window_seal(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode., TestBlindWindowPocket (+7 more)
...
Cohesion: 0.12
Nodes (15): bind_scale(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., resolve_page_scales(), The resolution ladder and how a scale binds to a floor plan.  Region binding is, A suspend_display factory that logs enter/exit, for the tests below., The regression: a live rich Progress torn down on a page that         resolved i, The renumbering hazard, end to end.          A scale stored against what used to (+7 more)

### Community 131 - "test_layout_segmenter.py"
Cohesion: 0.13
Nodes (14): assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, _attach_text_spans(), Grow paths-only boxes to absorb the text spans beside them.      The tier-2 cut, Split a page into drawing regions. Returns [] for a page with no vector     ink, segment_page(), PageData, block() (+6 more)
...
Cohesion: 0.17
Nodes (6): _contains(), select_room_scale(), effective_denominator(), Nominal beats raw so 1:50 sheets compute exactly (scale/factor.py rule)., TestSelectRoomScale, TestEffectiveDenominator
...
Cohesion: 0.19
Nodes (20): Page segmentation: split a sheet into its constituent drawings., InkMap, bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _col_profile(), count_paths_in(), _edge_gap_sq() (+12 more)
...
Cohesion: 0.07
Nodes (40): build_parser(), cmd_extract(), cmd_inspect(), main(), parse_page_spec(), positive_metres(), argparse type: a positive, finite height in metres., Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]. (+32 more)
...
Cohesion: 0.12
Nodes (21): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), detection_scale(), PageScales, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict(), info() (+13 more)
...
Cohesion: 0.11
Nodes (22): PageTruth, SheetTruth, TruthItem, evaluate_page(), _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists. (+14 more)
...
Cohesion: 0.14
Nodes (17): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), diagonal_window(), framed_triple_window(), path(), quad(), Window detection tests.  Ground truth was established interactively on s01 (form (+9 more)
...
Cohesion: 0.08
Nodes (41): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+33 more)
...
### Community 213 - "_prune_arc_spurs"
Cohesion: 0.18
...
Cohesion: 0.21
Nodes (6): A chimney breast / pier drawn as a closed box on the room side of a     wall ban, Closed stroked (fill-less) polyline exploded into chained `l` items., s03 corridor room_0014: the jamb nibs beside door_0007/door_0019 are     closed, stroked_ring_path(), TestJambNibRings, TestWallRecess

### Community 215 - "app.py"
Cohesion: 0.35
...
### Community 217 - "_spanning_glazing"
Cohesion: 0.33
...
### Community 220 - "viewport_bbox_to_px"
Cohesion: 0.40
...
Cohesion: 0.33
Nodes (4): The tight-pair interior gate (WINDOW_TIGHT_PAIR_GAP_PX /     WINDOW_TIGHT_PAIR_J, floor-plans true windows draw a narrow double glazing line (panes         1.75px, 5-1133 window_0022 (real diagonal 2-pane window): its band sits at         the c, TestWindowTightPairInterior

### Community 223 - "TestFoldingScaleBehavior"
Cohesion: 0.38
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `hline` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `DoorAssemblyTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `review.py`, `TestBlindWindowPocket`, `detect_windows`, `plumber.py`, `_projected_interval`, `_double_arc`, `renderer.py`, `TestWindowArbitraryAngle`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_prune_arc_spurs`, `test_batch_extract.py`, `_spanning_glazing`, `_is_light_pen`, `viewport_bbox_to_px`, `ParkedLeafTests`, `PruneUnreadPageOutputTests`, `TestFoldingScaleBehavior`, `TestFarSidePairs`, `wall_band_h`, `TestThickMaterialPairs`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `_find_openings`, `EntranceDoorTests`, `app.py`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `Candidate` connect `qualifying_clip_rects` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `TestExtractImagesInstances`, `Arc Detection Primitives`, `TestWindowArbitraryAngle`, `Wall Network Construction & Tests`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `hline`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `_is_light_pen`, `viewport_bbox_to_px`, `ParkedLeafTests`, `PruneUnreadPageOutputTests`, `TestMinWidthNegativeControl`, `wall_band_h`, `DoorV2OpeningCheckTests`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `wall_band_h` to `EntranceDoorTests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Wall Network Construction & Tests`, `_check_opening_clear`, `arcs.py`, `RunDirTests`, `resolver.py`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `plumber.py`, `_projected_interval`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `TestWindowTightPairInterior`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `_scan_striped_runs`, `TestWindowExteriorSide`, `test_batch_extract.py`, `app.py`, `_spanning_glazing`, `ParkedLeafTests`, `detect_doors`, `EntranceDoorTests`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `batch_extract.py`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Are the 129 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 129 INFERRED edges - model-reasoned connections that need verification._
- **Are the 83 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `CrossGates`) actually correct?**
  _`Candidate` has 83 INFERRED edges - model-reasoned connections that need verification._