...
## Corpus Check
- 220 files · ~367,883 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3899 nodes · 10037 edges · 225 communities (155 shown, 70 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 733 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4c318d77`
...
1. `PathPrimitive` - 278 edges
2. `PageData` - 171 edges
3. `Candidate` - 167 edges
4. `TextSpan` - 149 edges
5. `Region` - 112 edges
...
## Communities (225 total, 70 thin omitted)

### Community 0 - "Pipeline Orchestration & Extraction"
Cohesion: 0.17
Nodes (26): _load_font(), baseline_dir(), baseline_run(), classify(), compare(), compare_runs(), _crop(), _draw_entities() (+18 more)
...
Cohesion: 0.06
Nodes (40): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), Drop window candidates that materially sit on a detected door.      Door symbols (+32 more)
...
Cohesion: 0.08
Nodes (30): _cross_validate(), True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), One merged wall-face run with the evidence its members carried., Connected wall-centerline network (internal-only, never serialized)., Path indices of every face that contributed to a centerline., Length-weighted median stroke width of the paired stroked faces.          Anchor (+22 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.08
Nodes (59): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., Pair single-Bezier `curve_arc` swings that form a garden-door split.      The po, _component_indices() (+51 more)

### Community 7 - "Debug Trace Collector"
Cohesion: 0.08
Nodes (61): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+53 more)
...
Cohesion: 0.13
Nodes (15): diff_entities(), (kept, removed, added): `before` entities paired to `after` entities by     type, PageTruth, evaluate_page(), Score one page's entities against its three verdict lists., ClassifyTests, CompareRunsTests, DiffEntitiesTests (+7 more)
...
### Community 24 - "hline"
Cohesion: 0.08
Nodes (34): _fit_circle_3pt(), _is_arc_like(), _native_curve_chains(), Fit a circle through 3 points. Returns (cx, cy, radius) or None if     the point, Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a (+26 more)
...
Cohesion: 0.15
Nodes (12): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, paving_field(), Running-bond paving: continuous course lines, staggered joint lines.      Mirror, Striped fields (paving bonds, tile fields, treads) are not walls., Four wall bands forming a closed rectangular room (outer faces at the     given, Stroke-color pen identity: pairing, faint-ink demotion, dimension     chains, an, rect_room() (+4 more)
...
Cohesion: 0.07
Nodes (37): _accept_jamb_rings(), _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_door_lining() (+29 more)
...
Cohesion: 0.11
Nodes (17): _facing_cap_pairs(), _find_openings(), _glaze_index(), World-space window gates, pre-multiplied by the detection factor.      Exactly O, Two-axis lookup structure over a frame's glazing pool.      Every cap pair asks, Index pairs ``(i, j, width)`` of caps that face each other across an opening., Pair facing caps and confirm a glazing band bridges each opening.      ``cap_poo, WindowGates (+9 more)
...
Cohesion: 0.04
Nodes (108): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval(), Minimum distance between two line segments., Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars. (+100 more)
...
Cohesion: 0.10
Nodes (16): TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., Interrupted-run plugs need jambs that REACH the plug band and a mid     that is, A filled wall band exported as two triangles (CAD fill triangulation).      Each, A chimney breast / pier drawn as a closed box on the room side of a     wall ban, TestBlindWindowPocket, TestDiagonalWindowSeal (+8 more)
...
Cohesion: 0.23
Nodes (5): Schedule detection — tables carry real bboxes.  detect_schedules used to emit bb, extract_plumber_page must surface each table's bbox, normalized to     150-DPI p, _table(), TestDetectSchedulesBBox, TestPlumberTableBBox
...
Cohesion: 0.09
Nodes (20): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), BBox, Image (+12 more)
...
Cohesion: 0.11
Nodes (28): InkMap, bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _chains_across(), _clip_cut(), _col_profile(), count_paths_in() (+20 more)
...
Cohesion: 0.11
Nodes (26): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., scale_table() (+18 more)
...
Cohesion: 0.15
Nodes (11): dimension_matches(), Every ticked dimension line with a numeric label beside it., _dim_chain(), _line(), A single swing hinged at (x, y) on a room's top wall, radius r px., A ticked dimension line from x0 to x1 with its label centred above., _room(), _span() (+3 more)
...
Cohesion: 0.09
Nodes (38): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document() (+30 more)
...
Cohesion: 0.22
Nodes (6): cut(), page(), Recursive XY-cut tests (layout/segmenter.py)., Tier 3: a band that only SHORT annotation ink crosses is still a gutter.      Le, TestShortInkGutter, TestXYCut
...
Cohesion: 0.05
Nodes (52): _detect_polyline_arc_bboxes(), _prune_arc_cycle_caps(), _prune_arc_spurs(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Trim non-arc cap segments off a 2-leaf simple chain.      Some CAD draftsmen dra, Detect door-swing arcs approximated by connected short line segments.      Some, Remove short leaf-spurs (door stops, cap lines) from an arc component.      A cl (+44 more)
...
Cohesion: 0.27
Nodes (3): The synthetic PDF is 595x842 pt; at 150 DPI that is 1239.6x1754.2 px.         A, --disable-rooms means no rooms, so no takeoff and nothing to prompt for;     the, TestHeightPromptGating
...
Cohesion: 0.33
Nodes (4): The tight-pair interior gate (WINDOW_TIGHT_PAIR_GAP_PX /     WINDOW_TIGHT_PAIR_J, 5-1133 window_0016/0017: a step in a solid-filled wall block — the         step', 5-1133 window_0022 (real diagonal 2-pane window): its band sits at         the c, TestWindowTightPairInterior
...
### Community 225 - "TestMinWidthNegativeControl"
Cohesion: 0.32
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `hline` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `DoorAssemblyTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `ShaMismatchAgainstTruthTests`, `resolver.py`, `TestAnnotationPenBarriers`, `TestBlindWindowPocket`, `detect_windows`, `plumber.py`, `_projected_interval`, `_double_arc`, `renderer.py`, `TestSwingHingePlugRestriction`, `TestNetworkQueries`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `write_review_overlays`, `line`, `test_sliding_doors.py`, `PruneUnreadPageOutputTests`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_bridge_white_runs`, `_find_openings`, `app.py`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.165) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Detection & Tests` to `Window Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `TestExtractImagesInstances`, `ShaMismatchAgainstTruthTests`, `Arc Cycle-Cap Pruning Tests`, `TestAnnotationPenBarriers`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `test_window_detection.py`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `test_sliding_doors.py`, `TestWindowExteriorSide`, `Candidate`, `test_batch_extract.py`, `write_review_overlays`, `TestMinWidthNegativeControl`, `wall_band_h`, `DoorV2OpeningCheckTests`, `vline`, `_bridge_white_runs`, `_find_openings`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `qualifying_clip_rects` to `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `DoorAssemblyTests`, `Wall Network Construction & Tests`, `test_extraction_transform.py`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `test_layout_segmenter.py`, `detect_windows`, `plumber.py`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `_scan_striped_runs`, `TestWindowExteriorSide`, `test_batch_extract.py`, `PruneUnreadPageOutputTests`, `wall_band_h`, `detect_doors`, `vline`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Are the 134 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 134 INFERRED edges - model-reasoned connections that need verification._
- **Are the 59 inferred relationships involving `PageData` (e.g. with `InkMap` and `PageRegionResult`) actually correct?**
  _`PageData` has 59 INFERRED edges - model-reasoned connections that need verification._