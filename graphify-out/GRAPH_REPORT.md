# Graph Report - agent  (2026-08-20)

## Corpus Check
- 214 files · ~340,083 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3693 nodes · 9429 edges · 200 communities (135 shown, 65 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 690 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fbe968de`
...
1. `PathPrimitive` - 251 edges
2. `PageData` - 164 edges
3. `Candidate` - 158 edges
4. `TextSpan` - 138 edges
5. `Region` - 111 edges
6. `PageScales` - 90 edges
7. `ScaleInfo` - 86 edges
8. `detect_wall_network()` - 84 edges
9. `Entity` - 70 edges
...
Cohesion: 0.06
Nodes (41): door_open_leaf_path_indices(), _leaf_ink_indices(), A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics() (+33 more)
...
Cohesion: 0.05
Nodes (108): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+100 more)
...
Cohesion: 0.09
Nodes (23): _merge_double_door_assemblies(), Parse an evidence bbox value defensively; return None on any invalid shape., Merge pairs of adjacent single-door assemblies into double-swing candidates., _safe_bbox(), DoubleDoorTests, OpenLeafExclusionTests, PolylineArcBinCapTests, 270-degree polyline arc with 16 segments — far wider than a quarter-circle door (+15 more)
...
Cohesion: 0.16
Nodes (10): dimension_matches(), Every ticked dimension line with a numeric label beside it., _dim_chain(), _line(), A single swing hinged at (x, y) on a room's top wall, radius r px., A ticked dimension line from x0 to x1 with its label centred above., _span(), _swing() (+2 more)
...
Cohesion: 0.14
Nodes (14): detect(), LeafPairTests, OrientedRectFitTests, ParkedLeafTests, PocketLeafTests, prim(), qu_panel(), A panel pocketed at its -axis end, protruding at the +axis end. (+6 more)

### Community 15 - "Arc Cap-Trim Tests"
Cohesion: 0.11
Nodes (20): opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One door or window. `room_ids` is empty when it reached no room;     `dropped_ro, compute_takeoff(), _largest_polygon(), OpeningTakeoff, Polygon, compute_takeoff — the pure core: rooms + scale + heights → metres.  No I/O, no p (+12 more)
...
Cohesion: 0.07
Nodes (21): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, canonical_denominators(), cluster_denominators(), denominator_from_c(), format_scale(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta (+13 more)
...
### Community 24 - "hline"
Cohesion: 0.05
Nodes (40): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_center(), _bbox_iou(), CrossGates, _projected_overlap_1d() (+32 more)
...
Cohesion: 0.13
Nodes (14): build_ink_map(), is_page_spanning(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng, Golden segmentation results on the corpus reference sheets (s01, s02, s11).  Mea, This sheet carries full-page border rules. With the span filter applied     the, TestSpanFilterIsLoadBearing, page() (+6 more)
...
Cohesion: 0.11
Nodes (24): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), Heights, Wall / opening heights — the one input the plan cannot supply.  0/20 corpus shee, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., PageFrame (+16 more)
...
Cohesion: 0.12
Nodes (29): _apply(), _as_transform(), classify_page(), _color_tuple(), extract_document(), extract_images(), extract_page(), extract_paths() (+21 more)
...
Cohesion: 0.09
Nodes (24): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, render_page_png(), _candidate_to_dict(), collect_warnings(), _entity_to_dict() (+16 more)

### Community 109 - "_find_openings"
Cohesion: 0.12
Nodes (19): _dedupe_by_perp(), _facing_cap_pairs(), _glaze_index(), Collapse near-collinear duplicates (same perp offset) to one record.      A toil, Largest run of panes spaced like glazing, not like stair treads.      Walks the, Two-axis lookup structure over a frame's glazing pool.      Every cap pair asks, Distinct parallel glazing lines that connect cap ``c1`` to cap ``c2``.      A gl, Index pairs ``(i, j, width)`` of caps that face each other across an opening. (+11 more)
...
Cohesion: 0.04
Nodes (113): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bbox_expanded(), _bbox_union(), _bboxes_overlap(), _interval_overlap(), _line_angle_deg() (+105 more)
...
Cohesion: 0.21
Nodes (10): One room: geometry, its opening ids, and its quantities.      `quantities` is No, The whole page as one document., room_dict(), to_document(), _door(), _page(), The takeoff.json document (takeoff/document.py)., _room() (+2 more)
...
Cohesion: 0.07
Nodes (33): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., How far a text span sits from a region, or None if it is not near it.      Horiz (+25 more)
...
Cohesion: 0.09
Nodes (34): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, detection_scale(), DetectionScale (+26 more)
...
Cohesion: 0.09
Nodes (19): assess_scale(), check_dimensions(), check_door_leaves(), DimensionMatch, _fmt_scale(), leaf_width_px(), parse_dimension_mm(), _positive() (+11 more)
...
Cohesion: 0.17
Nodes (9): _find_openings(), World-space window gates, pre-multiplied by the detection factor.      Exactly O, Pair facing caps and confirm a glazing band bridges each opening.      ``cap_poo, WindowGates, Scale-aware window gates: WindowGates, threading, and the frozen classification', CROSS_WINDOW_THICKNESS_TOL_PX froze P (spec Evidence 5): the mismatch     it tol, TestCrossWindowToleranceUnscaled, TestThreading (+1 more)
...
## Knowledge Gaps
- **521 isolated node(s):** `storage`, `sheets`, `Project purpose`, `Algorithm reference`, `Commands` (+516 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Debug Trace Collector` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `RunDirTests`, `count_paths_in`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `_segments_min_distance`, `detect_windows`, `_double_arc`, `_projected_interval`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `DoorV2OpeningCheckTests`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `TestBlindWindowPocket`, `_FillRing`, `swing_door`, `TestXYCut`, `TestThickMaterialPairs`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.172) - this node is a cross-community bridge._
- **Why does `Candidate` connect `hline` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Double-Arc Split Tests`, `Arc Cycle-Cap Pruning Tests`, `count_paths_in`, `TestAnnotationPenBarriers`, `_segments_min_distance`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `renderer.py`, `_dedupe_openings`, `_FillRing`, `swing_door`, `TestPlumberTableBBox`, `TestMinWidthNegativeControl`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_bridge_white_runs`, `_find_openings`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Door Detection & Tests` to `EntranceDoorTests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `test_extraction_transform.py`, `count_paths_in`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `swing_door`, `TestXYCut`, `detect_doors`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `framed_triple_window`, `_segments_min_distance`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Are the 118 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 118 INFERRED edges - model-reasoned connections that need verification._
