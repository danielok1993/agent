...
## Corpus Check
- 298 files · ~725,053 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4949 nodes · 12436 edges · 280 communities (202 shown, 78 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 959 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c6f53e88`
...
- [[_COMMUNITY_TestSpanFilterIsLoadBearing|TestSpanFilterIsLoadBearing]]
- [[_COMMUNITY_jamb_seek_census.py|jamb_seek_census.py]]
- [[_COMMUNITY_TestThickMaterialPairs|TestThickMaterialPairs]]
- [[_COMMUNITY_.wall_stroke_reference|.wall_stroke_reference]]
...
Cohesion: 0.07
Nodes (25): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+17 more)
...
Cohesion: 0.11
Nodes (13): _contains(), is_verified(), _ratio_pair(), Which drawing scale a room is measured at, and whether it can be trusted.  Pages, Source-level trust, then the drawing's own evidence: a failed     plausibility c, (w_ratio, h_ratio) of page over ISO size, orientation-matched., select_room_scale(), sheet_size_tokens() (+5 more)
...
Cohesion: 0.11
Nodes (15): detect_wall_network(), _fill_ring_components(), _is_light_pen(), Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., paving_field(), Running-bond paving: continuous course lines, staggered joint lines.      Mirror (+7 more)
...
Cohesion: 0.08
Nodes (49): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+41 more)

### Community 22 - "geometry.py"
Cohesion: 0.14
Nodes (14): build_request_text(), is_grounded(), is_noise_span(), Polygon, Ask Gemini for the name written inside each detected room.  One text-only call p, The one user part: every room's spans as JSON, keyed by ordinal., True when every word of the label appears in that room's own spans.      This ma, True for text that can never be a room name: dimension strings, door     and win (+6 more)
...
Cohesion: 0.08
Nodes (16): attrib.py <slug> <x0> <y0> <x1> <y1> FIELD=MULT [FIELD=MULT ...]  Runs the harne, attrib_rooms.py <slug> FIELD=MULT [...]: for each config (each alone on top of t, COLLINEAR_OFFSET_TOL forms, measured with the census harness on top of the curre, crop_dash.py <slug> <name> X0 Y0 X1 Y1 [...]: crop the latest sweep render with, denom_at(), load(), mm(), W-gate census harness: run the stage-5 chain exactly as tools/regress.py does, w (+8 more)
...
Cohesion: 0.11
Nodes (25): _collect_material_marks(), _collect_stroked_rect_weak_faces(), _collect_stroked_rings(), _collect_weak_faces(), _dash_row_indices(), _is_dashed(), _pen_key(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0". (+17 more)
...
Cohesion: 0.19
Nodes (17): Every drawing scale this page's viewports state, smallest bbox first.      Small, viewport_scales(), DetectionPage, load_detection_pages(), One corpus sheet's detection page data, exactly as tools/regress.py sees it.  Sh, Every detected page of the sheet (or only `pages`, 1-based)., sheet_pdf(), dump() (+9 more)
...
Cohesion: 0.07
Nodes (31): _bbox_union(), Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f, _stroke_is_visible(), _layer_annotation_veto(), _layer_classes(), _layer_hint(), _layer_hint_from_layer(), _layer_strong_prior() (+23 more)
...
Cohesion: 0.07
Nodes (76): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+68 more)

### Community 102 - "DoorV2OpeningCheckTests"
Cohesion: 0.12
Nodes (8): hline(), path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, The material-mark length cap is per band (_mark_len_cap): the     page-wide WALL, TestCenterlines, TestPerBandMarkCap, TestWeakFacePairs, weak_hatched_band_h()

### Community 103 - "PathPrimitive"
Cohesion: 0.12
Nodes (24): Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sha256_of(), sheet_entry(), sheet_path(), slug_for_path() (+16 more)
...
Cohesion: 0.09
Nodes (18): TruthItem, iou(), match_entities(), MatchResult, BBox, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, entity(), IouTests (+10 more)
...
Cohesion: 0.17
Nodes (11): One fixture per paper-space family (spec §Testing). Each fails if its     named, TestPaperInvariance, hline(), Three parallel lines spaced far apart (e.g. stair treads) exceed the         gla, A toilet/sink fixture is a hatch of stacked short segments plus         collinea, The tight-pair interior gate (WINDOW_TIGHT_PAIR_GAP_PX /     WINDOW_TIGHT_PAIR_J, 5-1133 window_0020: the "recess" niche — a drawn rectangle whose         long si, 5-1133 window_0016/0017: a step in a solid-filled wall block — the         step' (+3 more)
...
Cohesion: 0.03
Nodes (121): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bbox_expanded(), _interval_overlap(), _line_angle_deg() (+113 more)
...
Cohesion: 0.09
Nodes (14): _clip_plug_tails(), _door_plugs(), How far along (ux, uy) from corner a tail sample first touches wall material., Thin barrier bands along the wall planes through a detected door.      The door, End each bbox-edge plug's tails AT the material they touch.      _door_plugs tri, _seek_jamb(), Interrupted-run plugs need jambs that REACH the plug band and a mid     that is, A doorway plug's tail SEEKS its jamb when the fixed SEAL reach finds     none th (+6 more)
...
Cohesion: 0.12
Nodes (16): _glaze_index(), Two-axis lookup structure over a frame's glazing pool.      Every cap pair asks, Scale-aware window gates: WindowGates, threading, and the frozen classification', TestThreading, framed_triple_window(), quad(), Window detection tests.  Ground truth was established interactively on s01 (form, 5-1133 W8: a three-light frame tagged with a single label. Two full-span     rai (+8 more)
...
Cohesion: 0.08
Nodes (26): _collect_fill_rings(), _collect_wall_faces(), _fill_seam_indices(), _FillRing, _rate_fill_classes(), Classify each fill color as wall material (True) or furniture (False).      Vect, Background-colored rings that could be hollow walls or built-in runs.      White, Path indices of fill-ring seams — see _fill_seams. (+18 more)
...
Cohesion: 0.12
Nodes (11): True when ``win`` stands beyond ``door``'s hinge-side jamb in the door's     own, _window_in_door_wall_run(), Barrier polygon sealing a window opening.      A horizontal/vertical window's bb, _window_seal(), Candidate, DoorEvidencePropagationTests, Verify Step 4 — door evidence keys land in Entity.attributes in offline mode., TestDiagonalWindowSeal (+3 more)
...
Cohesion: 0.11
Nodes (24): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, detection_scale(), _fallback_info(), PageScales, A scale the user supplied for the whole run, as a ladder entry.      `source="us, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict() (+16 more)
...
Cohesion: 0.09
Nodes (23): probe_box.py <slug> <seal> x0 y0 x1 y1 [door_id ...]  Run the harness at the giv, tap_fsc(), tap_gap(), draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1., The two step-9 pictures (guarded so crop_step10.py can import the drawing helper (+15 more)
...
Cohesion: 0.12
Nodes (27): Exception, Base class. `code` is a Firebase callable error code string., TakeoffFnError, FakeBucket, FakeDb, _make_extract(), A normally measured page: one scale, read off the sheet., A page the resolver could not read a scale for.      Rooms survive with their ge (+19 more)
...
Cohesion: 0.17
Nodes (6): Stick-font text drawn as line strokes (s06/s11/s16/s20: no text     spans, every, HITL' in 14px stick glyphs, cap line y, baseline y + 14., A DRAWN DASH LINE — one line its line type exploded into a periodic     row of s, TestDashRowExclusion, TestVectorTextExclusion, vline()
...
Cohesion: 0.08
Nodes (39): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv (+31 more)
...
Cohesion: 0.08
Nodes (17): parse_measure_viewports(), Tier 1 — the scale the PDF states in its own viewport measure dictionaries.  A C, Split a PDF array string into its top-level ``<< >>`` dictionaries.      Depth-c, Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, split_pdf_dicts(), _FakeDoc, _FakePage, _Matrix (+9 more)
...
Cohesion: 0.18
Nodes (8): _plane_stamp(), Bbox short-end edges of a sliding door: across the wall, never wall plane., The plug-less fallback seal, restricted to the door's wall-plane edges.      A d, How far, along (ux, uy) from corner, the material a plug tail touches runs., _sliding_end_edges(), _tail_material_end(), A plug-less door seals along its wall-plane edges only (W-gate     iteration 3 s, TestPlaneRestrictedFallback
...
Cohesion: 0.11
Nodes (21): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), _candidate_to_dict() (+13 more)
...
### Community 259 - "viewport_bbox_to_px"
Cohesion: 0.31
...
### Community 263 - "TestDashRowDiscriminators"
Cohesion: 0.33
...
### Community 266 - "jamb_seek_census.py"
Cohesion: 0.33
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `TestMarkerRings` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `TestWindowGates`, `Window Detection & Tests`, `TestFarSidePairs`, `TestDashRowDiscriminators`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `client.py`, `Double-Arc Split Tests`, `client.py`, `Room Polygonization Internals`, `TestThickMaterialPairs`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `_check_opening_clear`, `_fit_circle_3pt`, `_arc`, `TestAnnotationPenBarriers`, `review.py`, `viewport_bbox_to_px`, `TestSwingHingePlugRestriction`, `TestWindowArbitraryAngle`, `test_curve_arc_garden_doors.py`, `renderer.py`, `PruneArcSpursTests`, `_collect_wall_faces`, `path`, `_vector_text_indices`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `migrate-labour-rates-to-groups.ts`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `fill_ring`, `_covers`, `TestDashRowDiscriminators`, `squat_cap_window`, `wall_band_h`, `TestMinWidthReference`, `resolve_room_labels`, `bezier_arc`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `test_sliding_doors.py`, `_find_openings`, `app.py`, `_bridge_white_runs`, `TestBandPocket`, `_collect_wall_faces`, `_prune_arc_spurs`, `TestNetworkQueries`, `_is_arc_like`, `TestWindowTopology`, `WallSegment`, `_segments_min_distance`?**
  _High betweenness centrality (0.140) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `render_page_svg` to `Pipeline Orchestration & Extraction`, `Double-Door Merge & Gemini Client`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `ShaMismatchAgainstTruthTests`, `TestCliEquivalence`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `renderer.py`, `_dedupe_openings`, `TestXYCut`, `TestCheckDoorLeaves`, `TestWindowTightPairInterior`, `TakeoffRequest`, `resolve_room_labels`, `TestMarkerRings`, `detect_doors`, `TestAnnotationPenBarriers`, `resolve_heights`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `Candidate` connect `TestSwingHingePlugRestriction` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `TestWindowGates`, `TestWindowArbitraryAngle`, `_check_opening_clear`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `plumber.py`, `batch_extract.py`, `path`, `TestXYCut`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `EntranceDoorTests`, `TestSheetSize`, `SplitDoubleArcTests`, `migrate-labour-rates-to-groups.ts`, `TestWindowExteriorSide`, `test_batch_extract.py`, `fill_ring`, `TestWindowGates`, `TestWindowTightPairInterior`, `_covers`, `squat_cap_window`, `wall_band_h`, `TestMinWidthReference`, `TestMarkerRings`, `vline`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestBandPocket`, `framed_triple_window`, `qualifying_clip_rects`, `TestNetworkQueries`, `TestWindowTopology`, `WallSegment`, `_segments_min_distance`, `render_page_svg`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Are the 146 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 146 INFERRED edges - model-reasoned connections that need verification._
- **Are the 93 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `CrossGates`) actually correct?**
  _`Candidate` has 93 INFERRED edges - model-reasoned connections that need verification._