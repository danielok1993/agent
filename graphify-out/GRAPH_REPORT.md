...
## Corpus Check
- 181 files · ~278,194 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3039 nodes · 7509 edges · 196 communities (129 shown, 67 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 466 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ff89fbd9`
...
Cohesion: 0.07
Nodes (20): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+12 more)
...
Cohesion: 0.07
Nodes (25): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_center(), _bbox_iou(), CrossGates, _projected_overlap_1d() (+17 more)
...
Cohesion: 0.15
Nodes (20): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+12 more)

### Community 22 - "geometry.py"
Cohesion: 0.10
Nodes (52): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _is_arc_like(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+44 more)
...
Cohesion: 0.23
Nodes (9): build_ink_map(), is_page_spanning(), Binary ink occupancy map over a page, used to find whitespace gutters., True for sheet furniture: a border rule or column divider that runs the     leng, page(), path(), Ink occupancy map tests (layout/occupancy.py)., TestBuildInkMap (+1 more)
...
Cohesion: 0.22
Nodes (8): TextSpan, Every scale printed on the page, each carrying its span's bbox., text_scales(), span(), TestRegionTitleText, Reading a 1:N scale out of text spans.  Every string below is copied verbatim fr, span(), TestTextScales
...
Cohesion: 0.16
Nodes (9): Path, door_0007 -> d7. Unparseable ids are returned unchanged., Draw one review_<type>.png per entity type present in `unreviewed`.      Returns, short_id(), write_review_overlays(), Review images: one per page per entity type, ids stamped on., ReviewOverlayTests, ShortIdTests (+1 more)
...
Cohesion: 0.05
Nodes (80): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _interval_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis() (+72 more)
...
Cohesion: 0.09
Nodes (18): build_extract_command(), find_pdfs(), main(), prompt_bool(), Path, Run extract command for a single PDF.     Returns (pdf_path, success: bool, outp, Prompt user for a yes/no question, return bool., Find all PDF files in plans_dir (non-recursive). (+10 more)
...
Cohesion: 0.21
Nodes (6): Split a page into drawing regions. Returns [] for a page with no vector     ink, segment_page(), PageData, TestSegmentPage, run_heuristics emits per-stage wall-clock timings on its module logger.  The 202, TestStageTimingLogs
...
Cohesion: 0.21
Nodes (14): latest_run(), Path, Where a sweep leaves its output.  Sweeps used to extract into a `tempfile.Tempor, Wipe and recreate this slug's output directory., The most recent run directory for this slug, or None.      Timestamp names sort, reset_slug_dir(), slug_dir(), _centre() (+6 more)
...
Cohesion: 0.11
Nodes (28): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), The scales block written into each page's summary.json entry., scale_summary_dict(), detection_scale(), DetectionScale, _effective_denominator() (+20 more)
...
Cohesion: 0.07
Nodes (31): _building_masses(), detect_rooms(), _door_plugs(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges(), Room detection: rooms are the connected free-space components between walls.  Ea, World-space room gates pre-multiplied by the detection factor     (areas by fact (+23 more)
...
Cohesion: 0.40
Nodes (4): _production_cross_gates_unscaled_usages(), Ratchet on detection/'s production uses of CROSS_GATES_UNSCALED.      CROSS_GATE, Scan detection/**/*.py for PRODUCTION (non-import, non-comment) uses     of the, TestCrossGatesUnscaledStopgapRatchet
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Pipeline Orchestration & Extraction` to `Door Assembly & Heuristics Core`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `DoorAssemblyTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Detection Primitives`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `wall_band_v`, `arcs.py`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `detect_windows`, `_double_arc`, `SplitDoubleArcTests`, `ScaleInfo`, `renderer.py`, `test_scale_units.py`, `_projected_interval`, `_collect_wall_faces`, `batch_extract.py`, `test_curve_arc_garden_doors.py`, `TestOpeningSeals`, `renderer.py`, `TestNetworkQueries`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `EntranceDoorTests`, `TestCrossGatesUnscaledStopgapRatchet`, `TestDoorGatesUnscaledStopgapRatchet`, `TestFloorPlansRegression`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.147) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Arc Cap-Trim Tests` to `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `plumber.py`, `SplitDoubleArcTests`, `ScaleInfo`, `test_scale_units.py`, `batch_extract.py`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `EntranceDoorTests`, `TestCrossGatesUnscaledStopgapRatchet`, `TestDoorGatesUnscaledStopgapRatchet`, `_dedupe_openings`, `TestCrossWindowToleranceUnscaled`, `TestFloorPlansRegression`, `wall_band_h`, `TestMarkerRings`, `_find_openings`, `app.py`, `TestAnnotationPenBarriers`, `_segments_min_distance`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `TestWindowInteriorClutter` to `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `DoorAssemblyTests`, `Room Detection Tests`, `Wall Network Construction & Tests`, `EntranceDoorTests`, `Arc Cap-Trim Tests`, `wall_band_v`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `hline`, `detect_windows`, `_projected_interval`, `ScaleInfo`, `SplitDoubleArcTests`, `test_scale_units.py`, `renderer.py`, `scales_in_text`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `EntranceDoorTests`, `detect_doors`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`?**
