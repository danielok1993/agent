...
## Corpus Check
- 220 files · ~369,266 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3916 nodes · 10088 edges · 217 communities (149 shown, 68 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 737 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6cb88855`
...
## Communities (217 total, 68 thin omitted)

### Community 0 - "Pipeline Orchestration & Extraction"
Cohesion: 0.09
Nodes (45): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), _draw_regions(), _load_font(), BBox, Image (+37 more)

### Community 1 - "Door Assembly & Heuristics Core"
Cohesion: 0.14
Nodes (15): draw_overlay(), Entity, One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, Region, _page_summary_dict(), PageRegionResult, DetectionScale, Region outlines on the overlay (extraction/renderer.py). (+7 more)
...
Cohesion: 0.06
Nodes (44): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), _merge_double_door_assemblies(), Prefer the strongest door when two candidates use the same primitives., Merge pairs of adjacent single-door assemblies into double-swing candidates., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line (+36 more)
...
Cohesion: 0.06
Nodes (45): _bbox_area(), _bbox_center(), _bbox_iou(), _cross_validate(), _projected_overlap_1d(), BBox, Projected overlap fraction and perpendicular gap on bbox a's dominant axis., Type-specific NMS: higher confidence wins when two candidates overlap.      For (+37 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.07
Nodes (71): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), Detect door-swing arcs approximated by connected short line segments.      Some, Pair single-Bezier `curve_arc` swings that form a garden-door split.      The po, _door_fallback_candidate(), _find_threshold_line() (+63 more)
...
Cohesion: 0.06
Nodes (45): cache_file(), cache_key(), load_regions(), page_content_hash(), Path, On-disk cache of region classifications, keyed by page content AND the segmentat, Stable digest of a page's vector geometry and text. Changes if the PDF     is ed, Stable digest of a segmentation's geometry — the boxes and where they     came f (+37 more)
...
Cohesion: 0.10
Nodes (17): _effective_denominator(), _gate_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., The denominator allowed to drive gate scaling, or None to abstain.      Only a D, Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., canonical_denominators(), denominator_from_c() (+9 more)
...
Cohesion: 0.09
Nodes (21): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, hline(), path(), paving_field(), Stick-font text drawn as line strokes (s06/s11/s16/s20: no text     spans, every, HITL' in 14px stick glyphs, cap line y, baseline y + 14., Partition wall in the joinery pen: two hairline faces with diagonal     hatch st (+13 more)
...
Cohesion: 0.07
Nodes (41): attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No, The per-room quantity block mirrored onto Entity.attributes["takeoff"].      Liv (+33 more)
...
Cohesion: 0.06
Nodes (39): _accept_jamb_rings(), _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_door_lining() (+31 more)
...
Cohesion: 0.17
Nodes (5): Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, rect_room(), TestAnnotationPenBarriers, TestClosedRooms, TestComponentFiltering
...
Cohesion: 0.08
Nodes (32): _apply(), _as_transform(), classify_page(), _color_tuple(), extract_document(), extract_images(), extract_page(), extract_paths() (+24 more)
...
Cohesion: 0.04
Nodes (95): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_to_segment_distance(), _project_onto_axis(), _projected_interval(), Minimum distance from point p to line segment ab., Minimum distance between two line segments. (+87 more)
...
Cohesion: 0.06
Nodes (37): bind_scale(), binding_texts(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., Every text scale near enough to this region to be about it, nearest first., resolve_page_scales(), _cache_file(), _from_dicts() (+29 more)
...
Cohesion: 0.12
Nodes (17): PageTruth, TruthItem, _centre(), exit_code(), Sweep results, their rendering, and the exit-code contract.  Exit codes:   0  cl, render(), SheetResult, evaluate_page() (+9 more)
...
Cohesion: 0.07
Nodes (24): build_extract_command(), find_pdfs(), main(), prompt_bool(), Path, Run extract command for a single PDF.     Returns (pdf_path, success: bool, outp, Prompt user for a yes/no question, return bool., Find all PDF files in plans_dir (non-recursive). (+16 more)
...
Cohesion: 0.10
Nodes (23): assigned_path_fraction(), _centre_in_any(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, Share of the page's paths that any region would keep.      Deliberately the same, Text spans inside the given regions. Used to scope schedule detection to     sch, region_text_spans(), Page segmentation: split a sheet into its constituent drawings. (+15 more)
...
Cohesion: 0.07
Nodes (16): qualifying_clip_rects(), Read scissor rects off a fitz.Page and gate them. Returns [] if the     PDF expo, TestCase, Path, Skip helper for tests that need a real corpus sheet.  Corpus knowledge lives in, Return the sheet's path, or skip the test with an actionable message., require_sheet(), LoaderTests (+8 more)
...
Cohesion: 0.15
Nodes (9): _contains(), _ratio_pair(), Which drawing scale a room is measured at, and whether it can be trusted.  Pages, (w_ratio, h_ratio) of page over ISO size, orientation-matched., select_room_scale(), sheet_size_tokens(), verify_sheet_size(), TestSelectRoomScale (+1 more)
...
Cohesion: 0.23
Nodes (6): hline(), A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., Rect room with a 45px doorway gap in the top wall (240..285)., TestJambNib, wall_band_h(), wall_band_v()
...
Cohesion: 0.20
Nodes (9): _caption_distance(), _centroid(), _contains(), The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, How far a text span sits from a region, or None if it is not near it.      Horiz, _stored_info(), The nearest standard scale within tolerance, or None.      None is a real answer, snap_to_standard() (+1 more)

### Community 150 - "TestAnnotationPenBarriers"
Cohesion: 0.24
Nodes (6): path(), Filled arrowhead triangle (a marker ring) pointing down at `tip`., Stairs are furniture to the room stage: a room polygon runs to the     enclosing, stair_arrowhead(), TestStairFurniture, vline()
...
Cohesion: 0.14
Nodes (16): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, detection_scale(), PageScales, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict(), info(), detection_scale(): PageScales + regions -> one detection factor per page. (+8 more)
...
Cohesion: 0.33
Nodes (4): _fill_ring_components(), _is_light_pen(), Group ring ids (restricted to `members`) connected by shared seams.      Exporte, Faint (light-grey/pastel) ink: every channel at/above the light floor.
...
Cohesion: 0.08
Nodes (19): parse_measure_viewports(), Tier 1 — the scale the PDF states in its own viewport measure dictionaries.  A C, Every drawing scale this page's viewports state, smallest bbox first.      Small, Split a PDF array string into its top-level ``<< >>`` dictionaries.      Depth-c, Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, split_pdf_dicts(), viewport_scales(), _FakeDoc (+11 more)
...
Cohesion: 0.13
Nodes (16): Scale-aware window gates: WindowGates, threading, and the frozen classification', Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., rot_paths(), framed_triple_window(), quad(), Window detection tests.  Ground truth was established interactively on s01 (form, 5-1133 W8: a three-light frame tagged with a single label. Two full-span     rai, s04 BATHROOM 01 outer-wall window (paths 60-65, 0.56px A-DETL): the     opening (+8 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Double-Door Merge & Gemini Client` to `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `ShaMismatchAgainstTruthTests`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cap-Trim Tests`, `arcs.py`, `test_extraction_transform.py`, `Arc Cycle-Cap Pruning Tests`, `resolver.py`, `TestAnnotationPenBarriers`, `hline`, `test_layout_segmenter.py`, `detect_windows`, `_double_arc`, `_projected_interval`, `TestNetworkQueries`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `_FillRing`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `line`, `test_sliding_doors.py`, `PruneUnreadPageOutputTests`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_bridge_white_runs`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.176) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Door Detection & Tests` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `DoorAssemblyTests`, `ShaMismatchAgainstTruthTests`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `plumber.py`, `batch_extract.py`, `_dedupe_openings`, `test_window_detection.py`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `test_sliding_doors.py`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `_prune_arc_spurs`, `TestMinWidthNegativeControl`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `vline`, `_bridge_white_runs`, `_find_openings`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Door Detection & Tests` to `Door Assembly & Heuristics Core`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `_check_opening_clear`, `test_extraction_transform.py`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `detect_windows`, `_projected_interval`, `test_extraction_transform.py`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `_prune_arc_spurs`, `PruneUnreadPageOutputTests`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `vline`, `_bridge_white_runs`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`?**
