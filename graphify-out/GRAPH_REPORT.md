...
## Corpus Check
- 182 files · ~279,411 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3052 nodes · 7538 edges · 200 communities (134 shown, 66 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 466 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3d76d3e4`
...
## Communities (200 total, 66 thin omitted)
...
Cohesion: 0.07
Nodes (66): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+58 more)
...
Cohesion: 0.09
Nodes (28): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), Prefer the strongest door when two candidates use the same primitives., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_area() (+20 more)
...
Cohesion: 0.06
Nodes (31): Record whether a line segment passed the polyline-arc length filter., Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive., _native_curve_chains(), Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, _compute_hu_distance(), _rasterize_paths_to_canvas() (+23 more)
...
Cohesion: 0.13
Nodes (39): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), _is_arc_like(), BBox (+31 more)
...
Cohesion: 0.18
Nodes (11): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, Pier tier: strong faces spaced past WALL_MAX_THICKNESS_PX pair only     on drawn (+3 more)
...
Cohesion: 0.19
Nodes (8): _centre_in_any(), filter_page_data(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, A copy of page_data holding only primitives whose bbox centre falls in     one o, Text spans inside the given regions. Used to scope schedule detection to     sch, region_text_spans(), TestFilterPageData
...
Cohesion: 0.14
Nodes (12): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, _stored_info(), Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, Every scale printed on the page, each carrying its span's bbox., text_scales(), The nearest standard scale within tolerance, or None.      None is a real answer, snap_to_standard() (+4 more)
...
Cohesion: 0.14
Nodes (22): manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sha256_of(), sheet_entry(), sheet_path() (+14 more)
...
Cohesion: 0.05
Nodes (77): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _fit_oriented_rect(), Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u, _angle_diff_mod180(), _interval_overlap(), _line_angle_deg(), _line_length() (+69 more)
...
Cohesion: 0.16
Nodes (8): parse_measure_viewports(), Tier 1 — the scale the PDF states in its own viewport measure dictionaries.  A C, Split a PDF array string into its top-level ``<< >>`` dictionaries.      Depth-c, Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, split_pdf_dicts(), Parsing /VP -> /Measure viewport dictionaries.  The array strings below are copi, TestParseMeasureViewports, TestSplitPdfDicts
...
Cohesion: 0.16
Nodes (14): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, The per-region scale table printed after each page., The scales block written into each page's summary.json entry., scale_summary_dict(), scale_table(), Scale reporting inside the pipeline: the console table and summary.json., raw is lifted verbatim from PDF text and can contain a bracket         sequence (+6 more)
...
Cohesion: 0.10
Nodes (21): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), Resolve a scale for every floor-plan region on one page., How far a text span sits from a region, or None if it is not near it.      Horiz, Every text scale near enough to this region to be about it, nearest first. (+13 more)

### Community 131 - "test_layout_segmenter.py"
Cohesion: 0.13
Nodes (17): assigned_path_fraction(), Share of the page's paths that any region would keep.      Deliberately the same, _merge_captions(), page_fallback_region(), Fold zero-path title strips into the drawing they belong to.      A caption is a, Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page() (+9 more)
...
Cohesion: 0.11
Nodes (21): Flip a manifest entry's `labeled` flag and write the manifest back.      `labele, set_labeled(), dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload() (+13 more)
...
Cohesion: 0.21
Nodes (13): _page_summary_dict(), detection_scale(), DetectionScale, _effective_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., PageScales, info() (+5 more)
...
Cohesion: 0.35
Nodes (4): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DoorAssemblyTests, path()
...
Cohesion: 0.11
Nodes (18): _collect_fill_rings(), _collect_wall_faces(), _fill_key(), _FillRing, _is_dashed(), _rate_fill_classes(), A closed same-fill polygon reconstructed from exploded `l` items., Annotation arrowhead: a tiny filled triangle or concave dart.          Walls are (+10 more)
...
Cohesion: 0.18
Nodes (14): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, render_page_png(), _candidate_to_dict(), collect_warnings(), _entity_to_dict() (+6 more)
...
Cohesion: 0.13
Nodes (9): _FakeDoc, _FakePage, _Matrix, Just enough of fitz.Rect for viewport_scales: x0/y0/x1/y1 attributes., Just enough of fitz.Matrix for page_transform: a/b/c/d/e/f attributes., A page double with no fitz dependency -- no PDF is ever opened.      Only test_s, The bug lived here, not in viewport_bbox_to_px's arithmetic: the flip     formul, _Rect (+1 more)
...
### Community 193 - "TestCrossWindowToleranceUnscaled"
Cohesion: 0.31
...
Cohesion: 0.39
Nodes (7): latest_run(), Path, Where a sweep leaves its output.  Sweeps used to extract into a `tempfile.Tempor, Wipe and recreate this slug's output directory., The most recent run directory for this slug, or None.      Timestamp names sort, reset_slug_dir(), slug_dir()

### Community 196 - "TestMarkerRings"
Cohesion: 0.39
...
## Knowledge Gaps
- **454 isolated node(s):** `storage`, `sheets`, `Project purpose`, `Algorithm reference`, `Commands` (+449 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `DoorAssemblyTests`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `Room Detection Tests`, `Double-Arc Split Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `detect_windows`, `_double_arc`, `SplitDoubleArcTests`, `_projected_interval`, `renderer.py`, `test_scale_units.py`, `batch_extract.py`, `_collect_wall_faces`, `TestWindowArbitraryAngle`, `test_curve_arc_garden_doors.py`, `TestOpeningSeals`, `renderer.py`, `TestNetworkQueries`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestWindowTightPairInterior`, `EntranceDoorTests`, `TestCrossGatesUnscaledStopgapRatchet`, `TestSlugForPath`, `_dedupe_openings`, `TestMarkerRings`, `TestBlindWindowPocket`, `wall_band_h`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.151) - this node is a cross-community bridge._
- **Why does `Candidate` connect `Arc Detection Primitives` to `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `TestWindowArbitraryAngle`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `plumber.py`, `SplitDoubleArcTests`, `ScaleInfo`, `test_scale_units.py`, `batch_extract.py`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `TestWindowTightPairInterior`, `TestFloorPlansRegression`, `EntranceDoorTests`, `_dedupe_openings`, `TestBlindWindowPocket`, `wall_band_h`, `TestMarkerRings`, `_find_openings`, `app.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Arc Cap-Trim Tests` to `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `EntranceDoorTests`, `wall_band_v`, `arcs.py`, `geometry.py`, `hline`, `detect_windows`, `_projected_interval`, `SplitDoubleArcTests`, `renderer.py`, `test_scale_units.py`, `scales_in_text`, `test_curve_arc_garden_doors.py`, `TestThickMaterialPairs`, `TestSlugForPath`, `TestBlindWindowPocket`, `TestFloorPlansRegression`, `EntranceDoorTests`, `_dedupe_openings`, `TestBlindWindowPocket`, `wall_band_h`, `TestWindowInteriorClutter`, `detect_doors`, `app.py`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Are the 101 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 101 INFERRED edges - model-reasoned connections that need verification._
