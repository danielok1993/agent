...
## Corpus Check
- 57 files · ~98,965 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1250 nodes · 3219 edges · 114 communities (56 shown, 58 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 150 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `da1f4acb`
...
1. `PathPrimitive` - 161 edges
2. `Candidate` - 107 edges
3. `detect_wall_network()` - 70 edges
4. `TextSpan` - 58 edges
...
## Communities (114 total, 58 thin omitted)
...
Cohesion: 0.12
Nodes (16): _band_interior_clutter(), _cap_orientation_frames(), detect_windows(), _frame_axes(), _merge_mullion_chains(), Caps grouped by direction into overlapping frames, each ``(center, caps)``., Unit run-axis u (perpendicular to the caps) and perp-axis v (along caps).      C, Join collinear glazing segments across mullion blocks into logical panes.      A (+8 more)
...
Cohesion: 0.07
Nodes (30): _check_opening_clear(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, detect_doors(), Minimum distance between two line segments., _segments_min_distance(), _curve(), CurveArcGardenDoorTests, _line() (+22 more)
...
Cohesion: 0.06
Nodes (36): _cross_validate(), BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), BBox, One merged wall-face run with the evidence its members carried., Connected wall-centerline network (internal-only, never serialized). (+28 more)
...
Cohesion: 0.11
Nodes (35): _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip(), _mean_axis_deg(), _open_v_match() (+27 more)
...
Cohesion: 0.16
Nodes (8): door_candidate(), Rect room with a 45px doorway gap in the top wall (240..285)., The dilated-bbox fallback is the one seal with no evidence of its     own, so it, TestBboxSealFloor, TestOpeningSeals, TestSwingRecessDissolution, wall_band_h(), wall_band_v()

### Community 10 - "Wall Network Construction & Tests"
Cohesion: 0.16
Nodes (11): detect_wall_network(), Build the internal wall-centerline network for a page.      exclude_path_indices, hline(), path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, Horizontal wall drawn as two stroked faces., TestCenterlines, TestNetworkAssembly (+3 more)
...
Cohesion: 0.05
Nodes (79): cmd_extract(), cmd_inspect(), main(), parse_page_spec(), Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]., build_extract_command(), find_pdfs(), main() (+71 more)

### Community 13 - "Window Geometry Internals"
Cohesion: 0.29
Nodes (8): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_iou(), Type-specific NMS: higher confidence wins when two candidates overlap.      For, # NOTE: there is deliberately no "drop windows that look like wall linework", _suppress()
...
Cohesion: 0.10
Nodes (16): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox short-end edges of a sliding door: across the wall, never wall plane., Thin barrier bands along the wall planes through a detected door.      The door, _sliding_end_edges(), TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet (+8 more)

### Community 16 - "Arc Cycle-Cap Pruning Tests"
Cohesion: 0.14
Nodes (11): _chain(), PruneArcCycleCapsTests, Tests for _prune_arc_cycle_caps.      A 'closed-cycle cap' is a closed loop of s, An arc with no degree-3+ vertices has nothing to prune., 11-seg arc + closed 4-seg rectangle attached at arc end.         The junction is, The polyline_856 shape: 11-seg arc + 7-seg closed cap loop         attached at t, A cycle of more than DOOR_POLYLINE_CYCLE_MAX_SEGMENTS segments         exceeds t, A pure cycle (no junction) has nothing to attach to. The helper         only fir (+3 more)
...
Cohesion: 0.14
Nodes (26): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+18 more)
...
Cohesion: 0.16
Nodes (21): _axial_offsets(), _collect_slide_panels(), _fit_oriented_rect(), _line_segs(), _merge_spans(), _pair_leaf_panels(), _panel_shape_ok(), _pocket_leaf_match() (+13 more)
...
Cohesion: 0.21
Nodes (8): hline(), Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, rect_room(), rooms_for(), TestAnnotationPenBarriers, TestClosedRooms, TestComponentFiltering, vline()
...
Cohesion: 0.17
Nodes (7): fill_ring(), path(), Closed filled rectangle exploded into 4 chained `l` items (the     Vectorworks f, Room-interior ink (masks, tile grids, furniture) must not chop rooms;     classi, Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, TestBarrierAllowlist, TestPhantomDoorSeals
...
Cohesion: 0.35
Nodes (7): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), detect_schedules()
...
Cohesion: 0.15
Nodes (11): _collect_wall_faces(), Return (stroked wall faces, filled-band centerlines)., fill_ring(), marker_ring(), Wall-network builder tests (detection/walls.py).  Synthetic PathPrimitive fixtur, Closed filled rectangle exploded into 4 chained `l` items., Filled triangle/dart exploded into chained `l` items (a leader tip)., Leader/dimension arrowheads share the wall pen on Vectorworks-style     exports; (+3 more)
...
Cohesion: 0.20
Nodes (17): _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _is_arc_like(), _prune_arc_cycle_caps(), _prune_arc_spurs(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The (+9 more)

### Community 104 - "detect_doors"
Cohesion: 0.22
Nodes (4): Record result of the _is_door_leaf check for a primitive., Pre-populate by_path_index with raw metadata for every PathPrimitive., Record result of the _is_arc_like check for a primitive., Record whether a line segment passed the polyline-arc length filter.

### Community 105 - "PageData"
Cohesion: 0.25
Nodes (14): Client, build_user_message(), call_gemini(), _candidate_to_dict(), encode_image_inline(), init_client(), parse_gemini_response(), should_skip_gemini() (+6 more)
...
### Community 107 - "test_curve_arc_garden_doors.py"
Cohesion: 0.33
...
Cohesion: 0.13
Nodes (21): _interval_overlap(), _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _area(), _cap_record(), _dedupe_by_perp() (+13 more)

### Community 111 - "app.py"
Cohesion: 0.07
Nodes (51): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _band_has_wall_material(), _claims_interior_pair(), _collapse_redundant_centerlines(), _collect_fill_rings(), _collect_material_marks() (+43 more)

### Community 112 - "TestWindowArbitraryAngle"
Cohesion: 0.40
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `PathPrimitive` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `windows.py`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `hline`, `detect_windows`, `plumber.py`, `_projected_interval`, `renderer.py`, `batch_extract.py`, `_collect_wall_faces`, `renderer.py`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `PageData`, `TestNetworkQueries`, `test_door_assembly.py`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.311) - this node is a cross-community bridge._
- **Why does `Candidate` connect `PageData` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Double-Arc Split Tests`, `Window Geometry Internals`, `Arc Cap-Trim Tests`, `_fit_circle_3pt`, `geometry.py`, `hline`, `detect_windows`, `_projected_interval`, `renderer.py`, `batch_extract.py`, `_collect_wall_faces`, `renderer.py`, `vline`, `wall_band_h`, `TestWindowInteriorClutter`, `DoorV2OpeningCheckTests`, `_find_openings`, `EntranceDoorTests`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`?**
  _High betweenness centrality (0.119) - this node is a cross-community bridge._
- **Why does `detect_wall_network()` connect `Wall Network Construction & Tests` to `wall_band_h`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestMarkerRings`, `PathPrimitive`, `renderer.py`, `Arc Detection Primitives`, `test_door_assembly.py`, `app.py`, `Arc Cap-Trim Tests`, `windows.py`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Are the 62 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 62 INFERRED edges - model-reasoned connections that need verification._
- **Are the 36 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `TestDoorPenalties`) actually correct?**
  _`Candidate` has 36 INFERRED edges - model-reasoned connections that need verification._