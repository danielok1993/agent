...
## Corpus Check
- 405 files · ~1,532,697 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5425 nodes · 13444 edges · 330 communities (222 shown, 108 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 1018 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `acd7157d`
...
- `TestArcGatesThreading` --uses--> `DoorGates`  [INFERRED]
...
Cohesion: 0.08
Nodes (23): agreement(), DimensionMatch, measured_denominator(), BBox, The drawing's own dimension strings as a scale MEASUREMENT.  "3600" written besi, The drawing's measured scale: the median implied denominator over the     matche, How the measured scale relates to a claimed one: "ok" within     DIM_AGREE_TOL,, assess_scale() (+15 more)
...
Cohesion: 0.12
Nodes (16): _attach_text_spans(), page_fallback_region(), Grow paths-only boxes to absorb the text spans beside them.      The tier-2 cut, Split a page into drawing regions. Returns [] for a page with no vector     ink, The whole page as a single region, for sheets too dense to split., segment_page(), PageData, block() (+8 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.12
Nodes (30): baseline_dir(), baseline_run(), compare(), compare_runs(), _crop(), diff_entities(), _draw_entities(), _entities_by_page() (+22 more)
...
Cohesion: 0.10
Nodes (29): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_center(), detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut (+21 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.03
Nodes (141): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+133 more)
...
Cohesion: 0.11
Nodes (12): paving_field(), Running-bond paving: continuous course lines, staggered joint lines.      Mirror, Striped fields (paving bonds, tile fields, treads) are not walls., Stroke-color pen identity: pairing, faint-ink demotion, dimension     chains, an, A DRAWN DASH LINE — one line its line type exploded into a periodic     row of s, Four wall bands forming a closed rectangular room (outer faces at the     given, rect_room(), TestDashRowExclusion (+4 more)
...
Cohesion: 0.08
Nodes (34): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., render_page_png(), _candidate_to_dict(), collect_warnings(), _entity_to_dict(), make_output_dir() (+26 more)
...
Cohesion: 0.05
Nodes (55): dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), Path (+47 more)

### Community 107 - "vline"
Cohesion: 0.12
Nodes (13): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DegenerateCompanionTests, DoorAssemblyTests, EntranceDoorTests, line(), path(), quarter_arc_lines() (+5 more)
...
Cohesion: 0.12
Nodes (20): detect(), EndToEndTests, fold_chain(), FoldChainTests, folding_of(), leaf(), OpenVTests, parked_stack() (+12 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.05
Nodes (69): _accept_jamb_rings(), _back_edge_cover(), _building_masses(), _clip_plug_tails(), _contains_text(), detect_rooms(), _doorway_pens(), _drop_window_exterior_sides() (+61 more)

### Community 120 - "TestNetworkQueries"
Cohesion: 0.11
Nodes (12): door_candidate(), On a colour-coded drawing the wall pens are the pens the sheet's     DOORWAYS ar, Black rect room, 45px doorway in the top band (240..285)., Three red same-pen pairs (unit boxes, 20px deep, 90px long) —         540px of p, Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, rooms_for(), TestBboxSealFloor (+4 more)
...
Cohesion: 0.15
Nodes (18): _accept_standoff(), _cap_lines(), census(), _edge_cover(), _mrr(), Step 15 — `_is_band_pocket`'s COVER READING measured on both populations, all 20, The reading the rule used before step 15 (`_edge_face_cover`, since     replaced, Covered length of one boundary run by the lines lying beside it:     faces at th (+10 more)

### Community 126 - "_segments_min_distance"
Cohesion: 0.09
Nodes (14): _door_plugs(), Bbox edges whose plug tails may SEEK their jamb beyond the fixed reach.      Onl, How far along (ux, uy) from corner a tail sample first touches wall material., Thin barrier bands along the wall planes through a detected door.      The door, _seek_edges(), _seek_jamb(), Interrupted-run plugs need jambs that REACH the plug band and a mid     that is, A doorway plug's tail SEEKS its jamb when the fixed SEAL reach finds     none th (+6 more)
...
Cohesion: 0.11
Nodes (23): _cross_validate(), Validate doors/windows against the wall-centerline network.      Doors keep the, One merged wall-face run with the evidence its members carried., WallFace, continuous_h_wall(), door(), face(), h_wall_with_gap() (+15 more)
...
Cohesion: 0.22
Nodes (7): _door_attribute_overlay(), finalize_candidates(), Selected door-evidence keys to merge into Entity.attributes. {} for None / non-d, Promote candidates to entities, applying the offline confidence floors.      Gem, cand(), finalize_candidates applies the offline confidence floors unconditionally., TestFinalizeCandidates
...
Cohesion: 0.11
Nodes (29): manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, sha256_of(), sheet_entry(), sheet_path() (+21 more)
...
Cohesion: 0.17
Nodes (11): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, OpenLeafExclusionTests, Tests for door_open_leaf_path_indices on merged double-swing pairs.      Geometr, French leaves are drawn closed IN the wall plane — legitimate wall         evide, candidate_pairs(), main(), Measure face-pair TAPER on a corpus sheet — the diagnostic behind WALL_PAIR_TAPE (+3 more)

### Community 165 - "ScaleInfo"
Cohesion: 0.07
Nodes (40): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, PageRegionResult, detection_scale(), DetectionScale, `dimensions` is the page's ticked-dimension-string matches on the FULL     page (+32 more)
...
Cohesion: 0.18
Nodes (9): band_segments(), hatch(), A band hatched THROUGH — every diagonal stroke ending on both faces — is a drawn, 45° strokes across the band; inset > 0 stops them short of each face., WALL_THROUGH_HATCH_MAX_PX is 72px — 610mm at 1:50 (W-gate census     2026-09-04), ThroughHatchBandTests, ThroughHatchCapReferenceTests, Horizontal wall drawn as two stroked faces. (+1 more)
...
Cohesion: 0.16
Nodes (10): _collect_wall_faces(), _FillRing, _rate_fill_classes(), Classify each fill color as wall material (True) or furniture (False).      Vect, Return (stroked wall faces, filled-band centerlines)., A closed same-fill polygon reconstructed from exploded `l` items., Annotation arrowhead: a tiny filled triangle or concave dart.          Walls are, A filled wall band the exporter triangulated into two rings that     each carry (+2 more)
...
Cohesion: 0.06
Nodes (55): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, The whole page as one document. (+47 more)
...
Cohesion: 0.20
Nodes (9): Evidence: what broke at f = 50/92.2 = 0.542 (all measured on the real PDF), Handoff: W-gate recalibration (the proper fix behind `fix/measured-scale-detection-factor`), How the ablation was done (reproduce in ~30 min), Outcome — iteration 3, step 4 (2026-09-06, branch `recal/wall-max-thickness-40`, measurement only — `WALL_MAX_THICKNESS_PX` NOT moved), Prompt for the next agent (iteration 3, step 13 — the band-pocket ceiling — fresh context), Read these first (in order), The problem in one paragraph, The recalibration task (the "proper fix") (+1 more)

### Community 197 - "EntranceDoorTests"
Cohesion: 0.11
Nodes (11): Bbox short-end edges of a sliding door: across the wall, never wall plane., _sliding_end_edges(), Candidate, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., A plug-less door seals along its wall-plane edges only (W-gate     iteration 3 s, detect_rooms consumes candidates before the offline floor, so a door     the pip, TestBlindWindowPocket, TestDiagonalWindowSeal (+3 more)
...
Cohesion: 0.23
Nodes (7): match_entities(), MatchResult, entity(), MatchTests, Ground truth is matched to output geometrically.  Entity ids (door_0015) are ord, diff_vs_baseline(), Type+IoU>=0.5 match of a run against the f-baseline run.

### Community 203 - "HygieneRuleTests"
Cohesion: 0.21
Nodes (10): Scale-factor behavior of walls/rooms gates: identity at 1.0, shrunk-world at 0.5, Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), rooms_for(), shrink(), TestOrchestratorForwardsFactor, TestRoomsScaled (+2 more)

### Community 205 - "parse_answer"
Cohesion: 0.05
Nodes (52): _angle_diff_mod180(), Smaller angular distance between two directions, both already mod 180°., _band_interior_clutter(), _cap_orientation_frames(), Caps grouped by direction into overlapping frames, each ``(center, caps)``., Clutter strictly between the glazing panes — empty for a real window.      The s, census(), _gt_class() (+44 more)

### Community 206 - "DoorAssemblyTests"
Cohesion: 0.10
Nodes (26): classify(), EntityChange, entity_id -> verdict, using the sweep's own matching order     (confirmed first,, PageTruth, SheetTruth, TruthItem, evaluate_page(), _labeled_but_unreviewed() (+18 more)
...
Cohesion: 0.28
Nodes (13): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), _draw_regions(), _load_font(), BBox, Image (+5 more)
...
Cohesion: 0.10
Nodes (16): _open_leaf_edges(), Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, _restrict_swing_plugs(), _swing_hinge_edges(), Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, Single swing doors: plugs live on the hinge edges, one wall plane.      Geometry (+8 more)
...
Cohesion: 0.25
Nodes (3): draw_overlay(), Region outlines on the overlay (extraction/renderer.py)., TestDrawOverlayWithRegions
...
Cohesion: 0.03
Nodes (37): attrib.py <slug> <x0> <y0> <x1> <y1> FIELD=MULT [FIELD=MULT ...]  Runs the harne, attrib_rooms.py <slug> FIELD=MULT [...]: for each config (each alone on top of t, COLLINEAR_OFFSET_TOL forms, measured with the census harness on top of the curre, crop_dash.py <slug> <name> X0 Y0 X1 Y1 [...]: crop the latest sweep render with, denom_at(), _install_taps(), load(), mm() (+29 more)

### Community 251 - "resolve_heights"
Cohesion: 0.26
Nodes (4): A positive, finite number of metres — or ValueError naming the offender., resolve_heights(), valid_height_m(), TestResolveHeights
...
Cohesion: 0.36
Nodes (4): parse_height(), _prompt_ceiling(), Metres from "2.4", "2.4m", "2400", "2400mm". None to skip., TestParseHeight
...
Cohesion: 0.03
Nodes (128): _bbox_expanded(), _bbox_union(), _bboxes_overlap(), _interval_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox() (+120 more)
...
Cohesion: 0.21
Nodes (11): main(), Step-11 checkpoint pictures: s01 at its true factor (0.542), the share gate alon, draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1., The two step-9 pictures (guarded so crop_step10.py can import the drawing helper, side_by_side() (+3 more)
...
Cohesion: 0.48
Nodes (6): census(), edge_line(), owners_for(), The doorway-ownership wall-pen rule AS IT WOULD BE IMPLEMENTED, censused on ever, tails_of(), today_pens()
...
### Community 305 - "MainExceptionIsolationTests"
Cohesion: 0.48
...
### Community 316 - "_dedupe_openings"
Cohesion: 0.50
...
## Knowledge Gaps
- **787 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+782 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Double-Door Merge & Gemini Client` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Wall Cross-Validation`, `TestWindowArbitraryAngle`, `TestVectorTextExclusion`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `Pipeline Orchestration & Extraction`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Room Polygonization Internals`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `client.py`, `arcs.py`, `TestAnnotationPenBarriers`, `hline`, `viewport_bbox_to_px`, `_production_cross_gates_unscaled_usages`, `_production_door_gates_unscaled_usages`, `TestSwingHingePlugRestriction`, `plumber.py`, `test_curve_arc_garden_doors.py`, `_projected_interval`, `renderer.py`, `Outcome — iteration 3, step 4 (2026-09-06, branch `recal/wall-max-thickness-40`, measurement only — `WALL_MAX_THICKNESS_PX` NOT moved)`, `test_layout_golden.py`, `TestSpanFilterIsLoadBearing`, `jamb_pens.py`, `_dedupe_openings`, `test_window_detection.py`, `_is_light_pen`, `TestDashRowBarriers`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `label_rooms`, `test_batch_extract.py`, `_covers`, `TestDashRowDiscriminators`, `collect_room_spans`, `squat_cap_window`, `TestMarkerRings`, `bezier_arc`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `ablate.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.132) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `_fit_circle_3pt` to `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `client.py`, `Wall Network Construction & Tests`, `ShaMismatchAgainstTruthTests`, `resolver.py`, `TestCliEquivalence`, `_projected_interval`, `ScaleInfo`, `_collect_wall_faces`, `path`, `TestXYCut`, `TestCheckDoorLeaves`, `TestWindowTightPairInterior`, `resolve_page_regions`, `TestBandPocket`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `resolve_heights`, `TestRoomGatesConstruction`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `Candidate` connect `EntranceDoorTests` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `TestVectorTextExclusion`, `TestWindowArbitraryAngle`, `_dedupe_openings`, `TestThickMaterialPairs`, `ShaMismatchAgainstTruthTests`, `_leaf_ink_indices`, `arcs.py`, `_fit_circle_3pt`, `TestAnnotationPenBarriers`, `viewport_bbox_to_px`, `_production_cross_gates_unscaled_usages`, `plumber.py`, `test_curve_arc_garden_doors.py`, `ScaleInfo`, `test_window_detection.py`, `TestXYCut`, `_dedupe_openings`, `TestDashRowBarriers`, `TestValidationPathIsGone`, `TestSheetSize`, `TestWindowExteriorSide`, `label_rooms`, `test_batch_extract.py`, `TestWindowGates`, `_covers`, `collect_room_spans`, `squat_cap_window`, `resolve_room_labels`, `vline`, `_find_openings`, `EntranceDoorTests`, `ablate.py`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `_segments_min_distance`?**
