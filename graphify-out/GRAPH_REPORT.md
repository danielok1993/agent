...
## Corpus Check
- 220 files · ~354,001 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3792 nodes · 9692 edges · 222 communities (151 shown, 71 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 705 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `39a576d3`
...
## Communities (222 total, 71 thin omitted)
...
Cohesion: 0.07
Nodes (26): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+18 more)
...
Cohesion: 0.21
Nodes (4): Rect room with a 45px doorway gap in the top wall (240..285)., TestClosedRooms, TestEmptyNetwork, wall_band_h()
...
Cohesion: 0.08
Nodes (17): Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, canonical_denominators(), cluster_denominators(), denominator_from_c(), Scale arithmetic shared by every resolution tier.  A PDF /Measure dictionary sta, Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present., The 1:N denominator for a /Measure /X conversion factor. (+9 more)
...
### Community 24 - "hline"
Cohesion: 0.16
Nodes (13): detect(), fold_chain(), FoldChainTests, folding_of(), leaf(), OpenVTests, parked_stack(), ParkedStackPairTests (+5 more)
...
Cohesion: 0.07
Nodes (44): Entity, attach_takeoff(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., _room_entity(), attributes_by_room(), opening_dict(), takeoff.json — the document the web app's overlay and assembly table are both bu, One room: geometry, its opening ids, and its quantities.      `quantities` is No (+36 more)
...
Cohesion: 0.05
Nodes (55): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), Prefer the strongest door when two candidates use the same primitives., Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f, _stroke_is_visible(), detect_labels() (+47 more)
...
Cohesion: 0.12
Nodes (25): _building_masses(), detect_rooms(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _is_wall_recess(), Polygon, Room detection: rooms are the connected free-space components between walls.  Ea (+17 more)
...
Cohesion: 0.13
Nodes (14): _dedupe_by_perp(), Collapse near-collinear duplicates (same perp offset) to one record.      A toil, Largest run of panes spaced like glazing, not like stair treads.      Walks the, Distinct parallel glazing lines that connect cap ``c1`` to cap ``c2``.      A gl, _spanning_glazing(), _tight_band(), The spatial pruning inside detect_windows must be output-identical.      `_facin, Caps scattered over a page-sized (u, v) field, perp-sorted.          Determinist (+6 more)
...
Cohesion: 0.04
Nodes (90): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _project_onto_axis(), _projected_interval() (+82 more)
...
Cohesion: 0.09
Nodes (59): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect door-swing arcs approximated by connected short line segments.      Some (+51 more)
...
Cohesion: 0.19
Nodes (6): exit_code(), render(), SheetResult, ExitCodeTests, RenderTests, ReviewLineIdentityTests
...
Cohesion: 0.07
Nodes (32): bind_scale(), The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page., resolve_page_scales(), _cache_file(), _from_dicts(), load_stored(), match_stored() (+24 more)
...
Cohesion: 0.15
Nodes (12): binding_texts(), _caption_distance(), _centroid(), _contains(), The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, How far a text span sits from a region, or None if it is not near it.      Horiz, Every text scale near enough to this region to be about it, nearest first., _stored_info() (+4 more)

### Community 149 - "TestExtractPageFrame"
Cohesion: 0.12
Nodes (24): detect_schedules(), build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document(), extract_plumber_page(), _normalize_bbox_plumber() (+16 more)
...
Cohesion: 0.09
Nodes (35): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, Region, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., scale_table() (+27 more)
...
Cohesion: 0.35
Nodes (6): EndToEndTests, OrientedRectFitTests, prim(), qu_panel(), rect_corners(), white_ring()
...
Cohesion: 0.09
Nodes (16): _centre(), Sweep results, their rendering, and the exit-code contract.  Exit codes:   0  cl, _cache_missed(), _extract_for_sweep(), _prune_unread_page_output(), Path, Run the pipeline over corpus sheets and score the output.  Sheets are extracted, Delete the page-level files a sweep persists but never uses.      Making sweep o (+8 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Room Detection Tests` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `DoorAssemblyTests`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Double-Arc Split Tests`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `RunDirTests`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `detect_windows`, `_projected_interval`, `renderer.py`, `TestSwingHingePlugRestriction`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `renderer.py`, `TestNetworkQueries`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestXYCut`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestFarSidePairs`, `TestRenderPageSvg`, `TestSlidingScaleBehavior`, `TestThickMaterialPairs`, `_is_light_pen`, `TestJambNib`, `TestComponentFiltering`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `vline`, `_find_openings`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `test_door_assembly.py`, `batch_extract.py`, `_segments_min_distance`?**
  _High betweenness centrality (0.170) - this node is a cross-community bridge._
- **Why does `Candidate` connect `renderer.py` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `TestExtractImagesInstances`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `RunDirTests`, `TestExtractPageFrame`, `TestAnnotationPenBarriers`, `plumber.py`, `ScaleInfo`, `batch_extract.py`, `_dedupe_openings`, `_FillRing`, `test_window_detection.py`, `TestXYCut`, `TestPlumberTableBBox`, `TestWindowTightPairInterior`, `TestThickMaterialPairs`, `_is_light_pen`, `_dedupe_openings`, `TestWindowExteriorSide`, `_is_light_pen`, `TestJambNib`, `TestComponentFiltering`, `wall_band_h`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `vline`, `_find_openings`, `EntranceDoorTests`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `TestNetworkQueries`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `renderer.py` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Wall Network Construction & Tests`, `_check_opening_clear`, `arcs.py`, `RunDirTests`, `resolver.py`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `detect_windows`, `_projected_interval`, `ScaleInfo`, `renderer.py`, `scales_in_text`, `_dedupe_openings`, `TestWindowTightPairInterior`, `_is_light_pen`, `_dedupe_openings`, `_scan_striped_runs`, `TestWindowExteriorSide`, `TestJambNib`, `TestComponentFiltering`, `wall_band_h`, `detect_doors`, `vline`, `EntranceDoorTests`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`?**
