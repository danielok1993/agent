...
## Corpus Check
- 186 files · ~289,420 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3159 nodes · 7867 edges · 189 communities (125 shown, 64 thin omitted)
...
Cohesion: 0.06
Nodes (49): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+41 more)
...
Cohesion: 0.07
Nodes (39): _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _fit_circle_3pt(), _native_curve_chains(), Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Detect door-swing arcs approximated by connected short line segments.      Some, Fit a circle through 3 points. Returns (cx, cy, radius) or None if     the point (+31 more)

### Community 18 - "windows.py"
Cohesion: 0.05
Nodes (25): Path, Create the unlabeled ground-truth file for a newly adopted sheet., truth_path(), write_empty_truth(), address_match(), Shared address-detection patterns for corpus hygiene checks.  Two callers share, The matched address-like substring in `text`, or None., AdoptTests (+17 more)
...
Cohesion: 0.16
Nodes (18): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), BBox, Image (+10 more)

### Community 22 - "geometry.py"
Cohesion: 0.10
Nodes (38): _arc_corners(), _estimate_arc_sweep_deg(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _collect_door_leaves(), _collect_linework_door_leaves(), _find_thin_rectangle_cycle(), _is_door_leaf() (+30 more)
...
Cohesion: 0.10
Nodes (22): detect_wall_network(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., hline(), path(), paving_field(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st (+14 more)

### Community 35 - "plumber.py"
Cohesion: 0.13
Nodes (12): Client, init_client(), Vertex AI client construction.  Per-candidate validation was removed on 2026-07-, _door_attribute_overlay(), finalize_candidates(), Selected door-evidence keys to merge into Entity.attributes. {} for None / non-d, Promote candidates to entities, applying the offline confidence floors.      Gem, _room_entity() (+4 more)
...
Cohesion: 0.23
Nodes (8): framed_triple_window(), quad(), Window detection tests.  Ground truth was established interactively on s01 (form, 5-1133 W8: a three-light frame tagged with a single label. Two full-span     rai, 5-1133 W8 topology: block caps (qu jambs/mullions) + mullion-bridged     center, Collinear segments merge only across a gap a mullion block occupies —         th, A block with an X drawn through it is a post/column symbol (the         5-1133 b, TestFramedMultiLightWindow
...
Cohesion: 0.13
Nodes (15): _interval_overlap(), _dedupe_by_perp(), Collapse near-collinear duplicates (same perp offset) to one record.      A toil, Largest run of panes spaced like glazing, not like stair treads.      Walks the, Distinct parallel glazing lines that connect cap ``c1`` to cap ``c2``.      A gl, _spanning_glazing(), _tight_band(), The spatial pruning inside detect_windows must be output-identical.      `_facin (+7 more)

### Community 110 - "EntranceDoorTests"
Cohesion: 0.09
Nodes (20): Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, _stored_info(), Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, canonical_denominators(), cluster_denominators(), format_scale() (+12 more)

### Community 111 - "app.py"
Cohesion: 0.07
Nodes (40): _accept_white_walls(), _collect_fill_rings(), _collect_material_marks(), _collect_wall_faces(), _collect_weak_faces(), _dimension_line_indices(), _fill_key(), _FillRing (+32 more)
...
Cohesion: 0.14
Nodes (22): build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document(), extract_plumber_page(), _normalize_bbox_plumber(), BBox (+14 more)
...
Cohesion: 0.05
Nodes (43): dump_truth(), Write a sheet's verdicts back to tests/ground_truth/<slug>.json.      Round-trip, bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), Resolve a scale for every floor-plan region on one page. (+35 more)

### Community 131 - "test_layout_segmenter.py"
Cohesion: 0.17
Nodes (9): Split a page into drawing regions. Returns [] for a page with no vector     ink, segment_page(), PageData, block(), A solid-ish blob: a horizontal line every 4px so every bin row is inked., TestPathsOnlyRetry, TestSegmentPage, run_heuristics emits per-stage wall-clock timings on its module logger.  The 202 (+1 more)
...
Cohesion: 0.13
Nodes (14): dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), The user's per-sheet verdicts, and how they are read.  One file per sheet under, Stash a flat number array as a one-line literal; return its token.      json.dum (+6 more)
...
Cohesion: 0.16
Nodes (9): _merge_collinear_segs(), _pens_compatible(), Merge segments lying on the same infinite line into runs.      Bridges gaps up t, A wall's two faces are drawn by one pen; colorless (fill-outline)     geometry s, _hface(), Isolates _merge_collinear_segs's offset-tolerance scaling directly —     the exa, A bare horizontal wall-face _Seg for isolated merge-tolerance tests., TestMergeCollinearOffsetScaling (+1 more)
...
Cohesion: 0.08
Nodes (42): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _building_masses(), detect_rooms(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges() (+34 more)
...
Cohesion: 0.39
Nodes (3): Schedule detection — tables carry real bboxes.  detect_schedules used to emit bb, _table(), TestDetectSchedulesBBox
...
Cohesion: 0.09
Nodes (19): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., Drop window candidates that materially sit on a detected door.      Door symbols, _resolve_door_window_conflicts(), Candidate, DoorEvidencePropagationTests, _far_wall_network() (+11 more)
...
Cohesion: 0.10
Nodes (14): _door_plugs(), Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, Thin barrier bands along the wall planes through a detected door.      The door, _restrict_swing_plugs(), _swing_hinge_edges(), Wide garden pairs: jamb-scale anchor window + parked-leaf edge veto., Plug extensions end at their supporting material; slide ends veto.      Geometry (+6 more)
...
Cohesion: 0.08
Nodes (25): _check_opening_clear(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, DoorAssemblyTests, DoorV2OpeningCheckTests, EntranceDoorTests, line() (+17 more)
...
Cohesion: 0.15
Nodes (16): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), The scales block written into each page's summary.json entry., scale_summary_dict(), DetectionScale, Scale reporting inside the pipeline: the console table and summary.json., det_scale is resolved before the skip_detection branch in run_extract     (so th (+8 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cap-Trim Tests`, `test_extraction_transform.py`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `detect_windows`, `_projected_interval`, `renderer.py`, `batch_extract.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `MainExceptionIsolationTests`, `TestNetworkQueries`, `TestThickMaterialPairs`, `TestBlindWindowPocket`, `swing_door`, `TestCrossGatesUnscaledStopgapRatchet`, `TestWindowTightPairInterior`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `detect_doors`, `_find_openings`, `app.py`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.147) - this node is a cross-community bridge._
- **Why does `Candidate` connect `TestThickMaterialPairs` to `Pipeline Orchestration & Extraction`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `plumber.py`, `batch_extract.py`, `TestSlugForPath`, `TestBlindWindowPocket`, `swing_door`, `TestCrossGatesUnscaledStopgapRatchet`, `TestDoorGatesUnscaledStopgapRatchet`, `TestWindowTightPairInterior`, `TestMinWidthNegativeControl`, `TestWindowSpanOvershootRetune`, `TestMarkerRings`, `_find_openings`, `TestAnnotationPenBarriers`, `_collect_wall_faces`, `qualifying_clip_rects`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `PageData` connect `test_layout_segmenter.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `TestExtractImagesInstances`, `Double-Door Merge & Gemini Client`, `_dedupe_openings`, `Wall Network Construction & Tests`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cap-Trim Tests`, `test_extraction_transform.py`, `_fit_circle_3pt`, `fill_ring`, `TestWindowTightPairInterior`, `TestBlindWindowPocket`, `TestNetworkQueries`, `_projected_interval`, `renderer.py`, `scales_in_text`, `_collect_wall_faces`, `TestSlugForPath`, `swing_door`, `TestDoorGatesUnscaledStopgapRatchet`, `detect_doors`, `EntranceDoorTests`, `TestAnnotationPenBarriers`?**
