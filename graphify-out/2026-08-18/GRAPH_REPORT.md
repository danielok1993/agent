...
## Corpus Check
- 186 files · ~297,701 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3196 nodes · 8022 edges · 190 communities (127 shown, 63 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 506 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `04e7c667`
...
## Communities (190 total, 63 thin omitted)
...
Cohesion: 0.07
Nodes (33): _project_onto_axis(), Scalar projection of p onto the unit axis (dx, dy) from origin., Whether a path's segments are DRAWN lines, as opposed to the boundary     of a f, _stroke_is_visible(), _area(), _band_interior_clutter(), _block_cap_records(), _cap_orientation_frames() (+25 more)

### Community 3 - "Door Detection & Tests"
Cohesion: 0.21
Nodes (14): _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., _bbox_area(), _bbox_center(), _bbox_expanded(), BBox, _bbox_iou() (+6 more)
...
Cohesion: 0.12
Nodes (21): _cross_validate(), Validate doors/windows against the wall-centerline network.      Doors keep the, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried., WallFace, WallSegment, continuous_h_wall(), door() (+13 more)
...
Cohesion: 0.08
Nodes (64): _door_fallback_candidate(), _find_threshold_line(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, Parse an evidence bbox value defensively; return None on any invalid shape., _safe_bbox() (+56 more)

### Community 8 - "Arc Detection Primitives"
Cohesion: 0.08
Nodes (24): DoorGates, World-space door gates, pre-multiplied by the detection factor.      Fields keep, _collect_slide_panels(), _panel_shape_ok(), CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, prim(), _production_cross_gates_unscaled_usages() (+16 more)

### Community 9 - "Room Detection Tests"
Cohesion: 0.05
Nodes (39): Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, _restrict_swing_plugs(), _swing_hinge_edges(), door_candidate(), fill_ring(), hline(), path() (+31 more)
...
Cohesion: 0.10
Nodes (22): _native_curve_chains(), Group native `c` (Bezier) primitives by endpoint adjacency.      PDF arcs are of, _compute_hu_distance(), _rasterize_paths_to_canvas(), Rasterize line/curve primitives onto a normalized binary canvas.      Segments a, Distance between candidate arc paths and the door Hu Moment template.      Lower, PathPrimitive, ChainedCurveSwingDetectionTests (+14 more)
...
Cohesion: 0.08
Nodes (27): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+19 more)

### Community 22 - "geometry.py"
Cohesion: 0.12
Nodes (36): _arc_corners(), _collect_door_swings(), _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _estimate_arc_sweep_deg(), _fit_circle_3pt(), BBox, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The (+28 more)
...
### Community 24 - "hline"
Cohesion: 0.12
Nodes (18): door_open_leaf_path_indices(), _leaf_ink_indices(), _merge_double_door_assemblies(), Merge pairs of adjacent single-door assemblies into double-swing candidates., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, DoubleDoorTests, OpenLeafExclusionTests (+10 more)
...
Cohesion: 0.13
Nodes (18): _cache_file(), _from_dicts(), load_stored(), match_stored(), _merge(), BBox, Path, Tier 4 persistence — where a scale the user typed is kept.  Two back-ends, mirro (+10 more)
...
Cohesion: 0.12
Nodes (25): Path, Path to a downloaded sheet, or None when it is not on disk., sha256_of(), sheet_entry(), sheet_path(), _check_provenance(), _ordered(), What a persisted sweep still needs verdicts on.  Reads the run output the sweep (+17 more)
...
Cohesion: 0.06
Nodes (72): _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., _accept_white_walls(), _band_has_wall_material() (+64 more)
...
Cohesion: 0.16
Nodes (19): build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document(), extract_plumber_page(), _normalize_bbox_plumber(), BBox (+11 more)
...
Cohesion: 0.10
Nodes (33): baseline_dir(), baseline_run(), classify(), compare(), compare_runs(), _crop(), diff_entities(), _entities_by_page() (+25 more)
...
Cohesion: 0.06
Nodes (40): _building_masses(), detect_rooms(), _door_plugs(), _drop_window_exterior_sides(), _folding_chain_gap_plug(), _free_space_components(), _open_leaf_edges(), Room detection: rooms are the connected free-space components between walls.  Ea (+32 more)
...
Cohesion: 0.24
Nodes (9): detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, Per-stage wall-clock log line. Detection on 100k+-path sheets runs for     minut, run_heuristics(), _stage(), detect_schedules(), Schedule detection — tables carry real bboxes.  detect_schedules used to emit bb, _table() (+1 more)
...
Cohesion: 0.31
Nodes (6): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, raw is lifted verbatim from PDF text and can contain a bracket         sequence, region(), TestScaleTable, TestScaleInfoDefaults
...
Cohesion: 0.14
Nodes (15): detect_doors(), Detect doors. scale_factor scales the world-space gates (1.0 = 1:50).      Built, EntranceDoorTests, line(), PolylineArcBinCapTests, quarter_arc_lines(), Regression + new-feature tests for entrance-door threshold-line handling.      G, Swing-anchored single-line leaf check (v3).      A door panel is often drawn as (+7 more)
...
Cohesion: 0.15
Nodes (22): One drawing on a sheet, found by whitespace segmentation.      bbox is 150-DPI p, Region, _page_summary_dict(), PageRegionResult, The scales block written into each page's summary.json entry., scale_summary_dict(), detection_scale(), DetectionScale (+14 more)

### Community 190 - "TestSlugForPath"
Cohesion: 0.10
Nodes (13): parse_measure_viewports(), Every rectilinear measure viewport, as ``(bbox_pt_yup, c)``.      The bbox is le, _FakeDoc, _FakePage, _Matrix, Parsing /VP -> /Measure viewport dictionaries.  The array strings below are copi, Just enough of fitz.Rect for viewport_scales: x0/y0/x1/y1 attributes., Just enough of fitz.Matrix for page_transform: a/b/c/d/e/f attributes. (+5 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `arcs.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `test_layout_segmenter.py`, `TestExtractImagesInstances`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Wall Cross-Validation`, `Wall Network Construction & Tests`, `Room Detection Tests`, `_dedupe_openings`, `Room Polygonization Internals`, `wall_band_v`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `TestExtractPageFrame`, `geometry.py`, `hline`, `TestWindowTightPairInterior`, `detect_windows`, `_projected_interval`, `renderer.py`, `_collect_wall_faces`, `test_curve_arc_garden_doors.py`, `DoorV2OpeningCheckTests`, `MainExceptionIsolationTests`, `TestNetworkQueries`, `TestThickMaterialPairs`, `renderer.py`, `_FillRing`, `swing_door`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `detect_doors`, `_find_openings`, `app.py`, `_collect_wall_faces`, `qualifying_clip_rects`, `qualifying_clip_rects`, `_segments_min_distance`?**
  _High betweenness centrality (0.166) - this node is a cross-community bridge._
- **Why does `Candidate` connect `TestThickMaterialPairs` to `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `geometry.py`, `TestExtractPageFrame`, `hline`, `TestWindowTightPairInterior`, `_double_arc`, `plumber.py`, `DoorV2OpeningCheckTests`, `renderer.py`, `TestSlugForPath`, `_FillRing`, `swing_door`, `TestDoorGatesUnscaledStopgapRatchet`, `TestMarkerRings`, `DoorV2OpeningCheckTests`, `_find_openings`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `Arc Cap-Trim Tests` to `EntranceDoorTests`, `test_layout_segmenter.py`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Wall Network Construction & Tests`, `arcs.py`, `test_extraction_transform.py`, `TestExtractPageFrame`, `geometry.py`, `TestAnnotationPenBarriers`, `hline`, `TestWindowTightPairInterior`, `detect_windows`, `_double_arc`, `_projected_interval`, `renderer.py`, `scales_in_text`, `DoorV2OpeningCheckTests`, `TestThickMaterialPairs`, `TestSlugForPath`, `swing_door`, `TestDoorGatesUnscaledStopgapRatchet`, `detect_doors`, `app.py`?**
