...
## Corpus Check
- 57 files · ~96,303 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1233 nodes · 3173 edges · 105 communities (49 shown, 56 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 146 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9e9de3c0`
...
Cohesion: 0.07
Nodes (26): _check_opening_clear(), Check if the door opening (bridge between arc endpoints) is free of crossing lin, detect_doors(), _curve(), CurveArcGardenDoorTests, _line(), _quarter_arc_bezier(), Garden-door detection for native single-Bezier (`curve_arc`) swings.  The polyli (+18 more)
...
Cohesion: 0.07
Nodes (37): _cross_validate(), BBox, True when a wall FACE line runs unbroken through the bbox span.      A real wind, Validate doors/windows against the wall-centerline network.      Doors keep the, _wall_runs_through(), BBox, One wall centerline segment (pixel space, y-down)., One merged wall-face run with the evidence its members carried. (+29 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.09
Nodes (24): Trim non-arc cap segments off a 2-leaf simple chain.      Some CAD draftsmen dra, _trim_chain_extension_caps(), _arc(), _chain(), PruneArcCycleCapsTests, Tests for _trim_chain_extension_caps.      Walks a 2-leaf simple chain (no junct, An 11-segment quarter arc has only small inter-seg angle deltas         (~8.2° e, The polyline_393 / linework_226 shape: an 11-seg quarter arc         followed by (+16 more)

### Community 7 - "Debug Trace Collector"
Cohesion: 0.12
Nodes (35): _find_threshold_line(), Find an entrance-door threshold/sill line parallel to the leaf long axis.      T, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+27 more)
...
Cohesion: 0.05
Nodes (41): _bridge_white_runs(), detect_wall_network(), _equivalent_sides(), _is_light_pen(), Build the internal wall-centerline network for a page.      exclude_path_indices, Faint (light-grey/pastel) ink: every channel at/above the light floor., (short, long) of the rectangle with this polygon's area and perimeter.      The, Band-shaped convex hulls closing the gaps in accepted white-ring runs.      A wa (+33 more)
...
Cohesion: 0.16
Nodes (16): _building_masses(), detect_rooms(), _folding_chain_gap_plug(), _free_space_components(), Room detection: rooms are the connected free-space components between walls.  Ea, Fraction of a bbox area covered by the text spans lying over it., Barrier polygon sealing a window opening.      A horizontal/vertical window's bb, Seal the doorway a PARKED folding chain leaves uncovered.      A concertina draw (+8 more)

### Community 13 - "Window Geometry Internals"
Cohesion: 0.17
Nodes (19): _door_fallback_candidate(), _leaf_ink_indices(), _nearest_pair_distance(), _pair_door_assemblies(), BBox, Parse an evidence bbox value defensively; return None on any invalid shape., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, _safe_bbox() (+11 more)
...
Cohesion: 0.33
Nodes (7): door_open_leaf_path_indices(), Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, detect_labels(), Detect architectural labels (e.g. D-01, W-03) near geometric candidates.      Re, run_heuristics(), detect_schedules(), TextSpan
...
Cohesion: 0.50
Nodes (3): _is_dashed(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0"., TestDashDetection

### Community 19 - "Arc Spur-Pruning Tests"
Cohesion: 0.14
Nodes (29): _arc_corners(), _collect_door_swings(), _estimate_arc_sweep_deg(), _is_arc_like(), BBox, Estimate sweep angle of a Bézier arc from its endpoints and estimated center., _collect_door_leaves(), _collect_linework_door_leaves() (+21 more)
...
Cohesion: 0.11
Nodes (14): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, Thin barrier bands along the wall planes through a detected door.      The door, _restrict_swing_plugs(), _swing_hinge_edges() (+6 more)

### Community 22 - "geometry.py"
Cohesion: 0.15
Nodes (19): _detect_curve_arc_double_partners(), _detect_polyline_arc_bboxes(), _prune_arc_cycle_caps(), _prune_arc_spurs(), Remove a small closed-cycle cap attached at a single articulation point.      So, Detect a 2-leaf simple chain that is two arc halves meeting at a hinge.      The, Detect door-swing arcs approximated by connected short line segments.      Some, Pair single-Bezier `curve_arc` swings that form a garden-door split.      The po (+11 more)
...
Cohesion: 0.24
Nodes (10): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _project_onto_axis(), _projected_interval(), Project segment (p1, p2) onto a unit axis and return (lo, hi) scalars., Scalar projection of p onto the unit axis (dx, dy) from origin., _cap_record(), _glaze_record() (+2 more)
...
Cohesion: 0.19
Nodes (12): _collect_fill_rings(), _collect_wall_faces(), _fill_key(), _FillRing, _rate_fill_classes(), A closed same-fill polygon reconstructed from exploded `l` items., Annotation arrowhead: a tiny filled triangle or concave dart.          Walls are, Chain consecutive same-fill `l` items (plus filled re/qu) into rings.      extra (+4 more)
...
Cohesion: 0.05
Nodes (78): cmd_extract(), cmd_inspect(), main(), parse_page_spec(), Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]., build_extract_command(), find_pdfs(), main() (+70 more)
...
Cohesion: 0.14
Nodes (21): Client, _component_indices(), _dedupe_door_components(), Prefer the strongest door when two candidates use the same primitives., build_user_message(), call_gemini(), _candidate_to_dict(), encode_image_inline() (+13 more)
...
Cohesion: 0.08
Nodes (48): _find_anchored_leaf_line(), Search for a single line that could be the door leaf for this arc swing.      Ar, _bboxes_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _band_has_wall_material() (+40 more)

### Community 115 - "constants.py"
Cohesion: 0.15
Nodes (23): _axial_offsets(), _collect_slide_panels(), _detect_sliding_doors(), _fit_oriented_rect(), _line_segs(), _merge_spans(), _pair_leaf_panels(), _panel_shape_ok() (+15 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `geometry.py` to `Pipeline Orchestration & Extraction`, `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `Double-Arc Split Tests`, `Window Geometry Internals`, `Room Polygonization Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `windows.py`, `Arc Spur-Pruning Tests`, `_fit_circle_3pt`, `hline`, `detect_windows`, `plumber.py`, `_projected_interval`, `_collect_wall_faces`, `_FillRing`, `vline`, `TestWindowInteriorClutter`, `plumber.py`, `extractor.py`, `PageData`, `TestWindowArbitraryAngle`, `app.py`, `TestFloorPlansRegression`, `constants.py`?**
  _High betweenness centrality (0.297) - this node is a cross-community bridge._
- **Why does `Candidate` connect `PageData` to `Door Assembly & Heuristics Core`, `Window Detection & Tests`, `Door Detection & Tests`, `Wall Cross-Validation`, `Debug Trace Collector`, `Room Detection Tests`, `Double-Arc Split Tests`, `Window Geometry Internals`, `Arc Cap-Trim Tests`, `Arc Cycle-Cap Pruning Tests`, `_fit_circle_3pt`, `hline`, `detect_windows`, `_collect_wall_faces`, `vline`, `TestWindowInteriorClutter`, `plumber.py`, `TestWindowArbitraryAngle`, `TestFloorPlansRegression`, `constants.py`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `detect_wall_network()` connect `Wall Network Construction & Tests` to `Door Detection & Tests`, `Wall Cross-Validation`, `_collect_wall_faces`, `Room Detection Tests`, `_FillRing`, `app.py`, `Arc Cycle-Cap Pruning Tests`, `geometry.py`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Are the 60 inferred relationships involving `PathPrimitive` (e.g. with `DebugTraceCollector` and `_SlidePanel`) actually correct?**
  _`PathPrimitive` has 60 INFERRED edges - model-reasoned connections that need verification._
- **Are the 35 inferred relationships involving `Candidate` (e.g. with `_SlidePanel` and `TestDoorPenalties`) actually correct?**
  _`Candidate` has 35 INFERRED edges - model-reasoned connections that need verification._