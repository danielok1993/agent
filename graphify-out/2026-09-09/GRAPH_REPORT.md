...
## Corpus Check
- 413 files · ~1,552,809 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5522 nodes · 13544 edges · 330 communities (222 shown, 108 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 1018 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8f2ae57a`
...
- `TestCrossGates` --uses--> `DoorGates`  [INFERRED]
  tests/test_scale_door_gates.py → detection/doors/constants.py
...
## Communities (330 total, 108 thin omitted)

### Community 0 - "Pipeline Orchestration & Extraction"
Cohesion: 0.06
Nodes (32): assess_scale(), check_door_leaves(), _fmt_scale(), leaf_width_px(), _positive(), Is the resolved scale believable? Two checks that read the drawing itself.  A sc, One leaf's width from detector evidence; None when only a bbox exists., The denominator that would put the median leaf at LEAF_TYPICAL_M,     snapped to (+24 more)
...
Cohesion: 0.09
Nodes (38): baseline_dir(), baseline_run(), classify(), compare(), compare_runs(), _crop(), diff_entities(), _draw_entities() (+30 more)
...
Cohesion: 0.09
Nodes (29): agreement(), DimensionMatch, measured_denominator(), page_dimensions(), BBox, The drawing's own dimension strings as a scale MEASUREMENT.  "3600" written besi, The page's matches, on the FULL page — the takeoff's convention, and     what ru, The drawing's measured scale: the median implied denominator over the     matche (+21 more)

### Community 6 - "Double-Door Merge & Gemini Client"
Cohesion: 0.03
Nodes (160): DebugTraceCollector, Record whether a line segment passed the polyline-arc length filter., Record a polyline arc component evaluation. Returns component_id.          ``pre, Mark a previously-collected polyline component as rejected post-hoc., Record a linework leaf component evaluation. Returns component_id.          clea, Record result of the _is_door_leaf check for a primitive., Register a collected swing. Returns swing_id., Pre-populate by_path_index with raw metadata for every PathPrimitive. (+152 more)
...
Cohesion: 0.16
Nodes (9): ChainedCurveSwingDetectionTests, _circle_arc_chain(), _curve(), NativeCurveChainsTests, The door_0051 pattern: native curves with shared endpoints group         into a, End-to-end via detect_doors: a 5-curve chain forming a 30° arc of     a larger c, Build a native cubic Bezier (`c`) primitive from 4 control points., A single `c` primitive that already passes _is_arc_like (square         bbox, si (+1 more)
...
Cohesion: 0.11
Nodes (21): _collect_material_marks(), _dash_row_indices(), _is_dashed(), _pen_key(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0"., Path indices of drawn dash lines: annotation, never faces.      A dashed line ty, (midpoint, angle) of every short solid stroke, gathered once per page.      Thes, Quantized stroke color — the pen identity of a drawn line. (+13 more)
...
Cohesion: 0.12
Nodes (19): assigned_path_fraction(), _centre_in_any(), filter_page_data(), BBox, Reduce a PageData to the primitives inside a set of regions.  This filters, it d, A copy of page_data holding only primitives whose bbox centre falls in     one o, Share of the page's paths that any region would keep.      Deliberately the same, Text spans inside the given regions. Used to scope schedule detection to     sch (+11 more)
...
Cohesion: 0.14
Nodes (19): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., Document, MuPDF's own vector redraw of the page, in render.png's coordinate space.      Sa, render_page_png(), render_page_svg(), _candidate_to_dict() (+11 more)
...
### Community 24 - "hline"
Cohesion: 0.10
Nodes (17): DetectionScale, The scales block written into each page's summary.json entry, and into     takeo, scale_summary_dict(), True when a scale resolved and quantities exist., _contains(), is_verified(), _ratio_pair(), Which drawing scale a room is measured at, and whether it can be trusted.  Pages (+9 more)
...
Cohesion: 0.03
Nodes (87): _interval_overlap(), _area(), _band_interior_clutter(), _cap_orientation_frames(), _clutter_grid(), _dedupe_by_perp(), _dedupe_openings(), detect_windows() (+79 more)
...
Cohesion: 0.05
Nodes (51): dump_truth(), dumps_truth(), _inline_number_array(), _inline_point_array(), _item(), _item_payload(), load_truth(), Path (+43 more)
...
Cohesion: 0.07
Nodes (21): One fixture per paper-space family (spec §Testing). Each fails if its     named, Rotate every primitive's points about (cx, cy) by deg (bbox rebuilt)., The one world-space gate, exercised at a non-grid angle.      A faithful 1:100 e, rot_paths(), TestMinWidthNegativeControl, TestPaperInvariance, hline(), A clean 2-line capped rectangle IS a window on 5-1133 (see Window B:         two (+13 more)
...
Cohesion: 0.13
Nodes (19): Path, Turning a human's selections into committed ground truth.  Pure and terminal-fre, One decision about one detection.      `entity` is the raw dict from a run's fin, Append verdicts to a sheet's ground truth and flag it labeled.      Returns the, record_verdicts(), _truth_item(), Verdict, door() (+11 more)
...
Cohesion: 0.05
Nodes (43): bind_scale(), binding_texts(), _caption_distance(), _centroid(), _contains(), Every text scale near enough to this region to be about it, nearest first., The scale governing one region, or None.      `viewports` must arrive smallest-b, Resolve a scale for every floor-plan region on one page.      `fallback` is a sc (+35 more)
...
Cohesion: 0.08
Nodes (27): _accept_jamb_rings(), _building_masses(), detect_rooms(), _entrance_run(), _folding_chain_gap_plug(), _free_space_components(), _is_door_lining(), Polygon (+19 more)

### Community 120 - "TestNetworkQueries"
Cohesion: 0.13
Nodes (11): door_candidate(), On a colour-coded drawing the wall pens are the pens the sheet's     DOORWAYS ar, Black rect room, 45px doorway in the top band (240..285)., Three red same-pen pairs (unit boxes, 20px deep, 90px long) —         540px of p, Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, rooms_for(), TestBboxSealFloor (+3 more)
...
Cohesion: 0.07
Nodes (24): _back_edge_cover(), _contains_text(), _edge_profile(), _EdgeProfile, _end_closures(), _is_band_pocket(), _is_wall_recess(), Room detection: rooms are the connected free-space components between walls.  Ea (+16 more)
...
Cohesion: 0.03
Nodes (84): _component_indices(), _dedupe_door_components(), door_open_leaf_path_indices(), _leaf_ink_indices(), Prefer the strongest door when two candidates use the same primitives., A single-swing candidate's leaf linework, pinned by its evidence:     leaf line, Path indices of swing doors' OPEN leaf linework.      A swing door's leaf is dra, _bbox_area() (+76 more)

### Community 131 - "test_layout_segmenter.py"
Cohesion: 0.11
Nodes (15): _collect_fill_rings(), detect_wall_network(), _fill_key(), _fill_ring_components(), _fill_seam_indices(), _is_light_pen(), Path indices of fill-ring seams — see _fill_seams., Group ring ids (restricted to `members`) connected by shared seams.      Exporte (+7 more)
...
Cohesion: 0.16
Nodes (9): _door_attribute_overlay(), finalize_candidates(), Selected door-evidence keys to merge into Entity.attributes. {} for None / non-d, Promote candidates to entities, applying the offline confidence floors.      Gem, _room_entity(), cand(), finalize_candidates applies the offline confidence floors unconditionally., TestFinalizeCandidates (+1 more)
...
Cohesion: 0.14
Nodes (8): Extraction puts geometry in the same frame as the declared page size.  page.get_, Every primitive, span AND image must land in the declared frame., A saved 200x400pt PDF with two lines, a word and an image, rotated.      Saved a, Builds all four rotations once; each test reopens what it needs., RotatedPdfTestCase, TestExtractPageFrame, TestPageTransform, write_rotated_pdf()
...
Cohesion: 0.09
Nodes (36): Tunable constants for page segmentation.  Values are measured, not guessed — see, InkMap, bins[row][col] is 1 where drawn ink falls, 0 elsewhere., _boxes_from_cut(), _centre_in(), _chains_across(), _clip_cut(), _col_profile() (+28 more)
...
Cohesion: 0.07
Nodes (38): load_manifest(), manifest_sheets(), Path, Resolution of corpus fixture sheets by slug.  The PDFs are NDA-covered and never, The committed manifest, or an empty corpus when it is absent., Path to a downloaded sheet, or None when it is not on disk., The corpus slug for a PDF path, or None if it is not a corpus sheet.      Compar, Flip a manifest entry's `labeled` flag and write the manifest back.      `labele (+30 more)

### Community 149 - "_arc"
Cohesion: 0.09
Nodes (9): TestCase, Path, Skip helper for tests that need a real corpus sheet.  Corpus knowledge lives in, Return the sheet's path, or skip the test with an actionable message., require_sheet(), LoaderTests, The corpus loader resolves slugs against the committed manifest.  Every test bui, This sheet carries full-page border rules. With the span filter applied     the (+1 more)
...
Cohesion: 0.10
Nodes (21): _draw_dashed_rect(), _draw_entity_box(), _draw_entity_polygon(), _draw_legend(), draw_overlay(), _draw_regions(), _load_font(), BBox (+13 more)
...
Cohesion: 0.09
Nodes (29): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, detection_scale(), `dimensions` is the page's ticked-dimension-string matches on the FULL     page, _fallback_info(), PageScales, The resolution ladder, and how a scale binds to a floor plan.  Binding is what m, A scale the user supplied for the whole run, as a ladder entry.      `source="us (+21 more)
...
Cohesion: 0.11
Nodes (14): Tier 2 — the scale a sheet prints as text.  Three corpus sheets carry no viewpor, Every scale printed on the page, each carrying its span's bbox., text_scales(), Measured scale expectations across the regression corpus.  Every number was meas, s13 is the one corpus sheet whose viewport and printed scale disagree.      It m, The resolver-level assertion: a region sitting inside the measuring         view, read(), TestKnownConflict (+6 more)
...
### Community 171 - "path"
Cohesion: 0.28
Nodes (13): build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), inspect_pdf(), _page_type_styled(), print_candidates_tree(), print_file_header() (+5 more)

### Community 172 - "DoorV2OpeningCheckTests"
Cohesion: 0.19
Nodes (12): EndToEndTests, leaf(), One folding leaf running p -> q, drawn in the Vectorworks joinery     signature:, line(), OrientedRectFitTests, prim(), qu_panel(), A panel pocketed at its -axis end, protruding at the +axis end. (+4 more)

### Community 173 - "test_layout_golden.py"
Cohesion: 0.09
Nodes (20): _collect_wall_faces(), _FillRing, _rate_fill_classes(), Classify each fill color as wall material (True) or furniture (False).      Vect, Path indices of stick-font glyph strokes: annotation, never faces.      A glyph, Return (stroked wall faces, filled-band centerlines)., A closed same-fill polygon reconstructed from exploded `l` items., Annotation arrowhead: a tiny filled triangle or concave dart.          Walls are (+12 more)
...
Cohesion: 0.13
Nodes (21): Entity, attach_takeoff(), _page_summary_dict(), Mirror the per-room takeoff onto room Entity.attributes["takeoff"]., Heights, Quantity takeoff — rooms + scale + heights → floor / ceiling / wall areas., PageFrame, The space every coordinate in this document lives in.      extractor.page_transf (+13 more)
...
Cohesion: 0.12
Nodes (13): TruthItem, Regression corpus: fixture resolution, ground truth, matching, and the sweep., match_entities(), MatchResult, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, entity(), MatchTests, Ground truth is matched to output geometrically.  Entity ids (door_0015) are ord (+5 more)
...
Cohesion: 0.15
Nodes (12): _plane_stamp(), Hold a single swing door to plugs on its hinge edges, one plane only.      A qua, The plug-less fallback seal, restricted to the door's wall-plane edges.      A d, How far, along (ux, uy) from corner, the material a plug tail touches runs., Bbox short-end edges of a sliding door: across the wall, never wall plane., Bbox edges meeting at the hinge corner of a single quarter-swing door.      A sw, _restrict_swing_plugs(), _sliding_end_edges() (+4 more)
...
Cohesion: 0.18
Nodes (16): back_extent(), back_runs(), census(), _gt_class(), make_runs_rule(), Step 17 — `_is_wall_recess`'s back edge on the tab: the EXTENT reading (as imple, Per outer flank (s = -1 / +1 across the band): the union over the gap     of the, The rule's verdict under every reading, with each candidate's numbers. (+8 more)
...
Cohesion: 0.06
Nodes (24): attrib.py <slug> <x0> <y0> <x1> <y1> FIELD=MULT [FIELD=MULT ...]  Runs the harne, attrib_rooms.py <slug> FIELD=MULT [...]: for each config (each alone on top of t, COLLINEAR_OFFSET_TOL forms, measured with the census harness on top of the curre, denom_at(), load(), mm(), W-gate census harness: run the stage-5 chain exactly as tools/regress.py does, w, Stage-5 chain as run_heuristics does it (labels/schedules omitted),     then the (+16 more)
...
Cohesion: 0.08
Nodes (17): _stored_info(), canonical_denominators(), cluster_denominators(), denominator_from_c(), Group near-equal denominators, largest group first in input order.      Lives he, One representative per cluster — how many DISTINCT scales are present., The 1:N denominator for a /Measure /X conversion factor., The nearest standard scale within tolerance, or None.      None is a real answer (+9 more)
...
Cohesion: 0.08
Nodes (17): TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet, A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., A filled wall band exported as two triangles (CAD fill triangulation).      Each, A chimney breast / pier drawn as a closed box on the room side of a     wall ban, A window reveal in a cavity wall (s17 rooms 0015/0034): the wall is     drawn as, An ENTRANCE is a door seal that runs ALONG the space's boundary over     at leas, TestBandPocket (+9 more)
...
Cohesion: 0.02
Nodes (192): _angle_diff_mod180(), _bbox_expanded(), _line_angle_deg(), _line_length(), _perpendicular_spacing(), _point_in_bbox(), _point_to_segment_distance(), _project_onto_axis() (+184 more)
...
### Community 274 - "render_pens.py"
Cohesion: 0.16
Nodes (12): main(), Step-11 checkpoint pictures: s01 at its true factor (0.542), the share gate alon, draw_geom(), main(), panel(), Checkpoint pictures for the _gate_denominator step (s01 identity vs 0.542):   1., The two step-9 pictures (guarded so crop_step10.py can import the drawing helper, side_by_side() (+4 more)
...
Cohesion: 0.20
Nodes (5): A wall face paired ACROSS the room with parallel wall-pen ink — a     kitchen co, A DRAWN DASH LINE — one line its line type exploded into a periodic     row of s, TestDashRowExclusion, TestFarSidePairs, vline()
...
### Community 304 - "TestSpanFilterIsLoadBearing"
Cohesion: 0.47
...
Cohesion: 0.15
Nodes (7): hline(), path(), Partition wall in the joinery pen: two hairline faces with diagonal     hatch st, The material-mark length cap is per band (_mark_len_cap): the     page-wide WALL, TestPerBandMarkCap, TestWeakFacePairs, weak_hatched_band_h()
...
## Knowledge Gaps
- **854 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+849 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `Double-Door Merge & Gemini Client` to `swing_door`, `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Pipeline Orchestration & Extraction`, `TestWindowArbitraryAngle`, `TestVectorTextExclusion`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `TestThickMaterialPairs`, `Double-Arc Split Tests`, `_leaf_ink_indices`, `Room Polygonization Internals`, `Room Detection Tests`, `overrides`, `test_extraction_transform.py`, `Arc Cycle-Cap Pruning Tests`, `client.py`, `TestAnnotationPenBarriers`, `_production_door_gates_unscaled_usages`, `Outcome — iteration 3, step 10 (2026-09-05, branch `fix/material-seeking-plug-tail`, shipped pending the user's decision)`, `TestSwingHingePlugRestriction`, `plumber.py`, `_projected_interval`, `renderer.py`, `Outcome — iteration 3, step 4 (2026-09-06, branch `recal/wall-max-thickness-40`, measurement only — `WALL_MAX_THICKNESS_PX` NOT moved)`, `DoorV2OpeningCheckTests`, `test_layout_golden.py`, `TestSpanFilterIsLoadBearing`, `jamb_pens.py`, `_is_light_pen`, `TestDashRowBarriers`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `TestDashRowDiscriminators`, `collect_room_spans`, `squat_cap_window`, `TestMarkerRings`, `bezier_arc`, `test_door_assembly.py`, `detect_doors`, `DoorV2OpeningCheckTests`, `vline`, `_find_openings`, `ablate.py`, `TestBandPocket`, `_collect_wall_faces`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`, `label_rooms`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `_fit_circle_3pt` to `Pipeline Orchestration & Extraction`, `EntranceDoorTests`, `Double-Door Merge & Gemini Client`, `client.py`, `_dedupe_openings`, `ShaMismatchAgainstTruthTests`, `Arc Cycle-Cap Pruning Tests`, `arcs.py`, `resolver.py`, `TestCliEquivalence`, `review.py`, `_projected_interval`, `ScaleInfo`, `_collect_wall_faces`, `path`, `TestXYCut`, `TestCheckDoorLeaves`, `TestSlugForPath`, `TestWindowTightPairInterior`, `resolve_page_regions`, `TestAnnotationPenBarriers`, `test_door_assembly.py`, `resolve_heights`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `TestVectorTextExclusion` to `Pipeline Orchestration & Extraction`, `EntranceDoorTests`, `test_layout_segmenter.py`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `client.py`, `Wall Network Construction & Tests`, `TestThickMaterialPairs`, `_leaf_ink_indices`, `Arc Cycle-Cap Pruning Tests`, `overrides`, `test_extraction_transform.py`, `arcs.py`, `_fit_circle_3pt`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `_projected_interval`, `renderer.py`, `PruneArcSpursTests`, `test_layout_golden.py`, `TestDashRowBarriers`, `EntranceDoorTests`, `_is_light_pen`, `TestSheetSize`, `TestWindowExteriorSide`, `test_batch_extract.py`, `TestWindowTightPairInterior`, `_covers`, `collect_room_spans`, `squat_cap_window`, `test_door_assembly.py`, `vline`, `ablate.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `_segments_min_distance`, `framed_triple_window`, `label_rooms`?**
