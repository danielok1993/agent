...
## Corpus Check
- 278 files · ~540,347 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4775 nodes · 12091 edges · 254 communities (180 shown, 74 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 931 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2d48c3df`
...
- [[_COMMUNITY_filter_page_data|filter_page_data]]
...
5. `detect_wall_network()` - 130 edges
6. `Region` - 116 edges
7. `PageScales` - 94 edges
8. `ScaleInfo` - 91 edges
9. `rooms_for()` - 80 edges
...
## Communities (254 total, 74 thin omitted)
...
Cohesion: 0.07
Nodes (42): _component_indices(), _dedupe_door_components(), _door_fallback_candidate(), door_open_leaf_path_indices(), _find_threshold_line(), _leaf_ink_indices(), _nearest_pair_distance(), _pair_door_assemblies() (+34 more)
...
Cohesion: 0.11
Nodes (21): _collect_material_marks(), _collect_stroked_rings(), _is_dashed(), _pen_key(), True for a real dash pattern; PyMuPDF encodes solid as "" or "[] 0"., (midpoint, angle) of every short solid stroke, gathered once per page.      Thes, Quantized stroke color — the pen identity of a drawn line., A small closed STROKED (fill-less) outline in one pen — a jamb-nib /     door-st (+13 more)
...
Cohesion: 0.11
Nodes (33): _load_font(), baseline_dir(), baseline_run(), classify(), compare(), compare_runs(), _crop(), diff_entities() (+25 more)
...
Cohesion: 0.08
Nodes (12): CrossGates, World-space cross-validation gates, pre-multiplied by the factor.      Only the, prim(), quarter_bezier(), A quarter-arc cubic Bezier of radius r, hinged at (cx, cy).      r is a WORLD ex, TestArcGatesThreading, TestAssemblyGatesThreading, TestCrossGates (+4 more)
...
Cohesion: 0.06
Nodes (19): main(), mult_for(), Per-constant ablations.    python ablate.py s01 s01mode   # f=0.542 full, scale-, attrib.py <slug> <x0> <y0> <x1> <y1> FIELD=MULT [FIELD=MULT ...]  Runs the harne, attrib_rooms.py <slug> FIELD=MULT [...]: for each config (each alone on top of t, COLLINEAR_OFFSET_TOL forms, measured with the census harness on top of the curre, denom_at(), load() (+11 more)
...
Cohesion: 0.07
Nodes (59): DoorGates, World-space door gates, pre-multiplied by the detection factor.      Fields keep, _absorb_hinged_white_rings(), _detect_folding_doors(), _double_line_leaves(), _fold_edges(), _fold_groups(), _leaf_tip() (+51 more)
...
Cohesion: 0.16
Nodes (21): _apply(), _as_transform(), classify_page(), extract_document(), extract_images(), extract_page(), extract_text(), get_ocg_names() (+13 more)
...
Cohesion: 0.11
Nodes (15): Regression corpus: fixture resolution, ground truth, matching, and the sweep., iou(), match_entities(), MatchResult, BBox, Matching ground-truth items to pipeline output.  Entity ids are ordinal — door_0, entity(), IouTests (+7 more)
...
Cohesion: 0.04
Nodes (120): _find_leaf_companion_lines(), Find lines forming the same thin-rect leaf as the anchored leaf line.      Door, _angle_diff_mod180(), _bboxes_overlap(), _interval_overlap(), _line_angle_deg(), _line_length(), _perpendicular_spacing() (+112 more)
...
Cohesion: 0.05
Nodes (36): _effective_denominator(), _gate_denominator(), One detection factor per page: which scale governs the ink detection sees.  Dete, Nominal beats raw so 1:50 sheets compute factor 1.0 EXACTLY., The denominator allowed to drive gate scaling, or None to abstain.      Only a D, Drawing-scale resolution: read a 1:N scale from the PDF and bind it to a plan., Tier 4 input — ask the user, but only when someone is there to answer.  batch_ex, binding_texts() (+28 more)

### Community 119 - "qualifying_clip_rects"
Cohesion: 0.08
Nodes (39): _accept_jamb_rings(), _building_masses(), _clip_plug_tails(), _contains_text(), detect_rooms(), _drop_window_exterior_sides(), _edge_face_cover(), _folding_chain_gap_plug() (+31 more)

### Community 120 - "TestNetworkQueries"
Cohesion: 0.17
Nodes (7): door_candidate(), Fallback-tier door candidates (label boxes, symbol clutter — kept     only for G, The dilated-bbox fallback is the one seal with no evidence of its     own, so it, rooms_for(), TestBboxSealFloor, TestComponentFiltering, TestPhantomDoorSeals
...
Cohesion: 0.10
Nodes (20): build_parser(), cmd_extract(), cmd_inspect(), main(), parse_page_spec(), positive_metres(), argparse type: a positive, finite height in metres., Parse '1,3-5' into 0-based page indices [0, 2, 3, 4]. (+12 more)

### Community 123 - "batch_extract.py"
Cohesion: 0.05
Nodes (37): detect_wall_network(), _fill_seam_indices(), Path indices of fill-ring seams — see _fill_seams., Build the internal wall-centerline network for a page.      exclude_path_indices, fan_triangulated_band_h(), hline(), path(), paving_field() (+29 more)
...
Cohesion: 0.08
Nodes (22): _collect_fill_rings(), _collect_wall_faces(), _fill_key(), _fill_seams(), _FillRing, _rate_fill_classes(), Background-colored rings that could be hollow walls or built-in runs.      White, (seam path indices, ring-id pairs sharing a seam).      Path indices of fill-rin (+14 more)
...
Cohesion: 0.14
Nodes (11): hline(), path(), Lone thin barriers require a wall pen. On color-coded drawings the     annotatio, Filled arrowhead triangle (a marker ring) pointing down at `tip`., Stairs are furniture to the room stage: a room polygon runs to the     enclosing, rect_room(), stair_arrowhead(), TestAnnotationPenBarriers (+3 more)
...
Cohesion: 0.24
Nodes (8): detect(), fold_chain(), FoldChainTests, folding_of(), parked_stack(), ParkedStackPairTests, A concertina run of hinged leaves, leaf k at angles_deg[k]., Two leaves fanned open from one shared hinge (a parked bifold V).
...
Cohesion: 0.09
Nodes (30): A drawing scale, and the evidence it came from.      `denominator` 100.0 means 1, ScaleInfo, _page_summary_dict(), PageRegionResult, The per-region scale table printed after each page., scale_table(), detection_scale(), DetectionScale (+22 more)
...
### Community 168 - "QuadPerimeterTests"
Cohesion: 0.21
Nodes (11): _fit_oriented_rect(), Fit an oriented rectangle to a bag of corner points, any rotation.      Orders u, EndToEndTests, leaf(), One folding leaf running p -> q, drawn in the Vectorworks joinery     signature:, OrientedRectFitTests, prim(), qu_panel() (+3 more)
...
Cohesion: 0.09
Nodes (25): PageTruth, SheetTruth, TruthItem, evaluate_page(), _labeled_but_unreviewed(), True when the manifest claims this sheet has been labeled but its     ground tru, Score one sheet's per-page pipeline output against its ground truth.      `pages, Score one page's entities against its three verdict lists. (+17 more)

### Community 179 - "filter_page_data"
Cohesion: 0.21
Nodes (6): A doorway whose jamb is a one-wall-thickness nib (s03 door_0018)., Rect room with a 45px doorway gap in the top wall (240..285)., TestJambNib, TestOpeningSeals, wall_band_h(), wall_band_v()
...
Cohesion: 0.07
Nodes (45): generate_debug_viewer(), Generate a self-contained HTML debug viewer for door detection traces., Write a single-file HTML viewer embedding the render image and trace JSON., build_plumber_counts(), build_pymupdf_counts(), compare_counts(), _delta_pct(), extract_plumber_document() (+37 more)
...
Cohesion: 0.08
Nodes (25): Evidence: what broke at f = 50/92.2 = 0.542 (all measured on the real PDF), Group 1 — safe reference moves, Group 2 — thin-margin moves (three of five tried and reverted), Group 3 — class fixes, Handoff: W-gate recalibration (the proper fix behind `fix/measured-scale-detection-factor`), How the ablation was done (reproduce in ~30 min), Outcome — iteration 2 (2026-09-04, branch `recal/w-gate-iter2`), Outcome — iteration 3, step 1 (2026-09-04, branch `fix/section-line-dashes-not-hatch`) (+17 more)

### Community 197 - "SplitDoubleArcTests"
Cohesion: 0.24
...
Cohesion: 0.10
Nodes (18): Scale-factor behavior of walls/rooms gates: identity at 1.0, shrunk-world at 0.5, Scale coordinates by s, keep stroke widths — a 1:100 export., A closed 400x300 room drawn as four double-line wall bands., room_box_walls(), rooms_for(), shrink(), TestOrchestratorForwardsFactor, TestRoomsScaled (+10 more)
...
### Community 222 - "TestSheetIsScaled"
Cohesion: 0.40
Nodes (3): BBox, True when any centerline corridor (dilated by thickness/2 + expand) hits bbox., Max fraction of the bbox long axis covered by one near-collinear centerline.
...
Cohesion: 0.07
Nodes (24): _door_plugs(), _open_leaf_edges(), Bbox edges of a garden-layout double door that are room floor, not wall.      A, Bbox short-end edges of a sliding door: across the wall, never wall plane., Thin barrier bands along the wall planes through a detected door.      The door, _sliding_end_edges(), TextSpan, Room detection tests (detection/rooms.py).  Fixtures build wall bands as synthet (+16 more)
...
### Community 248 - "_dedupe_openings"
Cohesion: 0.50
...
## Knowledge Gaps
- **661 isolated node(s):** `storage`, `sheets`, `What "generic" means here (the rule that overrides all others)`, `What counts as a win`, `1. Intake — extract the brief` (+656 more)
...
- **What is the exact relationship between `Door (architectural element)` and `floor-plans Proposed Ground & First Floor Plans (3 Penparcau Road)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `PathPrimitive` connect `bezier_arc` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `Window Detection & Tests`, `test_layout_segmenter.py`, `TestWindowArbitraryAngle`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `client.py`, `client.py`, `Room Polygonization Internals`, `_check_opening_clear`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `arcs.py`, `_fit_circle_3pt`, `_arc`, `TestAnnotationPenBarriers`, `review.py`, `viewport_bbox_to_px`, `TestBlindWindowPocket`, `TestSwingHingePlugRestriction`, `_projected_interval`, `test_curve_arc_garden_doors.py`, `renderer.py`, `batch_extract.py`, `QuadPerimeterTests`, `path`, `filter_page_data`, `test_window_detection.py`, `pipeline.py`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_sliding_doors.py`, `test_batch_extract.py`, `._spy_kwargs`, `_covers`, `TestNetworkQueries`, `squat_cap_window`, `wall_band_h`, `TestRenderPageSvg`, `TestMarkerRings`, `TestMinWidthReference`, `TestWindowSpanOvershootRetune`, `detect_doors`, `attrib_rooms.py`, `vline`, `_find_openings`, `app.py`, `_collect_wall_faces`, `TestNetworkQueries`, `batch_extract.py`, `framed_triple_window`, `_segments_min_distance`?**
  _High betweenness centrality (0.144) - this node is a cross-community bridge._
- **Why does `run_extract()` connect `test_window_detection.py` to `Pipeline Orchestration & Extraction`, `Door Detection & Tests`, `plumber.py`, `ScaleInfo`, `Double-Door Merge & Gemini Client`, `Debug Trace Collector`, `detect_doors`, `batch_extract.py`, `Wall Network Construction & Tests`, `PruneArcSpursTests`, `DoorAssemblyTests`, `TestAnnotationPenBarriers`, `resolver.py`, `_fit_circle_3pt`, `TestCliEquivalence`, `test_door_assembly.py`, `TestXYCut`, `TakeoffRequest`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `TextSpan` connect `squat_cap_window` to `Door Assembly & Heuristics Core`, `EntranceDoorTests`, `Door Detection & Tests`, `Window Detection & Tests`, `Double-Door Merge & Gemini Client`, `Arc Detection Primitives`, `Room Detection Tests`, `Wall Network Construction & Tests`, `client.py`, `client.py`, `Arc Cycle-Cap Pruning Tests`, `test_extraction_transform.py`, `_fit_circle_3pt`, `geometry.py`, `TestAnnotationPenBarriers`, `test_layout_segmenter.py`, `TestSwingHingePlugRestriction`, `ScaleInfo`, `renderer.py`, `batch_extract.py`, `filter_page_data`, `test_window_detection.py`, `TestWindowTightPairInterior`, `TestWindowExteriorSide`, `_is_light_pen`, `TestSheetSize`, `SplitDoubleArcTests`, `TestWindowExteriorSide`, `test_batch_extract.py`, `TestWindowTightPairInterior`, `TestRenderPageSvg`, `TestMarkerRings`, `detect_doors`, `vline`, `app.py`, `TestAnnotationPenBarriers`, `qualifying_clip_rects`, `TestNetworkQueries`, `batch_extract.py`?**
