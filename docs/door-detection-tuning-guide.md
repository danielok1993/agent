...
- **pocket_leaf**: a WHITE panel flanked on BOTH sides by wall-face lines (gap ∈ [0.5, 12] px beyond the panel edge, each face ≥ 0.4 × panel length, each side covering ≥ 0.25) over 35–90 % of its length, protruding 8–65 % at one end, where the protrusion zone contains ≤ `DOOR_SLIDE_ZONE_MAX_CROSSERS` crossing lines (a wall strip "protrudes" into hatch — ≥3 crossers; a doorway is clear, jamb caps ≤2) AND the panel bbox overlaps no collected swing (an open hinged leaf near its arc is the swing's business). White-ring representation is the default requirement for this weaker single-panel evidence tier (open hinged leaves on 5-1133 are stroked `qu` only); a BARE stroked/`qu` panel qualifies only when BOTH flank gaps are pocket-tight, ≤ `DOOR_SLIDE_POCKET_TIGHT_GAP_PX` (2.5 px) — the cavity hugging the leaf (s04 door_0014: a 4.7×92 px `qu` at 0.56 px, 54 % inside its split pocket wall, gaps 1.1/1.2 px; GD9's white panel measures 2.9–6.1 and keeps the 12 px allowance): an open hinged leaf against a wall is flanked on one side only, and nothing but a cavity draws two faces ~1 px off a leaf's edges. The pocket leaf then looks for its FAR JAMB under the slide law, like parked_leaf: a line ending in, or crossing, the corridor one panel length out of the pocket mouth (a pier's end cap spans the whole band, so its endpoints lie outside the leaf's corridor — the crossing counts) within `DOOR_SLIDE_PARK_SPAN_RATIO_TOL`, with ≤ `DOOR_SLIDE_ZONE_MAX_CROSSERS` crossers in between (s04: pier cap 90 px vs panel 91.7, the slide arrow's 45° tick is the one crosser). Found, the corridor joins the emitted bbox (`opening_span_px`, `opening_bbox`) so the room stage plugs the doorway the half-drawn leaf never covers — s04's BATHROOM 02 merged with its bedroom through the 741–790 px span the panel bbox left open; not found, the bbox stays the panel's own (5-1133 GD9 unchanged on the sweep).
...
| `DOOR_LAYER_KEYWORDS` | `["door", "a-door"]` | Exact-token match in the layer name (`detection/layers.py::_layer_tokens`), singular OR plural — a token of ≥ 4 chars ending in `s` also contributes its stem, because CAD conventions pluralise the class name (s03/s17 `A325G_INT_DOORS`; every door/window/wall layer on the six layered corpus sheets is plural, and the singular-only match never fired on any of them until 2026-08-25). Still no substring match: `doorstops` misses. The hint fires only when the layer names exactly ONE element class (`LAYER_CLASS_KEYWORDS` door/window/wall): a layer naming two — s04 `RR_New Doors and Windows` — is a joinery grouping layer that says door OR window, never which, and hints at neither (measured 2026-08-26: 16 of the corpus's 17 class-naming layers name one class; on s04 the door prior had fired on 3 window paths, no candidate affected). 14 of the 20 corpus sheets have no OCG layers at all, so the hint is a no-op there. |
...
- **Zero-length `l` items are never lines:** CAD exports scatter degenerate `l` items (point marks, stipple/vegetation dots, hatch remnants) across 17 of the 20 corpus sheets — s15 8,715, s12 4,786, s11 4,082, s05 4,006, s16 3,996, s17 1,385, s02 406, s01 none. A dot has no direction and no interval, so any rule that admits lines by PROXIMITY alone (a parallel companion to a leaf, a crosser in an `open_v` corridor, an obstruction in the swing bridge of `_check_opening_clear`) passes it on the distance test with the geometric test silently skipped. `detection.geometry._is_line_path` is the single gate every door proximity loop goes through and rejects lines shorter than `LINE_MIN_LEN_PX` (1e-6); `DOOR_LEAF_COMPANION_MIN_LEN_PX` (§4.7) is the same rule stated at the companion finder, where it was first found (s11/s16 half-detected double doors). Any new rule that takes `l` items by distance must go through `_is_line_path` — never `p.item_type == "l"` on its own.
