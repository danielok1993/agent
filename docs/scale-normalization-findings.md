...
| WALL_THICK_MATERIAL_MAX_PX | W | **56px = 475mm band at 1:50** (was 48px = 406mm until the W-gate census iteration 2, 2026-09-04, row 4). The "s01 pier 19 → 39px" it was set on is 613mm at s01's true 1:92.2; the true class — material-backed thick pairs — reaches 377–400mm on s15/s20 (47.2–47.3px = 1.01× under 48), 305–336mm on s16 and 475mm on s05; 56 leaves 1.19×. Moved only together with the per-band mark cap (`_mark_len_cap`, row below): at f=0.5 a 56 cap is 28px and s05's wall pairs at exactly 28, so it falls from the THROUGH tier into the THICK tier, whose fixed 24px mark cap could not see the wall's 39px strokes — with the per-band cap it pairs there (s05 9/9 rooms). The false class (hatched fixtures/floors from 349mm on s15, 495 on s02, 400+ on s20) is gated by material, never by this cap. Pinned by `tests/test_wall_network.py::TestPerBandMarkCap::test_fifty_px_band_with_short_interior_hatch_pairs_in_thick_tier` (fails at 48: the band lands in the through tier). **Since iteration 3 step 16 (2026-09-06) this cap is also the band-pocket ceiling**: `RoomGates` carries it (× f, exactly as `WallGates`) and `_is_band_pocket` reads it in place of `WALL_MAX_THICKNESS_PX` — two faces that could have paired as one wall, plain or thick — with enclosure by wall bands at both ends (`ROOM_BAND_POCKET_END_CLOSURE_MIN`) keeping the walled cupboard out; measured as implemented at 36 / 40 / 41 / 44 / 48 / 56 on all 20 sheets: at 56 it drops s17's four hollow-wall strips (328–343mm), s18's kitchen-corner box (360mm), s12's two unit cells (442 / 470mm) and s18's sofa-back strip (462mm) — eight recorded FPs, 0 lost — while the next real door-less spaces sit at 599–610mm (1.26–1.29× over). Pinned by `tests/test_room_detection.py::TestBandPocketCeilingAtTheThickTier` and `tests/test_scale_gates.py::TestRoomGatesConstruction::test_wall_thick_material_max_matches_wallgates_scaling` |
...
| ROOM_ENTRANCE_MIN_RUN_PX | W | how far an ENTRANCE seal runs ALONG the space's boundary (29.5px = 250mm at 1:50; 14.75px at 1:100, 16px at s01's 1:92.2, 10.8px at s13's 1:136), read net of the paper `ROOM_CONTACT_TOL_PX` at each end (`_entrance_run`). Iteration 3 step 14 (2026-09-06, `docs/w-gate-iter3-checkpoints/step-14.md`): the any-touch entrance test (a ≥ 0.55 seal within 4px of the boundary) read a doorway cut THROUGH the wall a strip lies inside of — its plug collinear with the strip, meeting the strip's end over the plug's cross-section — as the strip's entrance, and s17's four cavity-wall reveal strips escaped `_is_band_pocket` on it. Measured on every emitted room of all 20 sheets at their factors: per room the LARGEST run any entrance seal makes (a neighbour's tail grazes real rooms over 3.6–15px, so the statistic is per room, never per seal) is ≥ 59.2px on every confirmed entered room at f=1.0 (s03), 36–43px at f=0.5 (s18 36.0, s11/s16 36.5, s05 37.2, s06 38.7, s07 43.2, s12 43.0), 37.3px at s13's 0.367, 47.3px on s01 at 0.542; the false class — the s17 strips at 7–10px and s04's recorded-FP box at 13.5px — never exceeds a plug's cross-section (2 × `ROOM_PLUG_HALF_WIDTH_PX`). The tolerance is subtracted because it is PAPER: a crossing plug's raw contact is 2 × half-width + 2 × TOL, 18px at 1:50 but 12px at 1:136 against a 13px scaled floor, so no world floor on the RAW contact clears both classes at every factor, while on the net run 250mm sits 2.2× over the false ceiling and 2.0× under the true floor at f=1.0, 3.0× / 2.4× at f=0.5, 2.7× / 3.4× at f=0.367. Feeds the entrance gate only (the blind-window, wall-recess and band-pocket drops); `door_openings` and the confidence boost still count every touching seal. Corpus sweep: verdict-identical in counts (0 lost, 68 returned FPs, 0 REVIEW), 19 sheets entity- and polygon-identical, s01/s02 untouched; on s04 the recorded-FP box (1463,1042)–(1558,1131) is dropped as a blind-window pocket and the recorded-FP stair flight (1588,1053)–(1762,1131) on the other side of window_0004 returns, because the box was the door-bearing side that `_drop_window_exterior_sides` dropped the flight against — a trade, net 0; s17's four strips become entrance-less and STILL stay (their rotated rectangles are pinned by the 31.5px tabs and their spacing is over the cap). Pinned by `tests/test_room_detection.py::TestEntranceRunsAlongTheBoundary` |
| ROOM_BAND_POCKET_END_CLOSURE_MIN | D | fraction of an END of a band-pocket candidate that a wall SOLID stands behind (`_end_closures`, read on the component's own boundary runs at `ROOM_BAND_POCKET_END_PROBE_PX`); a component closed at BOTH ends by at least this much is enclosed by walls on all four sides and `_is_band_pocket` exempts it. Iteration 3 step 16 (2026-09-06, `docs/w-gate-iter3-checkpoints/step-16.md`): s11's confirmed "storage in utility" is not between two walls — a 5.7px partition on one side, its FRONT drawn as one wall-pen line on the other (the s02 "coats" class), so "material behind both sides" reads 1/2 on it as on s17's dropped 25.25px reveal — but a 6px band above and the 17.6px external wall below close it: 1.0 / 1.0. s17's four hollow-wall strips (313mm, two lines with rooms on both sides) end at a doorway's jamb line and a partition's face continuing across: 0.345 / 0.0, 0.338 / 0.201, 0.001 / 0.001, 0.0 / 0.0 (the 0.34 a jamb block behind a third of the doorway end); the 25.25px reveal 0.0 / 1.0. 0.65 sits 1.54× under the storage and 1.88× over the strips; the corpus's other confirmed door-less spaces (s07's cupboard 0.06 / 0.06, s20's passage 0.0 / 1.0, s15's space 1.0 / 0.5) are boxes of lone lines at 599–610mm, held out by the ceiling alone. Pinned by `tests/test_room_detection.py::TestBandPocketEnclosedByWallBands` (both tests bite: the banded cupboard fails with the exemption reverted, the nibbed hollow wall fails at a 0.3 floor) |
| ROOM_BAND_POCKET_END_PROBE_PX | P | where an end's closure is read: 7px outward of the boundary run. The run lies `ROOM_LINE_BARRIER_PX` ± `ROOM_RECESS_BACK_TOL_PX` off its face and a lone face's line barrier reaches `ROOM_LINE_BARRIER_PX` past the face, so nothing but a solid lies past 5.5px from the run, while a band behind the face is dilated `ROOM_WALL_DILATE_PX` past it and spans its thickness beyond — paper-space standoff geometry, inside any band's solid and 1.5px past any lone face's reach; 6 and 7 read identically on every component of the step-16 census |
| ROOM_RECESS_BACK_COVER_MIN | D | fraction of a wall-recess candidate's own extent along the collinear gap over which its back lies on the band's outer line — the union of its boundary runs parallel to the band at `ROOM_LINE_BARRIER_PX` ± `ROOM_RECESS_BACK_TOL_PX` inside the line, or ON it where a wall solid's flat end (`cap_lines`) lies on the run (`_back_edge_cover`). Iteration 3 step 17 (2026-09-06, `docs/w-gate-iter3-checkpoints/step-17.md`): `_is_wall_recess` read the back edge off the component's EXTENT (the extreme vertex across the band at 2 ± 1.5 inside the line), and the step-15 tab — a perpendicular partition's flat-capped segment ending ON the band's face line, the line drawn from its far flank on — pinned the extreme ON the line (standoff 0), flipping the verdict on `TestWallRecessTabbedByAPerpendicularBand`'s reveal (31.5px of tab on an 80px back: 0.61 on the face alone, 1.0 with the cap). Measured on every call (72) and every emitted room (236) of all 20 sheets at their factors: the 14 calls that reach the back edge (breast pockets and piers 0.85–2.94 bands deep in 112–466mm bands, all dropped today) read the extreme at −2.00 (s10 −2.12) and the runs at 1.00 of their own extent; no emitted room reaches the back edge; the corpus is identical under either reading (0 LOST / 60 returned FPs / 0 REVIEW, every sheet entity- and polygon-identical). Over the component's own extent, not the gap's (the gap-cover gate already asks how much of the gap is filled; over the gap the 14 read 0.74–1.00). Pinned by `tests/test_room_detection.py::TestWallRecessTabbedByAPerpendicularBand` (bites: the tabbed reveal survives with the detector reverted) |
