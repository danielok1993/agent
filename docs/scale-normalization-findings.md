...
| ROOM_OPENING_SEAL_PX | W | reach into jambs the arc stopped short of (12px = 102mm at 1:50). The jamb gap beyond a swing bbox at true scales is s01 8px = 125mm (1:92.2 — the 12 was set on it as if 1:50, which is why f=0.542 leaks a room), s17 8px = 135mm, s05/s07 6px @f=0.5 = 102mm (exactly the scaled tail, zero headroom); the false class is tails re-fencing known phantoms, s03 returning two recorded FP rooms at 18px. **The W-gate census (row 17) proposed 15 and iteration 2 (2026-09-04) tried and reverted it — no value above 12 is safe**: a tail TOUCHES material within `ROOM_PLUG_HALF_WIDTH_PX` of its end, so any bbox edge whose ends lie within SEAL + 5px of two walls qualifies as an interrupted run, and for a door with no derivable hinge edge that includes its SWING side — at 13–14 s01 room_0005 moves, at 14 s15's lounge and corridor lose their door swings (rooms 0023/0024, −5.4k px² each), at 15 s02's BEDROOM 2 is notched around the "A" section-marker bar (a fallback door on it); only 15 also cleaned s04's bedroom (+10k px²). The prerequisite is a swing-side veto for hinge-less doors. An interrupted plug seals a jamb gap of at most SEAL − 1px; two synthetic fixtures that sat at exactly 20px clearance were moved to 26–28px. Pinned by `tests/test_room_detection.py::TestPlugSealReach` |
